...
export const useColourStore = defineStore("colour", () => {
  ...
  const addColourMapping = async (
    ...
    const resp = await addColour(httpClient)(
      game._id,
      data.playerId,
      data.colour,
    );
  ...
  const getColourForPlayer = (
    ...
    if (colourOverride.value) {
      colour =
        colourMapping.value?.[playerId] ||
        GameHelper.getPlayerById(game!, playerId)!.colour;
    } else {
