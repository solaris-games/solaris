...
export const withMessages = () => {
  ...
  const sendForAllGames = computed(
    () => userStore.user?.subscriptions?.inapp?.notificationsForOtherGames,
  );
  ...
  const handler = (e: ConversationMessageSentResult<string>) => {
    ...
    if (isInGame) {
      ...
      const text = gameName
        ...
        : `In another game, you have received a message from ${e.fromPlayerAlias}.`;
