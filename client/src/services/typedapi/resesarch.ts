...
export const updateResesarchNow = (axios: Axios) => async (gameId: string, research: ResearchTypeNotRandom): Promise<ResponseResult<ResearchUpdateResponse>> => {
  return doPut(axios)(routes.updateNow, { gameId }, {}, { preference: research }, { withCredentials: true });
};

export const updateResesarchNext = (axios: Axios) => async (gameId: string, research: ResearchType): Promise<ResponseResult<ResearchUpdateResponse>> => {
