...
class GameHelper {
  ...
  getStarsOwnedByPlayer(player: Player, stars: Star[]) {
    ...
    return stars.filter(
      (s) => s.ownedByPlayerId && s.ownedByPlayerId === player._id,
    );
  }
  ...
  isCarrierInTransitToWaypoint(carrier: Carrier, waypoint) {
    return (
      carrier.waypoints.indexOf(waypoint) === 0 &&
  ...
  getStarTotalKnownShips(game: Game, star: Star) {
    ...
    return (
      (star.ships || 0) + carriers.reduce((sum, c) => sum + (c.ships || 0), 0)
    );
  }

  getHyperspaceDistance(game, player, carrier) {
    return (
      ((carrier.effectiveTechs.hyperspace || 1) + 1.5) *
      game.constants.distances.lightYear
    );
  }

  getScanningLevelByDistance(game, distance) {
    let distancePerLevel = Math.ceil(
      distance / game.constants.distances.lightYear - 1,
    );
  ...
  getHyperspaceLevelByDistance(game, distance) {
    let distancePerLevel = Math.ceil(
      distance / game.constants.distances.lightYear - 1.5,
    );
  ...
  isInstantTravel(prev: RulerPoint, curr: RulerPoint) {
    if (prev.type === "star" && curr.type === "star") {
      ...
      return (
        prevObj.wormHoleToStarId === currObj._id &&
        currObj.wormHoleToStarId === prevObj._id
      );
    }
  ...
  getTickDistance(
    ...
    tickDistanceModifier = 1,
  ) {
    let tickDistance =
      game.settings.specialGalaxy.carrierSpeed * tickDistanceModifier;
  ...
  getCarrierSpeed(
    ...
    const canWarpSpeed = this.canTravelAtWarpSpeed(
      game,
      player,
      carrier,
      source,
    ...
    const speed = this.getTickDistance(
      game,
      carrier,
      canWarpSpeed ? game.constants.distances.warpSpeedMultiplier : 1,
    );
  ...
  getTicksToProduction(
    game: { settings: { galaxy: GameSettingsGalaxyBase } },
    currentTick: number,
  ...
  calculateWaypointTicks(game: Game, carrier: Carrier, waypoint) {
    ...
    const instantSpeed = Boolean(
      sourceStar && this.isStarPairWormHole(sourceStar, destinationStar),
    );
    ...
    let warpSpeed = this.canTravelAtWarpSpeed(
      game,
      carrierOwner,
      carrier,
      sourceStar,
    ...
    let tickDistance = this.getCarrierDistancePerTick(
      game,
      carrier,
      warpSpeed,
      instantSpeed,
  ...
  canTravelAtWarpSpeed(
    ...
    player: Player,
    carrier: Carrier,
    sourceStar: Star | null,
    destinationStar: Star | null,
  ) {
    ...
    if (
      ...
      let sourceAllied =
        sourceStar.ownedByPlayerId === carrier.ownedByPlayerId ||
        (DiplomacyHelper.isFormalAlliancesEnabled(game) &&
          DiplomacyHelper.isDiplomaticStatusToPlayersAllied(
            game,
            sourceStar.ownedByPlayerId,
            [carrier.ownedByPlayerId!],
          ));
      let desinationAllied =
  ...
  getCarrierDistancePerTick(
    ...
    let distanceModifier = warpSpeed
  ...
  canLoop(game, player, carrier) {
    ...
    let distanceBetweenStars = this.getDistanceBetweenLocations(
      firstWaypointStar.location,
      lastWaypointStar.location,
    );
  ...
  isDarkMode(game) {
    return (
      game.settings.specialGalaxy.darkGalaxy === "standard" ||
      game.settings.specialGalaxy.darkGalaxy === "extra" ||
      (game.settings.specialGalaxy.darkGalaxy === "start" &&
        game.state.startDate == null)
    );
  }
  ...
  isTradeEnabled(game) {
    return (
      game.settings.player.tradeCredits ||
      game.settings.player.tradeCreditsSpecialists ||
      game.settings.player.tradeCost
    );
  }
  ...
  playerHasLowestTechLevel(
    game: Game,
    techKey: ResearchTypeNotRandom,
    player: Player,
  ) {
    const levels: number[] = [
      ...new Set(
  ...
  getSortedLeaderboardTeamList(game: Game): TeamLeaderboardData[] {
    ...
    const teamsWithData = game.galaxy.teams!.map((team) => {
      const players = team.players.map((playerId) =>
        this.getPlayerById(game, playerId)!,
      );
  ...
  isSpecialistsCurrencyCredits(game: Game) {
    return (
      this.isSpecialistsEnabled(game) &&
  ...
  isSpecialistsCurrencyCreditsSpecialists(game: Game) {
    return (
      this.isSpecialistsEnabled(game) &&
      game.settings.specialGalaxy.specialistsCurrency === "creditsSpecialists"
    );
  }
  ...
  isAllUndefeatedPlayersReadyToQuit(game) {
    ...
    return (
      undefeatedPlayers.filter((x) => x.readyToQuit).length ===
      undefeatedPlayers.length
    );
  }
  ...
  canTick(game: Game) {
    ...
    if (this.isRealTimeGame(game)) {
      ...
      nextTick = DateTime.fromJSDate(lastTick!)
        .plus({ seconds: game.settings.gameTime.speed })
        .toJSDate();
  ...
  starInfrastructureUpgraded(
    game: Game,
    data: InfrastructureUpgradeReport<string> & {
      type: keyof Star["infrastructure"];
    },
  ) {
    const userPlayer = this.getUserPlayer(game)!;
    ...
    if (data.manufacturing) {
      let manufacturingDifference =
        +data.manufacturing - (star.manufacturing || 0);

      star.manufacturing = +data.manufacturing.toFixed(2);

      userPlayer.stats!.newShips = +(
        userPlayer.stats!.newShips + manufacturingDifference
      ).toFixed(2);
    }
    ...
    switch (data.type) {
      ...
      case "science":
        userPlayer.stats!.totalScience +=
          game.constants.research.sciencePointMultiplier *
  ...
  isInGuild(guild, userId) {
    return (
      guild.members.find((m) => m._id === userId) ||
  ...
  _getBankingCredits(game: Game, player: Player) {
    const bankingEnabled =
      game.settings.technology.startingTechnologyLevel["banking"] > 0;

    if (
      !bankingEnabled ||
      !player.stats!.totalStars ||
      !player.research ||
      !player.research.banking
    ) {
      return 0;
    }
    ...
    switch (game.settings.technology.bankingReward) {
      case "standard":
        return Math.round(
          bankingLevel * multiplier +
            0.15 * bankingLevel * player.stats!.totalEconomy,
        );
      case "legacy":
  ...
  calculateTickIncome(game: Game, player: Player) {
    ...
    return (
      stars.reduce(
        (totalScience, star) =>
          totalScience + (star.infrastructure?.science ?? 0),
        0,
      ) * creditsPerTickByScience
    );
  }
  ...
  isNewPlayerGame(game) {
    return ["new_player_rt", "new_player_tb"].includes(
      game.settings.general.type,
    );
  }
  ...
  getLedgerGameEventPlayerSummary(
    game: Game,
    gameEvent: { data: PlayerDebtEventData<string> },
  ) {
  ...
  calculateCombatEventShipCount(star, carriers, key) {
    ...
    if (scrambled.length) {
      //If any carriers are scramled, add a *
      result += "*";
    }
  ...
  calculateTicksToBonusShip(
    shipsActual: number | undefined,
    manufacturing: number | undefined,
  ) {
    ...
    const partialManufacturing: number =
      manufacturing - Math.floor(manufacturing);
