<template>
  <view-container :is-auth-page="true">
      ...
      <guild-member-list :guild="guild">
        <template v-slot:default="{ value, getColumnClass }">
          <guild-member
            ...
            @onUserApplicationRejected="onUpdate"
          ></guild-member>
        </template>
      </guild-member-list>

      <guild-new-invite
        v-if="isLeader || isOfficer"
        :guildId="guild._id"
    ...
    <div v-if="!isLoading && !guild" class="mb-4">
      <p>
        You are not a member of a guild. Accept an invitation, apply to join or
        found a new guild.
      </p>
      ...
      <p v-if="!invites.length" class="text-warning">
        You have no guild invitations.
      </p>
      ...
      <p v-if="!applications.length" class="text-warning">
        You have not applied to become a member of a guild.
      </p>
...
import type {
  Guild,
  GuildWithUsers,
  GuildApplication as GuildApplicationData,
} from "@solaris/common";
import {
  detailMyGuild,
  listMyGuildApplications,
  listMyGuildInvites,
...
const isLeader = computed(() => {
  return (
    guild.value!.leader != null && guild.value!.leader._id === userStore.userId
  );
});
