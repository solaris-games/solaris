<template>
  <sortable-leaderboard
    ...
    @sortingRequested="sortMemberList"
  >
    <template v-slot:header="actions">
      ...
      <th
        ...
        title="Role"
        @click="actions.sort('role')"
        :class="actions.getColumnClass('role')"
      >
        Role
...
export type GuildRole =
  "leader" | "officer" | "member" | "invitee" | "applicant";
...
const isInGuild = computed(() =>
  GameHelper.isInGuild(props.guild, userStore.userId),
);
...
const getComparer = (key: string) => {
  if (key === "role") {
    return (u1: GuildUser<string>, u2: GuildUser<string>) =>
      roleToValue(u2.role) - roleToValue(u1.role);
  } else if (key === "rank") {
    return (u1: GuildUser<string>, u2: GuildUser<string>) =>
      u2.achievements.rank - u1.achievements.rank;
  } else if (key === "victories") {
    return (u1: GuildUser<string>, u2: GuildUser<string>) =>
      u2.achievements.victories - u1.achievements.victories;
  } else if (key === "renown") {
    return (u1: GuildUser<string>, u2: GuildUser<string>) =>
      u2.achievements.renown - u1.achievements.renown;
  } else {
