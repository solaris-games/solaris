<template>
  <tr>
    <td>
      <router-link
        :to="{ name: 'guild-details', params: { guildId: invite._id } }"
      >
...
const accept = async () => {
  if (
    !(await confirm(
      "Accept invitation",
      `Are you sure you want to accept the invitation from ${props.invite.name}[${props.invite.tag}]?`,
    ))
  ) {
    return;
  }
...
const reject = async () => {
  if (
    !(await confirm(
      "Decline invitation",
      `Are you sure you want to decline the invitation from ${props.invite.name}[${props.invite.tag}]?`,
    ))
  ) {
    return;
  }
