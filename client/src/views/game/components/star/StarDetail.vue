<template>
  <div class="menu-page container" v-if="star">
    <menu-title :title="title" @onCloseRequested="onCloseRequested">
      ...
      <modalButton modalName="abandonStarModal"
                   v-if="!isHistoricalMode && isOwnedByUserPlayer && !userPlayer?.defeated && isGameInProgress && isGameAllowAbandonStars"
    ...
    <div class="row bg-dark">
      <div class="col text-center pt-2">
        ...
        <p class="mb-2"
           v-if="star.ownedByPlayerId != null && (!userPlayer || star.ownedByPlayerId != userPlayer._id) && starOwningPlayer">
          This star is controlled by <a href="javascript:;"
                                        @click="onOpenPlayerDetailRequested">{{ starOwningPlayer.alias }}</a>.</p>
        <p class="mb-2" v-if="star.ownedByPlayerId == null">This star has not been claimed by any faction.<br/>Send a
          carrier here to claim it for yourself!</p>
        <div v-if="star.ownedByPlayerId != null && star.homeStar ">
          <p class="mb-2 text-info"
             v-if="userPlayer && isOwnedByUserPlayer && star.ownedByPlayerId == originalCapitalOwner._id"><small>This is
            your capital.</small></p>
          <p class="mb-2 text-info" v-if="!isOwnedByUserPlayer && star.ownedByPlayerId == originalCapitalOwner._id">
            <small>This is their capital.</small></p>
          <p class="mb-2 text-info" v-if="star.ownedByPlayerId != originalCapitalOwner._id"><small>The former capital of
            <a href="javascript:;" @click="onOpenOriginalCapitalOwner">{{ originalCapitalOwner.alias }}</a>.</small></p>
        </div>
        ...
        <div v-if="(!isCompactUIStyle || !star.ownedByPlayerId) && star.wormHoleToStarId">
          <hr/>
          <p class="mb-0" v-if="wormHolePairStar">This star is a <span class="text-warning">Worm Hole <star-icon
            :isWormHole="true"></star-icon></span> to <a href="javascript:;" @click="viewOnMap(wormHolePairStar)"><i
            class="fas fa-eye me-1"></i>{{ wormHolePairStar.name }}</a>.
          </p>
          <p class="mb-0" v-if="!wormHolePairStar">This star is a <span class="text-warning">Worm Hole <star-icon
            :isWormHole="true"></star-icon></span> to an unknown star.
          </p>
        ...
        <div v-if="(!isCompactUIStyle || !star.ownedByPlayerId) && star.warpGate">
          <hr/>
          <p class="mb-0">This star has a <span class="text-warning">Warp Gate <i class="fas fa-dungeon"></i></span>.
          </p>
          <p class="mb-2 text-info"><small><i>Carriers travel {{ warpSpeedMultiplier }}x faster between active warp
            gates.</i></small></p>
        </div>
        <div v-if="(!isCompactUIStyle || !star.ownedByPlayerId) && star.isBlackHole">
          <hr/>
          <p class="mb-0" v-if="star.isBlackHole">This star is a <span class="text-warning">Black Hole <star-icon
            :isBlackHole="true"></star-icon></span>.
          </p>
          <p class="mb-2 text-info" v-if="star.isBlackHole"><small><i>Black Holes have +3 Scanning Range but have
            reduced natural resources.</i></small></p>
        </div>
        <div v-if="(!isCompactUIStyle || !star.ownedByPlayerId) && star.isPulsar">
          <hr/>
          <p class="mb-0" v-if="star.isPulsar">This star is a <span class="text-warning">Pulsar <star-icon
            :isPulsar="true"></star-icon></span>.
          </p>
          <p class="mb-2 text-info" v-if="star.isPulsar"><small><i>Pulsars are always visible to all players in the
            game.</i></small></p>
        </div>
      </div>
    </div>
    <div class="pb-2" v-if="isCompactUIStyle && star.isInScanningRange">
      <div class="compact-panel">
        <table class="compact-element star-resources-table">
          <tbody>
          <tr>
            <star-resources :resources="star.naturalResources" :compareResources="star.terraformedResources"
                            :iconAlignLeft="true"/>
          </tr>
          </tbody>
        </table>
        ...
        <span v-if="star.isNebula"
          ...
          <span v-if="star.isAsteroidField"
          ...
          <span v-if="star.wormHoleToStarId"
          ...
          <span :title="star.warpGate ? 'Warp Gate - Carriers travel faster between active warp gates':'No Warp Gate'"
        ...
        <div class="compact-element">
          <span v-if="star.ownedByPlayerId && !isDeadStar && ticksToBonusShip"
      ...
      <div class="row" v-if="canShowSpecialist || star.ownedByPlayerId">
          <span v-if="canShowSpecialist && isOwnedByUserPlayer && canHireSpecialist">
            <specialist-icon :type="'star'" :defaultIcon="'user-astronaut'"
                             :specialist="star.specialist"></specialist-icon>
            <a href="javascript:;" @click="onViewHireStarSpecialistRequested">
              <span class="ms-1" v-if="star.specialistId && star.specialist"
                    :title="star.specialist.description">{{ star.specialist.name }}</span>
              <span v-if="!star.specialistId">No Specialist</span>
            </a>
            <span v-if="star.specialistId && star.specialistExpireTick" class="badge bg-warning ms-1"><i
              class="fas fa-stopwatch"></i> Expires Tick {{ star.specialistExpireTick }}</span>
          </span>
        <span v-if="canShowSpecialist && (!isOwnedByUserPlayer || !canHireSpecialist)">
            <specialist-icon :type="'star'" :defaultIcon="'user-astronaut'"
                             :specialist="star.specialist"></specialist-icon>
    ...
    <div v-if="getCarriersInOrbit().length">
      <div class="row pt-2">
        ...
        <div class="col-auto">
          <button title="Distribute ships evenly to carriers" v-if="isOwnedByUserPlayer" type="button"
                  class="btn btn-sm btn-outline-secondary" @click="distributeShips(star)">
            <i class="fas fa-arrow-down-wide-short"></i>
            Distribute
          </button>
          <button title="Transfer all ships to the star" v-if="isOwnedByUserPlayer" type="button"
                  class="btn btn-sm btn-outline-primary ms-1" @click="transferAllToStar(star)">
      ...
      <div v-for="carrier in getCarriersInOrbit()" :key="carrier._id" class="row mb-1 pt-1 pb-0 bg-dark">
        ...
        <div class="col-auto ps-0">
          <a href="javascript:;" v-if="!isHistoricalMode && isOwnedByUserPlayer && !isGameFinished"
             title="Transfer ships between the star and carrier" @click="onShipTransferRequested(carrier._id)"><i
    ...
    <div v-if="isStandardUIStyle && !isDeadStar">
      ...
      <div v-if="isOwnedByUserPlayer && userPlayer && !userPlayer.defeated && star.upgradeCosts != null">
        <div class="row bg-dark pt-2 pb-0 mb-1">
          ...
          <div class="col-4">
            <div class="d-grid gap-2">
              <button
                :disabled="isHistoricalMode || userPlayer.credits < star.upgradeCosts?.carriers! || (star.ships || 0) < 1 || isGameFinished"
        ...
        <div class="row bg-dark pt-2 pb-0 mb-1"
             v-if="(canBuildWarpGates && !star.warpGate) || (canDestroyWarpGates && star.warpGate)">
          <div class="col-8">
        ...
        <div class="row bg-dark pt-2 pb-0 mb-1" v-if="isGameInProgress && isGameAllowAbandonStars">
          ...
          <div class="col-4">
            <div class="d-grid gap-2">
              <modalButton modalName="abandonStarModal" classText="btn btn-outline-danger mb-2"
      ...
      <star-specialist v-if="canShowSpecialist" :starId="star._id"
                       @onViewHireStarSpecialistRequested="onViewHireStarSpecialistRequested"/>
    </div>

    <!-- Modals -->

    <dialogModal modalName="abandonStarModal" titleText="Abandon Star" cancelText="No" confirmText="Yes"
