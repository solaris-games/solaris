  ...
  <div class="position-static btn-group">
    ...
    >
      <i
        ...
        style="width: 13px"
        :class="{
          'fa-sync': action.active && action.repeat,
          'fa-arrow-right': action.active && !action.repeat,
          'fa-pause': !action.active,
        }"
      ></i>
    </button>
  </div>
...
const toggleRepeat = async () => {
  const response = await toggleScheduledBulk(httpClient)(
    store.game!._id,
    props.action._id,
  );
  ...
  if (isOk(response)) {
    ...
    if (props.action.repeat) {
      ...
      toast.default(
        `Your Bulk Upgrade will only be executed on tick ${props.action.tick}.`,
      );
    }
  } else {
