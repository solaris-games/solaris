<template>
  <div class="row bg-dark pt-2 pb-0 mb-1" v-if="star">
    ...
    <div
      v-if="!isHistoricalMode && canHireSpecialist && !isGameFinished"
      class="col-auto"
    >
      <button
        class="btn btn-sm btn-success"
        @click="onViewHireStarSpecialistRequested"
      >
        <i class="fas fa-user-astronaut"></i> Hire Specialist
      </button>
    </div>
    <div class="col-12 mt-2">
      <p v-if="star.specialist">{{ star.specialist.description }}</p>
      <p v-if="star.specialist && star.specialist.oneShot" class="text-warning">
        <small>This specialist cannot be replaced.</small>
      </p>
      <p
        v-if="star.specialist && star.specialistExpireTick"
        class="text-warning"
      >
        <small
          >This specialist expires on tick
          {{ star.specialistExpireTick }}.</small
        >
      </p>
...
const onViewHireStarSpecialistRequested = () =>
  emit("onViewHireStarSpecialistRequested", props.starId);
