<template>
  <div v-if="model" class="p-1 cg-weapons">
    ...
    <div class="cg-weapons-inputs">
      ...
      <input v-if="model.kind === 'level'" type="number" min="1" class="form-control form-control-sm" v-model.number="model.level" />
      <select v-if="model.kind === 'players'" class="form-control form-control-sm" v-model="model.players" multiple>
        <option v-for="player in players" :key="player._id" :value="player">{{ player.alias }}</option>
      </select>
    </div>
    <p v-if="model.kind === 'players' && model.players?.length" class="mb-1">Effective level: {{effectiveLevel}}</p>
  </div>
</template>
...
const effectiveLevel = computed(() => {
  if (model.value?.kind === 'players') {
    return serviceProvider.technologyService.getBaseWeapons(model.value.players || []).toString();
  }
