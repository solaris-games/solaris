<template>
  <div>
    <div class="row">
      ...
      <form class="col-12" @submit="calculate">
        <div class="mb-2 row">
          <label for="defenderWeaponTech" class="col-8 col-form-label">
            <i
              class="fas"
              :class="{
                'fa-gun': !defender.player,
                'fa-user': defender.player,
              }"
            ></i>
            <span class="text-success ms-2">{{
              defender.player ? defender.player.alias : "Defender"
            }}</span>
            Weapons
          </label>
          <div class="col-4">
        ...
        <div class="mb-2 row" v-if="isCarrierToStar && hasDefenderBonus">
          ...
          <label
            class="col-4 col-form-label text-end text-success pt-0 pb-0"
            v-if="includeDefenderBonus"
            >+1 Weapons</label
          >
        </div>
        ...
        <div class="mb-2 row">
          <label for="attackerWeaponTech" class="col-8 col-form-label">
            <i
              class="fas"
              :class="{
                'fa-gun': !attacker.player,
                'fa-user': attacker.player,
              }"
            ></i>
            <span class="text-danger ms-2">{{
              attacker.player ? attacker.player.alias : "Attacker"
            }}</span>
            Weapons
          </label>
          <div class="col-4">
        ...
        <div class="mb-2 row">
          <label for="attackerShips" class="col-8 col-form-label">
            ...
            <span class="text-danger ms-2">{{
              attacker.carrier ? attacker.carrier.name : "Attacker"
            }}</span>
            Ships
          </label>
          <div class="col-4">
        ...
        <div class="mb-2 row">
          <div class="col-auto col-sm-8">
            <button
              ...
              title="Swap the defender/attacker values"
              @click="swapValues"
            >
          ...
          <div class="col col-sm-4">
            <div class="d-grid gap-2">
              <button
                ...
                :disabled="isInvalid"
                title="Calculate the combat result"
              >
    ...
    <div class="row bg-dark pt-2 pb-2" v-if="result">
      <p
        class="col text-center mb-0"
        v-if="result.defender.shipsAfter >= result.attacker.shipsAfter"
      >
        <span class="text-success">Defender</span> wins with
        <span class="text-success">{{ result.defender.shipsAfter }}</span>
        ship(s) remaining.
      </p>
      <p
        class="col text-center mb-0"
        v-if="result.attacker.shipsAfter > result.defender.shipsAfter"
      >
        <span class="text-danger">Attacker</span> wins with
        <span class="text-danger">{{ result.attacker.shipsAfter }}</span>
        ship(s) remaining.
      </p>
    </div>
    <p class="col text-center mb-0" v-if="result?.defender.shipsNeeded">
      <small
        ><span class="text-success">Defender</span> would need
        <span class="text-success">{{ result.defender.shipsNeeded }}</span>
        ship(s) to win.</small
      >
    </p>
...
import { useGameServices } from "@/util/gameServices.js";
...
const hasDefenderBonus = computed(
  () => game.value.settings.specialGalaxy.defenderBonus === "enabled",
);
...
const tryAutoCalculate = () => {
  if (props.carrierId) {
    ...
    const attackerCarrier = GameHelper.getCarrierById(
      game.value,
      props.carrierId,
    );
    const attackerPlayer =
      attackerCarrier &&
      GameHelper.getCarrierOwningPlayer(game.value, attackerCarrier);
    ...
    if (
      attacker.value.carrier?.waypoints &&
      attacker.value.carrier?.waypoints.length
    ) {
      const defenderStar = GameHelper.getStarById(
        game.value,
        attacker.value.carrier.waypoints[0].destination,
      );
      ...
      if (defenderStar) {
        // May be out of scanning range.
        const defenderPlayer = GameHelper.getStarOwningPlayer(
          game.value,
          defenderStar,
        );
        const defenderShips = GameHelper.getStarTotalKnownShips(
          game.value,
          defenderStar,
        );
        ...
        defender.value.weaponsLevel =
          defender.value.player.research.weapons.level;
      }
    }
  }
};
...
const calculate = async (e: Event) => {
  ...
  result.value = serviceProvider.combatService.calculateBasic(
    defender.value,
    attacker.value,
    isCarrierToStar.value,
    includeDefenderBonus.value,
  );
};
