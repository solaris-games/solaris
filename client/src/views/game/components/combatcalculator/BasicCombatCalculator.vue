<template>
  <div>
    <div class="row">
      ...
      <form class="col-12" @submit="calculate">
        <div class="mb-2 row">
          ...
          <div class="col-4">
            <input type="number" class="form-control" id="defenderWeaponTech" placeholder="Tech Level"
                   v-model="defender.weaponsLevel" :required="true">
          </div>
        </div>
        <div class="mb-2 row">
          ...
          <div class="col-4">
            <input type="number" class="form-control" id="defenderShips" placeholder="Ships" v-model="defender.ships"
                   :required="true">
          </div>
        </div>
        <div class="mb-2 row">
        ...
        <div class="mb-2 row" v-if="hasDefenderBonus">
          ...
          <label class="col-4 col-form-label text-end text-success pt-0 pb-0" v-if="includeDefenderBonus">+1
            Weapons</label>
        </div>
        ...
        <div class="mb-2 row">
          ...
          <div class="col-4">
            <input type="number" class="form-control" id="attackerWeaponTech" placeholder="Tech Level"
                   v-model="attacker.weaponsLevel" :required="true">
          </div>
        </div>
        <div class="mb-2 row">
          ...
          <div class="col-4">
            <input type="number" class="form-control" id="attackerShips" placeholder="Ships" v-model="attacker.ships"
                   :required="true">
          </div>
        </div>
        ...
        <div class="mb-2 row">
          <div class="col-auto col-sm-8">
            <button type="button" class="btn btn-outline-info" title="Swap the defender/attacker values"
                    @click="swapValues"><i class="fas fa-exchange-alt"></i> Swap Values
            </button>
          </div>
          <div class="col col-sm-4">
            <div class="d-grid gap-2">
              <button type="submit" class="btn btn-success" :disabled="isInvalid" title="Calculate the combat result"><i
                class="fas fa-fist-raised"></i> Fight
              </button>
            </div>
          </div>
        </div>
    ...
    <div class="row bg-dark pt-2 pb-2" v-if="result">
      <p class="col text-center mb-0" v-if="result.defender.shipsAfter >= result.attacker.shipsAfter"><span
        class="text-success">Defender</span> wins with <span
        class="text-success">{{ result.defender.shipsAfter }}</span> ship(s) remaining.</p>
      <p class="col text-center mb-0" v-if="result.attacker.shipsAfter > result.defender.shipsAfter"><span
        class="text-danger">Attacker</span> wins with <span class="text-danger">{{ result.attacker.shipsAfter }}</span>
        ship(s) remaining.</p>
    </div>
    <p class="col text-center mb-0" v-if="result?.defender.shipsNeeded"><small><span
