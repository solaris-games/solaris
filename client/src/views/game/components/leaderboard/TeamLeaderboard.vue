    ...
    <div class="team-row" v-for="team in sortedTeams" :key="team.team.name">
      <div class="team-info">
        ...
        <span v-if="isStarCountWinCondition" class="team-score">
          ...
          <span class="d-none d-sm-block">
            {{ team.totalStars }} Star<span v-if="team.totalStars !== 1"
              >s</span
            >
          </span>
        </span>
...
const sortedTeams = computed(() =>
  GameHelper.getSortedLeaderboardTeamList(game.value),
);
const isHomeStarsWinCondition = computed(() =>
  GameHelper.isWinConditionHomeStars(game.value),
);
const isStarCountWinCondition = computed(() =>
  GameHelper.isWinConditionStarCount(game.value),
);
...
const isPlayerTeam = (team: TeamLeaderboardData) => {
  ...
  return (
    userPlayer &&
    team.players.some((p) => p._id.toString() === userPlayer._id.toString())
  );
};
</script>
