<template>
  <div class="row bg-dark pt-2 pb-0 mb-1" v-if="carrier">
    ...
    <div class="col-12 mt-2">
      ...
      <p
        v-if="carrier.specialist && carrier.specialistExpireTick"
        class="text-warning"
      >
        <small
          >This specialist expires on tick
          {{ carrier.specialistExpireTick }}.</small
        >
      </p>
...
const onViewHireCarrierSpecialistRequested = () =>
  emit("onViewHireCarrierSpecialistRequested", props.carrierId);
...
onMounted(() => {
  ...
  if (carrier.value.orbiting) {
    ...
    canHireSpecialist.value = Boolean(
      userPlayer &&
    ...
    ); // Carrier doesn't already have a spec on it
  }
});
</script>
