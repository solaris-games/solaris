<template>
  <tr>
    <td v-if="userPlayerOwnsCarrier">
      <span v-if="!(isFirstWaypoint(waypoint) && isInTransit)">{{
        waypoint.delayTicks
      }}</span>
    </td>
    <td>
      <a href="javascript:;" @click="onOpenStarDetailRequested">{{
    ...
    <td
      v-if="
        !showAction &&
        props.waypoint.ticksEta !== null &&
        props.waypoint.ticksEta !== undefined
      "
    >
...
const userPlayerOwnsCarrier = computed(
  () =>
    userPlayer.value &&
    GameHelper.getCarrierOwningPlayer(game.value, props.carrier)!._id ===
      userPlayer.value._id,
);
const canEditWaypoints = computed(
  () =>
    !GameHelper.isGameFinished(game.value) &&
    userPlayerOwnsCarrier.value &&
    !props.carrier.isGift,
);
const isInTransit = computed(() => !props.carrier.orbiting);

const getStarName = (starId: string) =>
  GameHelper.getStarById(game.value, starId)!.name;

const onOpenStarDetailRequested = () =>
  emit("onOpenStarDetailRequested", props.waypoint.destination);
