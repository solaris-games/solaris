  ...
  <div class="menu-page container" v-if="star">
    ...
    <div class="row bg-dark mb-2">
      <div class="col text-center pt-3">
        <p>
          Build a new Carrier at
          <a href="javascript:;" @click="onOpenStarDetailRequested">{{
            star.name
          }}</a
          >, decide how many ships the new Carrier will have.
        </p>
      </div>
    </div>
...
const saveTransfer = async () => {
  if (
    store.settings!.carrier.confirmBuildCarrier === "enabled" &&
    !(await confirm(
      "Build a carrier",
      `Are you sure you want to build a Carrier at ${star.value.name}? The carrier will cost $${star.value.upgradeCosts!.carriers}.`,
    ))
  ) {
    return;
  }
  ...
  const response = await buildCarrier(httpClient)(
    store.game!._id,
    star.value._id,
    ships,
  );
