<template>
  <div v-if="isTradeAllowed && player">
    ...
    <div
      v-if="
        isTradePossibleByScanning && isTradePossibleByDiplomacy && userPlayer
      "
    >
      <sendCredits
        v-if="tradeCreditsIsEnabled"
        :player="player"
        :userPlayer="userPlayer"
      />
      <sendCreditsSpecialists
        v-if="tradeCreditsSpecialistsIsEnabled"
        :player="player"
        :userPlayer="userPlayer"
      />
      <sendTechnology
        v-if="player && tradeTechnologyIsEnabled"
        :playerId="player._id"
      />
    </div>

    <p v-if="!isTradePossibleByScanning" class="text-danger pt-2 pb-0 mb-0">
      You cannot trade with this player, they are not within scanning range.
    </p>
    <p v-if="!isTradePossibleByDiplomacy" class="text-danger pt-2 pb-0 mb-0">
      You cannot trade with this player, they are not an ally.
    </p>
  </div>
</template>
...
const tradeCreditsIsEnabled = computed(
  () => game.value.settings.player.tradeCredits,
);
...
const isTradePossibleByScanning = computed(
  () =>
    player.value &&
    ((player.value.stats?.totalStars || 0) > 0 ||
      GameHelper.isDarkModeExtra(game.value)) &&
    (game.value.settings.player.tradeScanning === "all" ||
      player.value.isInScanningRange),
);
...
const tradeTechnologyIsEnabled = computed(
  () => game.value.settings.player.tradeCost > 0,
);
...
const isTradeAllowed = computed(
  () =>
    ...
    (tradeTechnologyIsEnabled.value ||
      tradeCreditsIsEnabled.value ||
      tradeCreditsSpecialistsIsEnabled.value),
);
