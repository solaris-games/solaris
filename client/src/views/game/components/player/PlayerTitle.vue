<template>
  <div v-if="player">
    <div
      ...
      :style="{
        'background-image':
          'linear-gradient(to left, ' + colour + ', #375a7f 100%)',
      }"
    >
      <div class="col">
        <h4 class="pt-2">
      ...
      <div class="col-auto">
        <h4 class="pt-2">
          <player-diplomatic-status-icon
            v-if="isFormalAlliancesEnabled"
            :toPlayerId="player._id"
            class="ms-2"
          />
          <i
            v-if="player.hasFilledAfkSlot && !player.afk"
            class="fas fa-user-friends ms-2"
            title="This player has filled an AFK slot and will be awarded 1.5x rank points and lose no rank (minimum 0)"
          ></i>
          <i
            class="fas fa-robot ms-2"
            v-if="player.isAIControlled"
            title="This player is AI Controlled"
          ></i>
          <span v-if="player.defeated" :title="getPlayerStatus()">
            <i
              v-if="!player.afk"
              class="fas fa-skull-crossbones ms-2"
              title="This player has been defeated"
            ></i>
            <i
              v-if="player.afk"
              class="fas fa-user-clock ms-2"
              title="This player is AFK"
            ></i>
          </span>
          <span
            class="ms-2"
            v-if="player.hasDuplicateIP"
            title="Warning: This player shares the same IP address as another player in this game."
          >
...
const colour = computed(() =>
  GameHelper.getFriendlyColour(
    colourStore.getColourForPlayer(game.value, props.player._id)!.value,
  ),
);
</script>
