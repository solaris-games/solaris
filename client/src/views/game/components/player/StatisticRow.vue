<template>
  <tr>
    ...
    <td
      class="text-end"
      :title="isPlayerStatUncertain ? scanningRangeTooltip : undefined"
    >
      {{ formatValue(player, playerStat)
    ...
    <td
      ...
      v-if="
        ...
        !isUserPlayer &&
        userPlayerStat !== null &&
        userPlayerStat !== undefined &&
        userPlayer
      "
      :class="{
        'text-danger': playerStat > userPlayerStat,
        'text-success': playerStat < userPlayerStat,
      }"
    >
      {{ formatValue(userPlayer, userPlayerStat) }}
    </td>
  </tr>
</template>
...
const isUserPlayer = computed(
  () => userPlayer.value && props.playerId === userPlayer.value._id,
);
