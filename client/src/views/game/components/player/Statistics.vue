  ...
  <div class="table-responsive mb-0" v-if="player">
    <table class="table table-sm mb-1">
      ...
      <tbody>
        ...
        <statistic-row
          v-if="playerTickIncome > 0 || userPlayerTickIncome > 0"
          :playerId="playerId"
          header="Tick Income"
    ...
    <p
      class="text-warning text-center mb-2"
      v-if="isDarkModeExtra && userIsInGame && isUserPlayer"
    >
      <small>Based on your scanning range.</small>
    </p>
  </div>
</template>
...
const isUserPlayer = computed(
  () => userPlayer.value && player.value._id === userPlayer.value._id,
);
...
const playerIncome = computed(() =>
  GameHelper.calculateIncome(game.value, player.value),
);
