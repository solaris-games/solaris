<template>
  ...
  <sendRenown
    v-if="userPlayer && player && canSendRenown"
    :player="player"
...
const playersAreAnonymous = computed(() =>
  isGameFinished.value
    ? serviceProvider.gameTypeService.isAnonymousAfterEnd(props.game)
    : serviceProvider.gameTypeService.isAnonymousGameDuringGame(props.game),
);

const canViewAchievements = computed(() =>
  Boolean(props.player && props.player.isRealUser && props.user?.achievements),
);
const playerIsUser = computed(() =>
  Boolean(
    props.player &&
    props.userPlayer &&
    props.userPlayer._id === props.player._id,
  ),
);

const canSendRenown = computed(() =>
  Boolean(
    props.player &&
    isGameStarted.value &&
    props.player.isRealUser &&
    props.userPlayer &&
    !playerIsUser.value &&
    (!playersAreAnonymous.value || isGameFinished.value),
  ),
);
...
const onOpenPurchasePlayerBadgeRequested = () =>
  emit("onOpenPurchasePlayerBadgeRequested", props.player._id);
</script>
