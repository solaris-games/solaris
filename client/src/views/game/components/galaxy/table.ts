...
export const useSortedMapObjectData = <A extends MapObject<string>>(
  tableData: Ref<A[]>,
  sortInfo: Ref<SortInfo>,
  showAll: Ref<boolean>,
  game: Ref<Game>,
  searchFilter: (v: A) => boolean,
) => {
  ...
  const filteredTableData = computed(() => {
    ...
    if (!showAll.value && userPlayer.value !== undefined) {
      td = tableData.value.filter(
        (s) => s.ownedByPlayerId === userPlayer.value!._id && searchFilter(s),
      );
    } else {
...
export const useSortedPlayerData = (
  tableData: Ref<Player[]>,
  sortInfo: Ref<SortInfo>,
  showAll: Ref<boolean>,
  game: Ref<Game>,
  searchFilter: (v: Player) => boolean,
) => {
  const userPlayer = computed(() => GameHelper.getUserPlayer(game.value));
  ...
  const filteredTableData = computed(() => {
    ...
    if (!showAll.value && userPlayer.value !== undefined) {
      td = tableData.value.filter(
        (s) => s._id === userPlayer.value!._id && searchFilter(s),
      );
    } else {
      return tableData.value.filter(searchFilter);
    }
