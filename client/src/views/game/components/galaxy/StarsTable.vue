    ...
    <div class="row mb-2 g-0">
      ...
      <div class="col-auto pt-1" v-if="!isGameFinished && userPlayer != null">
        <input
          class="me-1"
          type="checkbox"
          v-model="allowUpgrades"
          id="chkEnableUpgrades"
        />
        <label for="chkEnableUpgrades"> Upgrades </label>
      </div>
    </div>
...
const allowUpgrades = ref(
  store.settings!.interface.galaxyScreenUpgrades === "enabled" &&
...
const isEconomyEnabled = computed(
  () => game.value.settings.player.developmentCost.economy !== "none",
);
const isIndustryEnabled = computed(
  () => game.value.settings.player.developmentCost.industry !== "none",
);
const isScienceEnabled = computed(
  () => game.value.settings.player.developmentCost.science !== "none",
);
