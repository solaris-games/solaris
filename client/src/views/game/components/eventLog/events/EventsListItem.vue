<template>
  <div
    class="bg-dark pt-2 pb-1 mb-2 pointer row"
    :class="{ unread: !isRead, 'bg-secondary': isGlobal }"
    @click="doMarkAsRead"
  >
    <div class="col">
    ...
    <div v-if="event.tick" class="col-auto">
      <span
        class="badge"
        :class="{ 'bg-success': isRead, 'bg-warning': !isRead }"
    ...
    <div class="col-12 mt-2">
      <game-ended
        :event="event"
        v-if="event.type === 'gameEnded'"
        @onOpenPlayerDetailRequested="onOpenPlayerDetailRequested"
      />
      <game-player-afk
        :event="event"
        v-if="event.type === 'gamePlayerAFK'"
        @onOpenPlayerDetailRequested="onOpenPlayerDetailRequested"
      />
      <game-player-defeated
        :event="event"
        v-if="event.type === 'gamePlayerDefeated'"
        @onOpenPlayerDetailRequested="onOpenPlayerDetailRequested"
      />
      <game-player-joined
        :event="event"
        v-if="event.type === 'gamePlayerJoined'"
        @onOpenPlayerDetailRequested="onOpenPlayerDetailRequested"
      />
      <game-player-quit
        :event="event"
        v-if="event.type === 'gamePlayerQuit'"
        @onOpenPlayerDetailRequested="onOpenPlayerDetailRequested"
      />
      <game-started :event="event" v-if="event.type === 'gameStarted'" />
      <game-player-badge-purchased
        :event="event"
        v-if="event.type === 'gamePlayerBadgePurchased'"
        @onOpenPlayerDetailRequested="onOpenPlayerDetailRequested"
      />
      <game-diplomacy-peace-declared
        :event="event"
        v-if="event.type === 'gameDiplomacyPeaceDeclared'"
      />
      <game-diplomacy-war-declared
        :event="event"
        v-if="event.type === 'gameDiplomacyWarDeclared'"
      />

      <player-bulk-infrastructure-upgraded
        :event="event"
        v-if="event.type === 'playerBulkInfrastructureUpgraded'"
      />
      <player-combat-star
        :event="event"
        v-if="event.type === 'playerCombatStar'"
        @onOpenPlayerDetailRequested="onOpenPlayerDetailRequested"
      />
      <player-combat-carrier
        :event="event"
        v-if="event.type === 'playerCombatCarrier'"
      />
      <player-credits-received
        :event="event"
        v-if="event.type === 'playerCreditsReceived'"
        @onOpenPlayerDetailRequested="onOpenPlayerDetailRequested"
      />
      <player-credits-sent
        :event="event"
        v-if="event.type === 'playerCreditsSent'"
        @onOpenPlayerDetailRequested="onOpenPlayerDetailRequested"
      />
      <player-credits-specialists-received
        :event="event"
        v-if="event.type === 'playerCreditsSpecialistsReceived'"
        @onOpenPlayerDetailRequested="onOpenPlayerDetailRequested"
      />
      <player-credits-specialists-sent
        :event="event"
        v-if="event.type === 'playerCreditsSpecialistsSent'"
        @onOpenPlayerDetailRequested="onOpenPlayerDetailRequested"
      />
      <player-gift-received
        :event="event"
        v-if="event.type === 'playerGiftReceived'"
        @onOpenPlayerDetailRequested="onOpenPlayerDetailRequested"
      />
      <player-gift-sent
        :event="event"
        v-if="event.type === 'playerGiftSent'"
        @onOpenPlayerDetailRequested="onOpenPlayerDetailRequested"
      />
      <player-galactic-cycle-complete
        :event="event"
        v-if="event.type === 'playerGalacticCycleComplete'"
      />
      <player-renown-received
        :event="event"
        v-if="event.type === 'playerRenownReceived'"
        @onOpenPlayerDetailRequested="onOpenPlayerDetailRequested"
      />
      <player-renown-sent
        :event="event"
        v-if="event.type === 'playerRenownSent'"
        @onOpenPlayerDetailRequested="onOpenPlayerDetailRequested"
      />
      <player-research-complete
        :event="event"
        v-if="event.type === 'playerResearchComplete'"
      />
      <player-star-abandoned
        :event="event"
        v-if="event.type === 'playerStarAbandoned'"
      />
      <player-star-died :event="event" v-if="event.type === 'playerStarDied'" />
      <player-star-reignited
        :event="event"
        v-if="event.type === 'playerStarReignited'"
      />
      <player-technology-received
        :event="event"
        v-if="event.type === 'playerTechnologyReceived'"
        @onOpenPlayerDetailRequested="onOpenPlayerDetailRequested"
      />
      <player-technology-sent
        :event="event"
        v-if="event.type === 'playerTechnologySent'"
        @onOpenPlayerDetailRequested="onOpenPlayerDetailRequested"
      />
      <player-debt-forgiven
        :event="event"
        v-if="event.type === 'playerDebtForgiven'"
        @onOpenPlayerDetailRequested="onOpenPlayerDetailRequested"
      />
      <player-debt-settled
        :event="event"
        v-if="event.type === 'playerDebtSettled'"
        @onOpenPlayerDetailRequested="onOpenPlayerDetailRequested"
      />
      <player-star-specialist-hired
        :event="event"
        v-if="event.type === 'playerStarSpecialistHired'"
      />
      <player-carrier-specialist-hired
        :event="event"
        v-if="event.type === 'playerCarrierSpecialistHired'"
      />
      <player-conversation-created
        :event="event"
        v-if="event.type === 'playerConversationCreated'"
      />
      <player-conversation-invited
        :event="event"
        v-if="event.type === 'playerConversationInvited'"
      />
      <player-conversation-left
        :event="event"
        v-if="event.type === 'playerConversationLeft'"
      />
      <player-diplomacy-status-changed
        :event="event"
        v-if="event.type === 'playerDiplomacyStatusChanged'"
        @onOpenPlayerDetailRequested="onOpenPlayerDetailRequested"
      />
    </div>
  </div>
...
const isRead = computed(
  () =>
    props.event.read ||
    props.event.read === null ||
    props.event.read === undefined,
);

const isGlobal = computed(
  () => props.event.read === null || props.event.read === undefined,
);
