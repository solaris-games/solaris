<template>
  <div v-if="event">
    <p>
      ...
      <span class="text-warning">carrier-to-star</span> combat at
      <star-label
        :starId="starGroup.star!.starId"
        :starName="starGroup.star!.starName"
      />.
    </p>

    <combat-event-group
      v-for="(group, groupIndex) of event.data.groups"
      :title="getTitle(group, groupIndex)"
      :group="group"
    ...
    <div v-if="captureResult">
      <p>
        <span v-if="originalOwner">
          <a
            href="javascript:;"
            @click="emit('onOpenPlayerDetailRequested', originalOwner._id)"
            >{{ originalOwner.alias }}</a
          >'s
        </span>
        ...
        <a
          href="javascript:;"
          @click="
            emit('onOpenPlayerDetailRequested', captureResult.capturedById)
          "
          >{{ captureResult.capturedByAlias }}</a
        >.
      </p>
      <p v-if="captureResult?.specialistDestroyed">
        ...
        <star-label
          ...
          :starName="starGroup.star!.starName"
        />
        was destroyed due to the intense combat.
      </p>
      <p v-if="captureResult?.captureReward">
        ...
        >
        is awarded
        <span class="text-warning">${{ captureResult.captureReward }}</span>
        credits for destroying economic infrastructure.
      </p>
    </div>
    <div v-if="!captureResult">
      <p>
        ...
        <a
          ...
          @click="
            () =>
              originalOwnerId &&
              emit('onOpenPlayerDetailRequested', originalOwnerId)
          "
          >{{
            originalOwnerId
              ? gameHelper.getPlayerById(game, originalOwnerId)!.alias
              : "???"
          }}</a
        >.
      </p>
...
const starGroup = computed(() =>
  props.event.data.groups.find((g) => Boolean(g.star))!,
);
...
const originalOwner = computed(() =>
  GameHelper.getPlayerById(game, originalOwnerId.value),
);
