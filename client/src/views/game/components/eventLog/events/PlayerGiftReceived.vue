<template>
  <div v-if="player">
    <p>
      You have received a carrier gift
      <carrier-label
        :carrierId="event.data.carrierId"
        :carrierName="event.data.carrierName"
      />
      of
      <span class="text-warning">{{ event.data.carrierShips }} ships</span> from
...
const onOpenPlayerDetailRequested = () =>
  emit("onOpenPlayerDetailRequested", props.event.data.fromPlayerId);

const store = useGameStore();
const game = computed<Game>(() => store.game!);

const player = computed(() =>
  GameHelper.getPlayerById(game.value, props.event.data.fromPlayerId)!,
);
</script>
