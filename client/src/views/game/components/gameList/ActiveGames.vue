<template>
  <div>
    <div class="row">
      ...
      <div class="col-auto">
        <input
          ...
          type="checkbox"
          v-model="includeDefeated"
          id="chkIncludeDefeated"
        />
    ...
    <div class="table-responsive">
      <table
        v-if="!isLoadingActiveGames && filteredActiveGames.length"
        ...
        <tbody>
          <tr v-for="game in filteredActiveGames" v-bind:key="game._id">
            <td class="col-6">
              ...
              <span
                v-if="
                  game.userNotifications.defeated && !game.userNotifications.afk
                "
              ...
              <span
                v-if="
                  !game.userNotifications.defeated &&
                  game.userNotifications.turnWaiting
                "
                class="me-1 badge bg-danger"
                >Turn Waiting</span
              >
              <span
                v-if="
                  !game.userNotifications.defeated &&
                  game.userNotifications.turnTotalCount != null
                "
                class="me-1 badge bg-secondary"
                >{{ game.userNotifications.turnReadyCount }}/{{
                  game.userNotifications.turnTotalCount
                }}
                Ready</span
              >
              <span
                v-if="
                  !game.userNotifications.defeated &&
                  game.userNotifications.unreadEvents
                "
                class="me-1 badge bg-warning"
                >{{ game.userNotifications.unreadEvents }} Events</span
              >
              <span
                v-if="game.userNotifications.afk"
                class="me-1 badge bg-warning"
                >AFK</span
              >
              <span
                v-if="game.userNotifications.unreadConversations"
                class="me-1 badge bg-info"
                >{{ game.userNotifications.unreadConversations }} Messages</span
              >
              ...
              <div class="d-md-none text-info">
                <small>
                  ...
                  <span v-if="isGamePendingStart(game)"> Starting Soon </span>
            ...
            <td class="col-1 col-md-6 text-center">
    ...
    <div class="text-end" v-if="!isLoadingActiveGames">
      ...
      >
      <router-link to="/game/list" tag="button" class="btn btn-success"
        >Join New Game <i class="fas fa-arrow-right"></i
      ></router-link>
    </div>
  </div>
</template>
...
const includeDefeated = ref(
  loadLocalPreference(INCLUDE_DEFEATED_PREF_KEY, true),
);

watch(includeDefeated, (newValue) =>
  storeLocalPreference(INCLUDE_DEFEATED_PREF_KEY, newValue),
);
...
const isGameWaitingForPlayers = (game: UserActiveListGame<string>) =>
  GameHelper.isGameWaitingForPlayers(game);

const isGamePendingStart = (game: UserActiveListGame<string>) =>
  GameHelper.isGamePendingStart(game);

const isGameInProgress = (game: UserActiveListGame<string>) =>
  GameHelper.isGameInProgress(game);
...
const getGameTypeFriendlyText = (game: UserActiveListGame<string>) =>
  GameHelper.getGameTypeFriendlyText(game);
...
const loadActiveGames = async () => {
  ...
  if (isOk(response)) {
    activeGames.value = response.data.sort(
      (a, b) =>
        Number(a.userNotifications.defeated) -
        Number(a.userNotifications.afk) -
        (Number(b.userNotifications.defeated) -
          Number(b.userNotifications.afk)),
    );
  } else {
