<template>
  <div class="menu-page container pb-2">
    ...
    <component
      v-bind:is="currentTutorialComponent"
      v-bind="tutorialProps"
    ></component>
  </div>
</template>
...
const tutorialKey = ref(
  game.value.settings.general.createdFromTemplate || defaultTutorialKey,
);
const page = ref(0);
const maxPage = ref(0);
...
const currentTutorialComponent = computed(
  () =>
    tutorialComponents[tutorialKey.value] ||
    tutorialComponents[defaultTutorialKey],
);
...
const tutorialProps = computed<TutorialProps>(() => {
  ...
  return {
    ...
    onOpenStarDetailRequested: (starId) =>
      emit("onOpenStarDetailRequested", starId),
    startingStars: game.value.settings.player.startingStars,
    playerHomeStar: GameHelper.getPlayerHomeStar(
      player,
      game.value.galaxy.stars,
    )!,
    playerStars: game.value.galaxy.stars.filter(
      (st) => st.ownedByPlayerId === player._id,
    ),
