...
const purchaseBadge = async (badge: Badge) => {
  if (
    !(await confirm(
      `Purchase Badge`,
      `Are you sure you want to purchase the '${badge.name}' badge for ${props.recipientName}? It will cost ${badge.price} credit(s).`,
    ))
  ) {
    return;
  }
...
const getBadgeWebpSrc = (badge: Badge) => {
  return new URL(`../../../../assets/badges/${badge.key}.webp`, import.meta.url)
    .href;
};
</script>
