<template>
  <div class="menu-page container">
    ...
    >
      <button
        @click="(_) => emit('onOpenStarDetailRequested', star._id)"
    ...
    <div class="row mb-2 pt-1 pb-1 bg-dark" v-if="star">
      <div class="col">
        <a
          href="javascript:;"
          @click="(_) => emit('onOpenStarDetailRequested', star._id)"
          ><i class="fas fa-star"></i> {{ star.name }}</a
        >
      </div>
      <div class="col-auto">
      ...
      >
        ...
        <div class="col-auto mt-2">
          <button
            ...
            v-if="!(star.specialist && star.specialist.id === specialist.id)"
            :disabled="
              isHistoricalMode ||
              isHiringSpecialist ||
...
const star = computed(() =>
  game.value.galaxy.stars.find((s) => s._id === props.starId)!,
);
const userPlayer = computed(() => GameHelper.getUserPlayer(game.value)!);
const isCurrentSpecialistOneShot = computed(() =>
  Boolean(star.value.specialist?.oneShot),
);

const isHistoricalMode = useIsHistoricalMode(store);

const specialists = computed(() =>
  store.starSpecialists!.filter(
    (s) =>
      game.value.settings.specialGalaxy.specialistBans.star.indexOf(s.id) < 0,
  ),
);
...
const shouldSpecialistRequireReload = (specialist: Specialist) => {
  const localKeys = ["scanning", "manufacturing"];
...
const hireSpecialist = async (specialist: Specialist) => {
  if (
    !(await confirm(
      "Hire specialist",
      `Are you sure you want to hire a ${specialist.name} for ${getSpecialistActualCostString(specialist)} on Star ${star.value.name}?`,
    ))
  ) {
    return;
  }
  ...
  if (
    star.value.specialist &&
    !(await confirm(
      "Replace specialist",
      `Are you sure you want to replace the existing specialist ${star.value.specialist.name} for a ${specialist.name}?`,
    ))
  ) {
    return;
  }
  ...
  const requiresFullReload =
    (star.value.specialist &&
      shouldSpecialistRequireReload(star.value.specialist)) ||
    shouldSpecialistRequireReload(specialist);

  const response = await hireStar(httpClient)(
    game.value._id,
    props.starId,
    specialist.id,
  );
  ...
  if (isOk(response)) {
    if (requiresFullReload) {
      ...
      star.value.specialistExpireTick = specialist.expireTicks
    ...
    toast.default(
      `${specialist.name} has been hired for the star ${star.value.name}.`,
    );
  } else {
    toast.error(
