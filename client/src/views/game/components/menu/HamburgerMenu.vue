  ...
  <div class="dropdown-container" :class="dropType || 'dropleft'">
    ...
    >
      <div class="ps-2">
        ...
        <div>
          ...
          <button
            v-if="isLoggedIn"
            class="btn btn-primary btn-sm me-1 mb-1"
            @click="setMenuState({ state: 'ruler' })"
            title="Ruler (V)"
          >
            <i class="fas fa-ruler"></i>
          </button>
          <button
            v-if="userPlayer && !userPlayer.defeated"
            class="btn btn-primary btn-sm me-1 mb-1"
            @click="setMenuState({ state: 'bulkInfrastructureUpgrade' })"
            title="Bulk Upgrade (B)"
          >
      ...
      <button
        v-if="userPlayer"
        class="dropdown-item"
        :class="isCustomColoursEnabled ? 'active' : null"
      ...
      <div v-if="!userPlayer && gameIsJoinable">
        <a class="dropdown-item" v-on:click="setMenuState({ state: 'welcome' })"
          ><i class="fas fa-handshake me-2"></i>Welcome</a
        >
      </div>
      <div v-if="!userPlayer && !gameIsJoinable">
        <a
          ...
          title="Leaderboard (Q)"
          ><i class="fas fa-users me-2"></i>Leaderboard</a
        >
      </div>
      <a
        class="dropdown-item"
        ...
        title="Galaxy (G)"
        ><i class="fas fa-sun me-2"></i>Galaxy</a
      >
      <div v-if="userPlayer">
        <a
          class="dropdown-item"
        ...
        <a
          class="dropdown-item"
          v-on:click="setMenuState({ state: 'research' })"
          title="Research (R)"
          ><i class="fas fa-flask me-2"></i>Research</a
        >
        <a
          class="dropdown-item d-lg-none"
          v-on:click="store.setMenuStateChat({ state: 'inbox' })"
          title="Inbox (M)"
          v-if="!isTutorialGame"
          ><i class="fas fa-comments me-2"></i>Inbox</a
        >
        <a
          class="dropdown-item d-none d-lg-inline-block"
          v-on:click="onMenuChatSidebarRequested()"
          title="Inbox (M)"
          v-if="!isTutorialGame"
          ><i class="fas fa-comments me-2"></i>Inbox</a
        >
        <a
          class="dropdown-item"
          v-on:click="setMenuState({ state: 'diplomacy' })"
          title="Diplomacy (D)"
          v-if="isFormalAlliancesEnabled"
          ><i class="fas fa-globe-americas me-2"></i>Diplomacy</a
        >
        <a
          class="dropdown-item"
          v-on:click="setMenuState({ state: 'ledger' })"
          title="Ledger (L)"
          v-if="isTradeEnabled"
          ><i class="fas fa-file-invoice-dollar me-2"></i>Ledger</a
        >
        <a
          class="dropdown-item"
          v-on:click="setMenuState({ state: 'statistics' })"
          title="Statistics"
          ><i class="fas fa-chart-bar me-2"></i>Statistics</a
        >
        <a
          class="dropdown-item"
          v-on:click="setMenuState({ state: 'gameNotes' })"
          title="Notes (N)"
          ><i class="fas fa-book-open me-2"></i>Notes</a
        >
        <a
          class="dropdown-item"
          v-on:click="setMenuState({ state: 'spectators' })"
          title="Spectators"
          v-if="isSpectatingEnabled"
          ><i class="fas fa-people-arrows me-2"></i>Spectators</a
        >
      </div>
      <a
        v-if="
        ...
        title="Intel (I)"
        ><i class="fas fa-chart-line me-2"></i>Intel</a
      >
      <a
        v-if="isLoggedIn"
        class="dropdown-item"
        v-on:click="setMenuState({ state: 'options' })"
        title="Options (O)"
        ><i class="fas fa-cog me-2"></i>Options</a
      >
      <a :href="documentationUrl" class="dropdown-item" target="_blank"
        ><i class="far fa-question-circle me-2"></i>How to Play</a
      >
      <router-link
        v-if="isLoggedIn"
        to="/game/active-games"
        class="dropdown-item"
        ><i class="fas fa-dice me-2"></i>My Games
      </router-link>
      <a v-if="isLoggedIn" class="dropdown-item" v-on:click="goToMainMenu()"
        ><i class="fas fa-chevron-left me-2"></i>Main Menu</a
      >
      <router-link v-if="!isLoggedIn" to="/" class="dropdown-item"
        ><i class="fas fa-sign-in-alt me-2"></i>Log In
      </router-link>
      <router-link v-if="!isLoggedIn" to="/account/create" class="dropdown-item"
...
const toggleCustomColours = () => {
  colourStore.setColourOverride(
    !isCustomColoursEnabled.value,
    eventBus,
    store.game,
    store.settings,
  );
...
const isSpectatingEnabled = computed(() =>
  GameHelper.isSpectatingEnabled(game.value),
);
