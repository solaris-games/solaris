    ...
    <div class="header-bar-right header-bar-el">
      <div class="">
        <button
          ...
          v-if="isTutorialGame"
          @click="setMenuState({ state: 'tutorial' })"
        >
        ...
        <button
          ...
          v-if="!userPlayer && gameIsJoinable"
          @click="setMenuState({ state: 'welcome' })"
        >
          Join Now
        </button>
        ...
        <button
          class="btn btn-sm ms-1 d-lg-none"
          v-if="userPlayer && !isTutorialGame"
          :class="{
            'btn-outline-info': !unreadMessages,
            'btn-warning': unreadMessages,
          }"
          v-on:click="store.setMenuStateChat({ state: 'inbox' })"
          title="Inbox (M)"
        >
          <i class="fas fa-comments"></i>
          <span class="ms-1" v-if="unreadMessages">{{ unreadMessages }}</span>
        </button>
        ...
        <button
          ...
          v-if="userPlayer"
          :class="{
            'btn-outline-info': !unreadEvents,
            'btn-warning': unreadEvents,
          }"
          v-on:click="setMenuState({ state: 'eventLog' })"
          title="Event Log (E)"
        >
          <i class="fas fa-inbox"></i>
          <span class="ms-1" v-if="unreadEvents">{{ unreadEvents }}</span>
        </button>

        <hamburger-menu
          class="ms-1 d-none d-sm-inline-block"
          :buttonClass="'btn-sm btn-info'"
          :dropType="'dropleft'"
        />

        <button
          class="btn btn-sm btn-info ms-1 d-none d-sm-inline-block"
          type="button"
          @click="goToMyGames()"
        >
...
const gameIsPendingStart = computed(() =>
...
const gameIsWaitingForPlayers = computed(() =>
  GameHelper.isGameWaitingForPlayers(game.value),
);
const getGameStatusText = computed(() =>
  GameHelper.getGameStatusText(game.value),
);
const isTurnBasedGame = computed(() => GameHelper.isTurnBasedGame(game.value));
const isTimeMachineEnabled = computed(
  () => game.value.settings.general.timeMachine === "enabled",
);
const isSpecialistsCurrencyCreditsSpecialists = computed(() =>
  GameHelper.isSpecialistsCurrencyCreditsSpecialists(game.value),
);
...
const recalculateTimeRemaining = () => {
  ...
  if (gameIsPendingStart.value) {
    ...
    const ticksToProduction = GameHelper.getTicksToProduction(
      game.value,
      store.tick,
      store.productionTick || 0,
    );
    timeRemaining.value = getCountdownTimeStringByTicks(
      game.value,
      ticksToProduction,
    );
  }
...
const onTechnologyReceived = ({
  ...
  toast.info(
    `You received ${data.technology.name} level ${data.technology.level} from ${fromPlayer.alias}.`,
  );
};

const onCreditsReceived = ({
  data,
}: {
...
const onCreditsSpecialistsReceived = ({
  data,
}: {
  data: {
    fromPlayerId: string;
  ...
  toast.info(
    `You received ${data.creditsSpecialists} specialist token(s) from ${fromPlayer.alias}.`,
  );
};
...
onMounted(async () => {
  ...
  eventBus.on(
    PlayerEventBusEventNames.PlayerAllEventsRead,
    checkForUnreadEvents,
  );
  eventBus.on(
  ...
  onUnmounted(() => {
    ...
    eventBus.off(
      PlayerEventBusEventNames.PlayerEventRead,
      checkForUnreadEvents,
    );
    eventBus.off(
      PlayerEventBusEventNames.PlayerAllEventsRead,
