          ...
          <tr v-for="mapObject in mapObjects" :key="mapObject.data._id">
            <td
              :style="{
                padding: '0',
                width: '8px',
                'background-color': getFriendlyColour(mapObject),
              }"
            ></td>
            <td
              v-if="mapObject.type === 'star'"
              class="col-auto text-center ps-2 pe-2"
              @click="onViewObjectRequested(mapObject)"
            >
              <specialist-icon
                :type="'star'"
                :defaultIcon="'star'"
            ...
            <td class="bg-dark text-center ps-2 pe-2">
              <span>{{
                mapObject.data.ships == null ? "???" : mapObject.data.ships
              }}</span>
            </td>
            <td class="ps-2 pe-2">
              <span
            ...
            <td class="text-end ps-2 pe-2">
              <span
                v-if="
                  mapObject.type === 'carrier' &&
                  (userOwnsObject(mapObject) || mapObject.data.waypoints.length)
                "
              >
                <i class="fas fa-map-marker-alt"></i>
                <i
                  class="fas fa-sync ms-1"
                  v-if="mapObject.data.waypointsLooped"
                ></i>
            ...
            <td class="text-end ps-2 pe-2">
              <span
                v-if="
                  ...
                  !getObjectOwningPlayer(mapObject).defeated &&
                  !isGameFinished()
                "
              >
                <button
                  title="Transfer ships between carrier and star"
                  v-if="
                    mapObject.type === 'carrier' &&
                    mapObject.data.orbiting &&
                    userOwnsStar(mapObject.data.orbiting)
                  "
                  type="button"
                  class="btn btn-outline-primary"
                  @click="onShipTransferRequested(mapObject)"
                >
                  <i class="fas fa-exchange-alt"></i>
                </button>
                <button
                  title="Edit carrier waypoints"
                  v-if="mapObject.type === 'carrier'"
                  type="button"
                  class="btn btn-primary ms-2"
                  @click="onEditWaypointsRequested(mapObject.data._id)"
                >
                  <i class="fas fa-map-marker-alt"></i>
                </button>
                ...
                <button
                  ...
                  v-if="
                    mapObject.type === 'star' && hasCarriersInOrbit(mapObject)
                  "
                  type="button"
                  class="btn btn-outline-secondary ms-2"
                  @click="distributeShips(mapObject)"
                >
                ...
                <button
                  ...
                  @click="transferAllToStar(mapObject)"
                >
                  <i class="fas fa-arrow-up-wide-short"></i>
                </button>
                <button
                  title="Build a new carrier"
                  v-if="
                    mapObject.type === 'star' &&
                    mapObject.data.ships &&
                    hasEnoughCredits(mapObject) &&
                    mapObject.data.naturalResources &&
                    mapObject.data.naturalResources.economy &&
                    mapObject.data.naturalResources.industry &&
                    mapObject.data.naturalResources.science
                  "
                  type="button"
                  class="btn btn-info ms-2"
                  @click="onBuildCarrierRequested(mapObject.data._id)"
                >
...
const onBuildCarrierRequested = (starId: string) =>
  emit("onBuildCarrierRequested", starId);
...
const { transferAllToStar: tats, distributeShips: ds } =
  makeShipTransferActions(store, httpClient, toast);

const transferAllToStar = (mo: ObjectClicked) =>
  mo.type === "star" && tats(mo.data);
const distributeShips = (mo: ObjectClicked) =>
  mo.type === "star" && ds(mo.data);
...
const userOwnsObject = (mapObject: ObjectClicked) => {
  ...
  switch (mapObject.type) {
    case "star":
      owningPlayer = gameHelper.getStarOwningPlayer(
        store.game!,
        mapObject.data,
      )!;
      break;
    case "carrier":
      owningPlayer = gameHelper.getCarrierOwningPlayer(
        store.game!,
        mapObject.data,
      )!;
      break;
...
const getFriendlyColour = (mapObject: ObjectClicked) => {
  ...
  return gameHelper.getFriendlyColour(
    colourStore.getColourForPlayer(store.game!, owningPlayer._id)!.value,
  );
};
...
const onViewObjectRequested = (mapObject: ObjectClicked) => {
  switch (mapObject.type) {
    case "star":
      eventBus.emit(MapCommandEventBusEventNames.MapCommandClickStar, {
        starId: mapObject.data._id,
      });
      break;
    case "carrier":
      eventBus.emit(MapCommandEventBusEventNames.MapCommandClickCarrier, {
        carrierId: mapObject.data._id,
      });
      break;
  }
};
