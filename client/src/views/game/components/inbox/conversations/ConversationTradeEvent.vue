<template>
  <div
    class="container bg-dark mb-1"
    v-if="event"
    :class="{
      'left-message': !isFromUserPlayer,
      'right-message': isFromUserPlayer,
    }"
  >
    <div
      class="row mt-0"
      :style="{ 'background-color': fromPlayerColour }"
      style="height: 6px"
    ></div>
    <div class="row mt-0">
      <div class="col"></div>
    ...
    <div class="row mt-0">
      <div class="col mt-1">
        <p v-if="'renown' in event.data && event.data.renown" class="mb-1">
          <em
            >Sent
            <span class="text-warning">{{ event.data.renown }}</span>
            renown.</em
          >
        </p>
        <p v-if="'credits' in event.data && event.data.credits" class="mb-1">
          <em
            >Sent
            <span class="text-warning">{{ event.data.credits }}</span>
            credits.</em
          >
        </p>
        <p
          v-if="
            'creditsSpecialists' in event.data && event.data.creditsSpecialists
          "
          class="mb-1"
        >
          <em
            ...
            specialist token(s).</em
          >
        </p>
        <p
          v-if="'technology' in event.data && event.data.technology"
          class="mb-1"
        >
          <em
            >Sent
            <span class="text-warning"
              ...
              {{
                getTechnologyFriendlyName(
                  event.data.technology.name as ResearchTypeNotRandom,
                )
              }}</span
            >.</em
          >
        ...
        <p
          v-if="'carrierShips' in event.data && event.data.carrierShips"
          class="mb-1"
        >
          <em
            >Sent
        ...
        <p
          v-if="event.type === 'playerDebtSettled' && 'amount' in event.data"
          class="mb-1"
        >
          <em
            >Paid off
            <span class="text-warning">{{
              getFormattedDebtValue(Boolean(event.data.amount))
            }}</span>
            of debt.</em
          >
        </p>
        <p
          v-if="event.type === 'playerDebtForgiven' && 'amount' in event.data"
          class="mb-1"
        >
          <em
            >Forgave
        ...
        <p v-if="event.type === 'playerDiplomacyStatusChanged'" class="mb-1">
          ...
          <br /><br />
          <em
            >{{ event.data.playerToAlias }}:
            <span
              :class="{
                'text-success': event.data.statusFrom === 'allies',
                'text-info': event.data.statusFrom === 'neutral',
                'text-danger': event.data.statusFrom === 'enemies',
              }"
              >{{ event.data.statusFrom }}</span
            ></em
          >
          <br />
          <em
            >{{ event.data.playerFromAlias }}:
            <span
              :class="{
                'text-success': event.data.statusTo === 'allies',
                'text-info': event.data.statusTo === 'neutral',
                'text-danger': event.data.statusTo === 'enemies',
              }"
              >{{ event.data.statusTo }}</span
            ></em
          >
          <br /><br />
          <em
            ...
            <span
              :class="{
                ...
                'text-danger': event.data.actualStatus === 'enemies',
              }"
              >{{ event.data.actualStatus }}</span
            ></em
          >
        </p>
      </div>
...
const getTechnologyFriendlyName = (key: ResearchTypeNotRandom) =>
  TechnologyHelper.getFriendlyName(key);

const isTradeEvent = (
  ev: TradeEvent<string> | DiplomacyEvent<string>,
): ev is TradeEvent<string> => {
  return TRADE_EVENT_TYPES.includes(ev.type as BaseTradeEventTypes);
};

const isDiplomacyEvent = (
  ev: TradeEvent<string> | DiplomacyEvent<string>,
): ev is DiplomacyEvent<string> => {
...
const getFromPlayer = () => {
  if (isDiplomacyEvent(props.event)) {
    if (props.event.type === "playerDiplomacyStatusChanged") {
      return GameHelper.getPlayerById(
        game.value,
        props.event.data.playerIdFrom,
      );
    }
  } else {
    switch (props.event.type) {
      ...
      case "playerDebtSettled":
        return GameHelper.getPlayerById(
          game.value,
          props.event.data.debtorPlayerId,
        );
      case "playerDebtForgiven":
        return GameHelper.getPlayerById(
          game.value,
          props.event.data.creditorPlayerId,
        );
    }
  }
};
...
const dateText = computed(() => {
  ...
  if (
    "sentTick" in props.event &&
    (props.event.sentTick || props.event.sentTick === 0)
  ) {
