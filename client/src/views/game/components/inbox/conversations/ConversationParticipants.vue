<template>
  <div class="participant-container">
    <p v-if="isPartialPlayers" class="mb-2">
      <small>
        <span
          ...
          :key="participant"
          class="me-2 pointer"
          @click="onOpenPlayerDetailRequested(participant)"
        >
    ...
    <div v-if="!isPartialPlayers && !isOneVsOne" class="text-info mb-1">
      ...
      <p>
        <router-link :to="{ name: 'guidelines' }"
...
const isPartialPlayers = computed(
  () =>
    props.conversation.participants.length !== game.value.galaxy.players.length,
);
