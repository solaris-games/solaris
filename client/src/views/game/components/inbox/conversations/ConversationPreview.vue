<template>
  <div
    ...
    :class="{
      'bg-secondary': !isAllPlayersConversation,
      'bg-primary': isAllPlayersConversation,
      'bg-warning': conversation.unreadCount,
    }"
    ...
    <div class="row bg-dark mt-0">
      <div class="col-12" v-if="hasLastMessage">
        <p class="mt-2 mb-2" :class="{ truncate: isTruncated }">
          <player-icon
            v-if="lastMessageSender && lastMessage!.fromPlayerId"
            :playerId="lastMessageSender._id"
          />
      ...
      <div class="col-12" v-if="hasLastMessage && lastMessage">
        <small class="float-end mb-2"
          ><i>{{ GameHelper.getDateString(lastMessage.sentDate) }}</i></small
        >
      </div>
      <div class="col-12" v-if="!hasLastMessage">
...
const hasReadLastMessage = computed(
  () =>
    hasLastMessage.value &&
    props.conversation.lastMessage!.readBy.find(
      (r) => r === userPlayer.value._id,
    ),
);
const lastMessageSender = computed(
  () =>
    props.conversation.lastMessage?.fromPlayerId &&
    GameHelper.getPlayerById(
      game.value,
      props.conversation.lastMessage.fromPlayerId,
    ),
);
const isAllPlayersConversation = computed(
  () =>
    props.conversation.participants.length ===
    game.value.settings.general.playerLimit,
);

const lastMessageText = computed(
  () =>
    lastMessage.value &&
    mentionHelper.replaceMentionsWithNames(lastMessage.value.message),
);
