<template>
  <div
    class="container message-container"
    :class="{
      'left-message': !isFromUserPlayer,
      'right-message': isFromUserPlayer,
      'bg-dark': !message.pinned,
      'bg-secondary': message.pinned,
    }"
  >
    <div
      class="row mt-0"
      v-if="fromPlayer"
      :style="{ 'background-color': fromColour }"
      style="height: 6px"
    ></div>
    <div class="row mt-0" v-if="message">
      ...
      <conversation-message-context-menu
...
const isFromUserPlayer = computed(
  () => props.message.fromPlayerId === userPlayer.value._id,
);
const fromPlayer = computed(() =>
  GameHelper.getPlayerById(game.value, props.message.fromPlayerId!),
);
const userPlayerHasReadMessage = computed(
  () => props.message.readBy.find((x) => userPlayer.value._id === x) != null,
);
const fromColour = computed(
  () =>
    colourStore.getColourForPlayer(game.value, fromPlayer.value!._id)!.value,
);
