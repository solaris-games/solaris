    ...
    <table class="table table-striped table-hover">
      <tbody>
        <tr v-for="player of filledSlots">
          <td>{{ player.alias }}</td>
          <td>
            <button class="btn btn-warning" @click="kickPlayer(player)">
              <i class="fas fa-hammer"></i> Kick
            </button>
          </td>
        </tr>
      </tbody>
...
const filledSlots = computed(() =>
  props.game.galaxy.players.filter((p) => !p.isOpenSlot),
);
...
const kickPlayer = async (player: Player<string>) => {
  if (
    await confirm(
      `Kick Player`,
      `Do you really want to kick player ${player.alias}?`,
    )
  ) {
    ...
    if (isOk(response)) {
      ...
      toast.error(
        `Failed to kick player: ${extractErrors(response).join(", ")}`,
      );
    }
  }
};
