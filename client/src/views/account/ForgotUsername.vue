  ...
  <view-container :is-auth-page="false">
    ...
    <form @submit.prevent="handleSubmit">
      <div class="mb-2">
        ...
        <input type="email" :required="true" class="form-control" name="email" v-model="email" :disabled="isLoading">
      </div>
...
const handleSubmit = async (e: Event) => {
  ...
  if (isOk(response)) {
    toast.success(`Your username has been sent to your email address, please check your email inbox.`);
    router.push({ name: 'home' });
  } else {
    console.error(formatError(response));
    errors.value = extractErrors(response);
    toast.error(`There was a problem requesting your username, please check that you entered your email address correctly.`);
  }
