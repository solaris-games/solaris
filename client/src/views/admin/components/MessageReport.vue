    ...
    <div v-if="conversation" class="well report-messages">
      <div v-for="message in messages" :key="message._id" class="panel">
        <div class="panel-header">
          <h6
            class="panel-title"
            :class="isReportedMessage(message) ? 'text-danger' : null"
          >
            {{ message.sentDate }}: {{ message.fromPlayerAlias }}
          </h6>
        </div>
        <div class="panel-body">
          <p :class="isReportedMessage(message) ? 'text-danger' : null">
            {{ message.message }}
          </p>
        </div>
      </div>
...
const showMessages = async () => {
  ...
  if (isOk(resp)) {
    ...
    messages.value = conv.messages.filter(
      (msg) => msg.type === "message",
    ) as ConversationMessage<string>[];
  } else {
