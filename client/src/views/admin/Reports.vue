    ...
    <div v-if="reports">
      ...
      >
        <div class="panel-heading">
          <h5
            class="panel-title"
            :class="report.actioned ? 'text-default' : 'text-warning'"
          >
            <router-link
              :to="{
                path: '/administration/users/',
                query: { userId: report.reportedByUserId },
              }"
              >{{ report.reportedByPlayerAlias }}</router-link
            >
            reported
            <router-link
              :to="{
                path: '/administration/users/',
                query: { userId: report.reportedUserId },
              }"
              >{{ report.reportedPlayerAlias }}</router-link
            >
        ...
        <div class="panel-body">
          <p>
            ...
            <span v-if="report.reasons.multiboxing" class="me-2"
              >Multiboxing</span
            >
            <span v-if="report.reasons.inappropriateAlias" class="me-2"
              >Inappropriate Alias</span
            >
          </p>
        ...
        <div class="panel-footer">
          ...
          >
          <button
            class="btn btn-small clickable"
            :class="report.actioned ? 'btn-success' : 'btn-danger'"
            @click="doActionReport(report)"
          >
            <i
              class="fas"
              :class="{
                'fa-check': report.actioned,
                'fa-times': !report.actioned,
              }"
              title="Action Report"
            ></i>
          </button>
