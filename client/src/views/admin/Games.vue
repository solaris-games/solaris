          ...
          <tr v-for="game of games" :key="game._id">
            ...
            <td>
              <i
                class="clickable fas"
                :class="{
                  'fa-star text-success': game.settings.general.featured,
                  'fa-star text-danger': !game.settings.general.featured,
                }"
                @click="toggleFeaturedGame(game)"
                title="Featured"
              ></i>
              <i
                class="clickable ms-1 fas"
                :class="{
                ...
                @click="toggleTimeMachineGame(game)"
                v-if="isGameMaster"
                title="Time Machine"
              ></i>
            </td>
            <td>
              <i
                class="fas"
                :class="{
                  'fa-check text-success': game.state.startDate,
                  'fa-times text-danger': !game.state.startDate,
                }"
                :title="game.state.startDate?.toString()"
              ></i>
            </td>
            <td>
              <i
                class="clickable fas"
                :class="{
                  'fa-check text-success': game.state.endDate,
                  'fa-times text-danger': !game.state.endDate,
                }"
...
const isCommunityManager = computed(
  () => isAdministrator.value || userStore.roles?.communityManager,
);
const isGameMaster = computed(
  () => isAdministrator.value || userStore.roles?.gameMaster,
);

const gameNeedsAttention = (game: AdminListGame<string>) =>
  game.state.endDate && game.state.tick <= 12;
...
const forceGameFinish = async (game: AdminListGame<string>) => {
  ...
  if (
    await confirm(
      "Force Game Finish",
      "Are you sure you want to force this game to finish?",
    )
  ) {
