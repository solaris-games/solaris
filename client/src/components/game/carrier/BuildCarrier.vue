...
export default {
  ...
  methods: {
    ...
    async saveTransfer (e) {
      const confirmed = await this.$confirm('Build a carrier', `Are you sure you want to build a Carrier at ${this.star.name}? The carrier will cost ${this.star.upgradeCosts.carriers}.`)
      if (!confirmed) {
        return
      }
