...
const failed = (expected: string, value: any) => {
    const got =
        value === null
            ...
            : value === undefined
              ? "undefined"
              : value.toString().substring(1000);

    return new ValidationError(
        `Expected ${expected}, but got: ${got} (type ${typeof value})`,
    );
};
...
export const maybeNull = <A>(validator: Validator<A>): Validator<A | null> =>
    or(validator, nullish);

export const maybeUndefined = <A>(
    validator: Validator<A>,
): Validator<A | undefined> => or(validator, undefinedish);
...
export const sizedArray = <A>(
    ...
    return (v) => {
        ...
        if (arr.length < minSize) {
            throw new ValidationError(
                `Expected array of at least size ${minSize}, but got size ${arr.length}`,
            );
        }

        if (arr.length > maxSize) {
            throw new ValidationError(
                `Expected array of at most size ${maxSize}, but got size ${arr.length}`,
            );
        }
    ...
    return (v) => {
        ...
        for (const key of Object.keys(objValidator)) {
            try {
                ...
                throw new ValidationError(
                    `Error in field ${key}: ${err.message}`,
                );
            }
        }
...
export const stringEnumeration = <A extends string, M extends A[]>(
    members: readonly [any, ...M],
): Validator<A> => {
    return (v) => {
        const s = string(v);
...
export const numberEnumeration = <A extends number, M extends A[]>(
    members: readonly [any, ...M],
): Validator<A> => {
    return (v) => {
        const n = number(v);
...
export const stringValue = (props: StringValidationProps) => (v: any) => {
    ...
    if (props.ignoreForLengthCheck) {
        sForLengthCheck = sForLengthCheck.replace(
            props.ignoreForLengthCheck,
            "",
        );
    }
...
export const UNICODE_PRINTABLE_CHARACTERS_NON_WHITESPACE =
    /^[\p{L}\p{N}\p{M}\p{Cf}\p{S}\p{P}]+$/u;

export const UNICODE_PRINTABLE_CHARACTERS_WITH_WHITESPACE =
    /^[\p{L}\p{N}\p{M}\p{Cf}\p{S}\p{P}\p{Z}]+$/u;

export const UNICODE_INVISIBLE_CHARACTERS = /[\p{C}\p{Mn}\p{Me}]+/u;

export const UNICODE_LETTERS_NUMBERS_PUNCTUATION = /^[\p{L}\p{N}\p{P}]+$/u;

export const username = named(
    "username",
    stringValue({
        minLength: 3,
        maxLength: 30,
        trim: true,
        matches:
            /^[\p{L}\p{N}\p{M}\p{Cf}\p{S}\p{P}]+[\p{L}\p{N}\p{M}\p{Cf}\p{S}\p{P}\p{Z}]*$/u,
    }),
);

export const email = named(
    "email",
    stringValue({
        minLength: 3,
        maxLength: 100,
