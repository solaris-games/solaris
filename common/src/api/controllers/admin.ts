...
export type RoleSpecificUserInfo =
    { adminRole: "communityManager" } | AdminSpecificUserInfo;
...
export const createAdminRoutes = <ID>() => ({
    ...
    listPasswordResets: new SimpleGetRoute<ListPasswordReset<ID>[]>(
        "/api/admin/passwordresets",
    ),
    addWarning: new PostRoute<{ userId: string }, {}, AddWarningReq, null>(
        "/api/admin/user/:userId/warning",
    ),
    setRoleContributor: new PatchRoute<
        { userId: string },
        {},
        SetRoleReq,
        null
    >("/api/admin/user/:userId/contributor"),
    setRoleDeveloper: new PatchRoute<{ userId: string }, {}, SetRoleReq, null>(
        "/api/admin/user/:userId/developer",
    ),
    setRoleCommunityManager: new PatchRoute<
        { userId: string },
        {},
        SetRoleReq,
        null
    >("/api/admin/user/:userId/communityManager"),
    setRoleGameMaster: new PatchRoute<{ userId: string }, {}, SetRoleReq, null>(
        "/api/admin/user/:userId/gameMaster",
    ),
    setCredits: new PatchRoute<
        { userId: string },
        {},
        SetCreditsReq,
        { credits: number }
    >("/api/admin/user/:userId/credits"),
    ban: new PatchRoute<{ userId: string }, {}, null, null>(
        "/api/admin/user/:userId/ban",
    ),
    unban: new PatchRoute<{ userId: string }, {}, null, null>(
        "/api/admin/user/:userId/unban",
    ),
    resetAchievements: new PatchRoute<{ userId: string }, {}, null, null>(
        "/api/admin/user/:userId/resetAchievements",
    ),
    promoteToEstablishedPlayer: new PatchRoute<
        { userId: string },
        {},
        null,
        null
    >("/api/admin/user/:userId/promoteToEstablishedPlayer"),
    impersonate: new PostRoute<
        { userId: string },
        {},
        null,
        ImpersonateResp<ID>
    >("/api/admin/user/:userId/impersonate"),
    endImpersonate: new SimplePostRoute<null, ImpersonateResp<ID>>(
        "/api/admin/endImpersonate",
    ),
    listGames: new SimpleGetRoute<AdminListGame<ID>[]>("/api/admin/game"),
    setGameFeatured: new PatchRoute<
        { gameId: string },
        {},
        SetFeaturedReq,
        null
    >("/api/admin/game/:gameId/featured"),
    setGameTimeMachine: new PatchRoute<
        { gameId: string },
        {},
        SetTimeMachineReq,
        null
    >("/api/admin/game/:gameId/timeMachine"),
    finishGame: new PatchRoute<{ gameId: string }, {}, null, null>(
        "/api/admin/game/:gameId/finish",
    ),
    getConversationForReport: new GetRoute<
        { reportId: string },
        {},
        Conversation<ID>
    >("/api/admin/reports/:reportId/conversation"),
    listReports: new SimpleGetRoute<Report<ID>[]>("/api/admin/reports"),
    actionReport: new PatchRoute<{ reportId: string }, {}, {}, null>(
        "/api/admin/reports/:reportId/action",
    ),
    createAnnouncement: new SimplePostRoute<CreateAnnouncementReq, null>(
        "/api/admin/announcements/",
    ),
    deleteAnnouncement: new DeleteRoute<{ id: string }, {}, null>(
        "/api/admin/announcements/:id",
    ),
    getAllAnnouncements: new SimpleGetRoute<Announcement<ID>[]>(
        "/api/admin/announcements/",
    ),
});
