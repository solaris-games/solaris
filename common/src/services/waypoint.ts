...
} from "../types/common/carrierWaypoint";
...
export class WaypointService<ID extends Id> {
    ...
    supportsActionShips(action: CarrierWaypointActionType) {
        const actions: CarrierWaypointActionType[] = [
            "drop",
            "collect",
            "dropPercentage",
            "collectPercentage",
    ...
    starRouteIsWithinHyperspaceRange(
        game: Game<ID>,
        carrier: Carrier<ID>,
        sourceStar: Star<ID>,
        ...
        return this.carrierTravelService.isWithinHyperspaceRange(
            game,
            carrier,
            sourceStar,
            destinationStar,
    ...
    canLoop(game: Game<ID>, carrier: Carrier<ID>) {
        ...
        const firstWaypointStar = this.starService.getById(
            game,
            firstWaypoint.destination,
        );
        const lastWaypointStar = this.starService.getById(
            game,
    ...
    calculateWaypointTicks(
        game: Game<ID>,
        carrier: Carrier<ID>,
        waypoint: CarrierWaypoint<ID>,
    ) {
        ...
        let source =
            sourceStar == null ? carrier.location : sourceStar.location;
        ...
        if (
            !carrier.orbiting &&
            carrier.waypoints[0]._id.toString() === waypoint._id.toString()
        ) {
    ...
    populateCarrierWaypointEta(game: Game<ID>, carrier: Carrier<ID>) {
        ...
        if (carrier.waypoints.length) {
            ...
            carrier.ticksEtaTotal =
                carrier.waypoints[carrier.waypoints.length - 1].ticksEta;
        } else {
