...
export class StarDistanceService {
    ...
    getDistanceBetweenStars<ID extends Id>(
        star1: MapObject<ID>,
        star2: MapObject<ID>,
    ) {
    ...
    getDistanceBetweenStarAndLocation<ID extends Id>(
        star: Star<ID>,
        loc: Location,
    ) {
        return this.distanceService.getDistanceBetweenLocations(
    ...
    getClosestUnownedStars<ID extends Id>(
        star: Star<ID>,
        stars: Star<ID>[],
        amount: number,
    ) {
    ...
    getClosestPlayerOwnedStars<ID extends Id>(
        star: Star<ID>,
        stars: Star<ID>[],
        player: Player<ID>,
    ) {
        return this.getClosestOwnedStars(star, stars).filter(
            (s) =>
                s.ownedByPlayerId &&
                s.ownedByPlayerId.toString() === player._id.toString(),
        );
    }

    getClosestPlayerOwnedStarsFromLocation<ID extends Id>(
        location: Location,
        stars: Star<ID>[],
        ownedByPlayerId: ID,
    ) {
        return stars
            .filter(
                (s) =>
                    s.ownedByPlayerId &&
                    s.ownedByPlayerId.toString() === ownedByPlayerId.toString(),
            )
            .sort(
                (a, b) =>
                    this.getDistanceBetweenStarAndLocation(a, location) -
                    this.getDistanceBetweenStarAndLocation(b, location),
            );
    }

    getClosestPlayerOwnedStarsFromLocationWithinDistance<ID extends Id>(
        location: Location,
        stars: Star<ID>[],
        ownedByPlayerId: ID,
        maxDistance: number,
        ...
        return stars
            ...
            .filter((s) => {
                let distance = this.getDistanceBetweenStarAndLocation(
                    s,
                    location,
                );
    ...
    getClosestUnownedStarsFromLocation<ID extends Id>(
        location: Location,
        stars: Star<ID>[],
        amount: number,
    ) {
    ...
    getClosestUnownedStarFromLocation<ID extends Id>(
        location: Location,
        stars: Star<ID>[],
    ) {
    ...
    getStarsWithinRadiusOfStar<ID extends Id>(
        star: Star<ID>,
        stars: Star<ID>[],
        radius: number,
    ) {
        return stars.filter(
            (s) =>
                s._id.toString() !== star._id.toString() &&
                this.getDistanceBetweenStars(star, s) <= radius,
        );
    }

    isStarTooClose<ID extends Id>(
        game: Game<ID>,
        star: Star<ID>,
        otherStar: Star<ID>,
    ) {
    ...
    isStarLocationTooClose<ID extends Id>(
        game: Game<ID>,
        location: Location,
        otherStar: Star<ID>,
    ) {
    ...
    isLocationTooClose<ID extends Id>(
        game: Game<ID>,
        location: Location,
    ...
    isDuplicateStarPosition<ID extends Id>(
        location: Location,
        stars: Star<ID>[],
    ) {
        const samePositionStars = stars.filter((star2) => {
            return (
                location.x === star2.location.x &&
    ...
    getClosestStarFromLocation<ID extends Id>(
        loc: Location,
        stars: Star<ID>[],
    ) {
    ...
    getClosestStarsFromLocation<ID extends Id>(
        loc: Location,
        stars: Star<ID>[],
        amount: number,
    ) {
    ...
    getFurthestStarsFromLocation<ID extends Id>(
        loc: Location,
        stars: Star<ID>[],
        amount: number,
    ) {
