...
export class Helpers {
    rotateCarrierTowardsWaypoint(
        ...
        if (carrier.waypoints.length) {
            ...
            const starDestination = stars.find(
            ...
            if (!starDestination) {
                ...
                const angle = this.getAngleTowardsLocation(
                    carrier.location,
                    sourceStar.location,
                );
    ...
    calculateDepthModifiers(userSettings: UserGameSettings, seeds: string[]) {
        ...
        for (let seed of seeds) {
            sum =
                this.calculateDepthModifier(userSettings, sum.toString()) +
                this.calculateDepthModifier(userSettings, seed);
        }
    ...
    calculateMinStarX(game: Game): number {
        ...
        return game.galaxy.stars.sort((a, b) => a.location.x - b.location.x)[0]
            .location.x;
    }
    ...
    calculateMinStarY(game: Game) {
        ...
        return game.galaxy.stars.sort((a, b) => a.location.y - b.location.y)[0]
            .location.y;
    }
    ...
    calculateMaxStarX(game: Game) {
        ...
        return game.galaxy.stars.sort((a, b) => b.location.x - a.location.x)[0]
            .location.x;
    }
    ...
    calculateMaxStarY(game: Game) {
        ...
        return game.galaxy.stars.sort((a, b) => b.location.y - a.location.y)[0]
            .location.y;
    }
    ...
    calculateMinCarrierX(game: Game) {
        if (!game.galaxy.carriers.length) {
            return 0;
        }

        return game.galaxy.carriers.sort(
            (a, b) => a.location.x - b.location.x,
        )[0].location.x;
    }
    ...
    getPlayerTerritoryCenter(
        distanceService: DistanceService,
        game: Game,
        player: Player,
    ) {
        const playerStars = this.getStarsOwnedByPlayer(
            player,
            game.galaxy.stars,
        );
        ...
        const centerX =
            playerStars.reduce((sum, s) => sum + s.location.x, 0) /
            playerStars.length;
        const centerY =
            playerStars.reduce((sum, s) => sum + s.location.y, 0) /
            playerStars.length;

        let closestStar = this.getClosestPlayerStar(
            distanceService,
            game.galaxy.stars,
            {
                x: centerX,
    ...
    getCarrierOwningPlayer(game: Game, carrierData: Carrier) {
        return game.galaxy.players.find(
            (x) => x._id === carrierData.ownedByPlayerId,
        );
    }
}
