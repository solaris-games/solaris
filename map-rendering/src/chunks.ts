...
export class Chunks {
    ...
    update(game: Game, stars: Star[], carriers: Carrier[]) {
        ...
        stars.forEach((s) =>
            this.addContainerToChunk(s, this.firstChunkX, this.firstChunkY),
        );
        carriers.forEach((c) =>
            this.addContainerToChunk(c, this.firstChunkX, this.firstChunkY),
        );
    }
            ...
            for (let iy = 0; iy < this.numof_chunkY; iy++) {
                if (
                    ...
                    if (!this.chunks[ix][iy].visible) {
                        ...
                        if (zoomChanging) {
                            for (let mapObject of this.chunks[ix][iy]
                                .mapObjects) {
                                mapObject.onZoomChanging(zoomPercent);
                            }
                        }
    ...
    addContainerToChunk(mapObject: MapObject, firstX: number, firstY: number) {
        // Star or carrier
        ...
        for (let chunkX of this.chunks) {
            for (let chunkY of chunkX) {
                if (chunkY.mapObjects.indexOf(mapObject) > -1) {
                    chunkY.mapObjects.splice(
                        chunkY.mapObjects.indexOf(mapObject),
                        1,
                    );
