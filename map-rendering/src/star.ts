    ...
    */
    ...
    _getStarPlayer() {
        return this.game.galaxy.players.find(
            (x) => x._id === this.data.ownedByPlayerId,
        );
    }

    _getStarCarriers() {
        return this.game.galaxy.carriers.filter(
            (x) => x.orbiting === this.data._id,
        );
    }

    _getStarCarrierShips() {
        return this._getStarCarriers().reduce(
            (sum, c) => sum + (c.ships || 0),
            0,
        );
    }
    ...
    drawStar() {
        ...
        if (this.data.isInScanningRange) {
            ...
            if (this.isBinaryStar()) {
                if (this.hasBlackHole()) {
                    this.graphics_star = new Sprite(
                        TextureService.STAR_SYMBOLS["black_hole_binary"],
                    );
                } else if (this._isDeadStar()) {
                    this.graphics_star = new Sprite(
                        TextureService.STAR_SYMBOLS["binary_unscannable"],
                    );
                } else {
                    this.graphics_star = new Sprite(
                        TextureService.STAR_SYMBOLS["binary_scannable"],
                    );
                }
            }
            // ---- Non binary stars ----
                ...
                this.graphics_star = new Sprite(
                    TextureService.STAR_SYMBOLS["black_hole"],
                );
            } else if (this._isDeadStar()) {
                this.graphics_star = new Sprite(
                    TextureService.STAR_SYMBOLS["unscannable"],
                );
            } else if (this.data.homeStar) {
                this.graphics_star = new Sprite(
                    TextureService.STAR_SYMBOLS["home"],
                );
            } else {
                this.graphics_star = new Sprite(
                    TextureService.STAR_SYMBOLS["scannable"],
                );
            }
        } else {
            this.graphics_star = new Sprite(
        ...
        if (
            this.starDataService.isCapitalEliminationCapital(
                this.game,
                this.data,
            )
        ) {
            ...
            this.starDataService.isCapitalCaptureCapital(this.game, this.data)
        ) {
    ...
    drawAsteroidField() {
        ...
        this.asteroidFieldSprite.rotation =
            Star.seededRNG.random() * Math.PI * 2.0;
    ...
    drawPlanets() {
        ...
        for (let i = 0; i < planetCount; i++) {
            ...
            let planetSize =
                (Math.floor(Math.abs(this.data.location.y) + distanceToStar) %
                    1.5) +
                0.5;
    ...
    drawNaturalResourcesRing() {
        ...
        for (let lod = 0; lod < Star.maxLod; lod += 1) {
            ...
            let averageNaturalResources =
                this._calculateAverageNaturalResources(
            ...
            // TODO: Experimental:
            let ringRadius =
                averageNaturalResources <= 50
                    ? averageNaturalResources
                    : averageNaturalResources > 400
                      ? 100
                      : 12.5 * Math.log2(averageNaturalResources / 50) + 50;
            ...
            lineWidth *= lod + 1;
            this.graphics_natural_resources_ring[lod].clear();
            this.graphics_natural_resources_ring[lod].circle(
                0,
                0,
                ringRadius * 0.75,
            );
            ...
            this.graphics_natural_resources_ring[lod].scale.x =
                1.0 / ((1.0 / 8.0) * (lod + 1));
            this.graphics_natural_resources_ring[lod].scale.y =
                1.0 / ((1.0 / 8.0) * (lod + 1));
    ...
    _calculateAverageNaturalResources(naturalResources: NaturalResources) {
        return Math.floor(
            (naturalResources.economy +
                naturalResources.industry +
                naturalResources.science) /
                3,
        );
    }
    ...
    _getPlanetsCount() {
        ...
        let averageNaturalResources = this._calculateAverageNaturalResources(
            this.data.naturalResources,
        );
    ...
    drawColour() {
        ...
        if (Object.keys(TextureService.PLAYER_SYMBOLS).includes(player.shape)) {
            ...
            this.graphics_shape_part = new Sprite(
                TextureService.PLAYER_SYMBOLS[player.shape][2 + wgFlag],
            );
            this.graphics_shape_full = new Sprite(
                TextureService.PLAYER_SYMBOLS[player.shape][0 + wgFlag],
            );
        }
    ...
    _hasUnknownShips() {
        ...
        let scramblers = carriersOrbiting.reduce(
            (sum, c) => sum + (c.ships === null ? 1 : 0),
            0,
        );
    ...
    drawName() {
        ...
        let totalKnownShips =
            (this.data.ships || 0) + this._getStarCarrierShips();
        let carriersOrbiting = this._getStarCarriers();

        if (
            (this.data.ownedByPlayerId || carriersOrbiting) &&
            (totalKnownShips > 0 ||
                carriersOrbiting.length > 0 ||
                this._hasUnknownShips())
        ) {
            this.text_name.y =
                (NAME_SIZE + Star.shipsSmallSize) / 2.0 - NAME_SIZE;
        } else {
    ...
    drawShips() {
        ...
        if (this.data.ownedByPlayerId || carriersOrbiting.length) {
            ...
            if (carriersOrbiting) {
                scramblers = carriersOrbiting.reduce(
                    (sum, c) => sum + (c.ships === null ? 1 : 0),
                    0,
                );
            }
            ...
            if (carrierCount) {
                ...
                if (
                    helpers.isStarHasMultiplePlayersInOrbit(
                        this.game,
                        this.data,
                    )
                ) {
        ...
        if (shipsText) {
            if (!this.text_ships_small) {
                ...
                this.text_ships_small.y =
                    -this.text_ships_small.height +
                    ((NAME_SIZE + Star.shipsSmallSize) / 2.0 - NAME_SIZE);
            }
    ...
    drawInfrastructure() {
        ...
        if (
            ...
            (this.data.infrastructure.economy == null ||
                this.data.infrastructure.industry == null ||
                this.data.infrastructure.science == null)
        ) {
            return;
        }
        ...
        if (!this.text_infrastructure) {
            if (this.data.ownedByPlayerId && this.data.infrastructure) {
                ...
                this.text_infrastructure.x = -(
                    this.text_infrastructure.width / 2.0
                );
    ...
    drawInfrastructureBulkIgnored() {
        ...
        if (!this.text_infrastructureBulkIgnored) {
            ...
            this.text_infrastructureBulkIgnored = new BitmapText({
                text: displayInfrastructure,
                style: bitmapFont,
            });
            this.text_infrastructureBulkIgnored.x = -(
                this.text_infrastructureBulkIgnored.width / 2.0
            );
            this.text_infrastructureBulkIgnored.y = 12;
            this.text_infrastructureBulkIgnored.visible =
                this.showIgnoreBulkUpgradeInfrastructure;
    ...
    drawDepth() {
        const depth = Helpers.calculateDepthModifier(
            this.userSettings,
            this.data._id,
        );
    ...
    updateVisibility() {
        ...
        if (this.userSettings.map.naturalResources !== "planets") {
            if (this.graphics_natural_resources_ring[lod]) {
                this.graphics_natural_resources_ring[lod].visible = Boolean(
                    this.data.isInScanningRange &&
                    this.zoomPercent >=
                        Star.zoomLevelDefinitions.naturalResources,
                );
            }
        }

        if (this.text_name) {
            this.text_name.visible =
                this.isSelected ||
                this.zoomPercent >= Star.zoomLevelDefinitions.name;
        }

        if (this.container_planets) {
            this.container_planets.visible = Boolean(
                this.data.isInScanningRange &&
                this.zoomPercent >= Star.zoomLevelDefinitions.naturalResources,
            );
        }

        if (this.text_infrastructure)
            this.text_infrastructure.visible =
                this.isSelected ||
                this.zoomPercent >= Star.zoomLevelDefinitions.infrastructure;
        ...
        let visible_ships = !!(
            ...
            (this.isSelected ||
                this.isMouseOver ||
                this.zoomPercent >= Star.zoomLevelDefinitions.shipCount)
        );

        if (this.text_ships_small)
            this.text_ships_small.visible = small_ships && visible_ships;
        if (this.text_ships_big)
            this.text_ships_big.visible = !small_ships && visible_ships;

        let partial_ring =
            (this.text_ships_big && this.text_ships_big.visible) ||
    ...
    _isDeadStar() {
        return (
            this.data.naturalResources != null &&
            this.data.naturalResources.economy <= 0 &&
            this.data.naturalResources.industry <= 0 &&
            this.data.naturalResources.science <= 0
        );
    }
