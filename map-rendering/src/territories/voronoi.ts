...
const computeIntersection = (
    ...
    if (currentPoint[0] - prevPoint[0] === 0) {
        ...
        const m1 =
            (currentPoint[1] - prevPoint[1]) / (currentPoint[0] - prevPoint[0]);
...
const isInside = (point: P, edgeS: P, edgeE: P) => {
    const d =
        (point[0] - edgeS[0]) * (edgeE[1] - edgeS[1]) -
        (point[1] - edgeS[1]) * (edgeE[0] - edgeS[0]);
...
export const drawTerritoriesVoronoi = (
    game: Game,
    ...
    const hullPointsUnscaled: P[] = stars.map((s) => [
        s.location.x,
        s.location.y,
    ]);
