...
export const drawTerritoriesMarchingSquare = (
    ...
    const LINE_PROPORTION =
        (1 / 16) * userSettings.map.marchingSquareBorderWidth;
    ...
    const VERTEX_TABLE: VertexSpec[] = [
        ...
        [
            VertexAction.ACTION_NEW,
            [
                { x: 0, y: 0.5 + LINE_OFFSET },
                { x: 0.5 - LINE_OFFSET, y: 1 },
            ],
            [
                { x: 0, y: 0.5 + LINE_OFFSET },
                { x: 0.5 - LINE_OFFSET, y: 1 },
    ...
    const samplePoints: SamplePoint[][] = Array.from(
        Array(gridWidth + 1),
        () => new Array(gridHeight + 1),
    );
    ...
    for (let star of stars) {
        ...
        const startIX = Math.ceil(
            (star.location.x - METABALL_RADIUS - minX) / CELL_SIZE,
        ); //The minimum ix can be and still be in the METABALL_RADIUS
        const endIX = Math.floor(
            (star.location.x + METABALL_RADIUS - minX) / CELL_SIZE,
        ); //The maximum ix can be and still be in the METABALL_RADIUS

        for (let ix = startIX; ix <= endIX; ix++) {
            const startIY = Math.ceil(
                (star.location.y -
                    Math.sqrt(
                        METABALL_RADIUS ** 2 -
                            (star.location.x - (ix * CELL_SIZE + minX)) ** 2,
                    ) -
                    minY) /
                    CELL_SIZE,
            ); // The minimum iy can be and still be in the METABALL_RADIUS
            const endIY = Math.floor(
                (star.location.y +
                    Math.sqrt(
                        METABALL_RADIUS ** 2 -
                            (star.location.x - (ix * CELL_SIZE + minX)) ** 2,
                    ...
                    minY) /
                    CELL_SIZE,
            ); // The maximum iy can be and still be in the METABALL_RADIUS
            ...
            for (let iy = startIY; iy <= endIY; iy++) {
                ...
                const distance = distanceService.getDistanceBetweenLocations(
                    gridLocation,
                    star.location,
                ); // Get the distance between the gridPoint and the star

                if (
                    samplePoints[ix][iy] &&
                    samplePoints[ix][iy].distance < distance
                ) {
                    // If the gridpoint has a value AND the distance currently logged (from a previous star) is smaller than the current one THEN don't log anything
                    ...
                    const owner = game.galaxy.players.find(
    ...
    const playerSamplePoints: (Player | undefined)[][] = new Array(
        samplePoints.length,
    );
    ...
    for (let player of game.galaxy.players) {
        ...
        const getLinesX =
            ...
            (offset: number) =>
                VERTEX_TABLE[idx][LINES_INDEX][offset].x * CELL_SIZE +
                cellOrigin.x;
        const getLinesY =
            (idx: number) =>
            (cellOrigin: { x: number; y: number }) =>
            (offset: number) =>
                VERTEX_TABLE[idx][LINES_INDEX][offset].y * CELL_SIZE +
                cellOrigin.y;
        ...
        for (let ix = 0; ix < samplePoints.length - 1; ix++) {
            for (let iy = 0; iy < samplePoints[ix].length - 1; iy++) {
                ...
                if (
                    VERTEX_TABLE[lookUpIndex][ACTION_INDEX] !=
                    VertexAction.ACTION_SKIP
                ) {
                    const cellOrigin = {
                        x: ix * CELL_SIZE + minX,
                    ...
                    if (
                        ...
                        if (combining) {
                            //finish combining
                            territoryPolygons.lineTo(
                                VERTEX_TABLE[15][POLYGON_INDEX][1].x *
                                    CELL_SIZE +
                                    cellOrigin.x,
                                VERTEX_TABLE[15][POLYGON_INDEX][1].y *
                                    CELL_SIZE +
                                    cellOrigin.y,
                            );
                            territoryPolygons.lineTo(
                                VERTEX_TABLE[15][POLYGON_INDEX][0].x *
                                    CELL_SIZE +
                                    cellOrigin.x,
                                VERTEX_TABLE[15][POLYGON_INDEX][0].y *
                                    CELL_SIZE +
                                    cellOrigin.y,
                            );
                            combining = false;
                        }
                        territoryPolygons.moveTo(
                            VERTEX_TABLE[lookUpIndex][POLYGON_INDEX][0].x *
                                CELL_SIZE +
                                cellOrigin.x,
                            VERTEX_TABLE[lookUpIndex][POLYGON_INDEX][0].y *
                                CELL_SIZE +
                                cellOrigin.y,
                        );
                        alpha = 1;
                        ...
                        for (let vertex of vertices) {
                            ...
                            territoryPolygons.lineTo(
                                vertex.x * CELL_SIZE + cellOrigin.x,
                                vertex.y * CELL_SIZE + cellOrigin.y,
                            );
                        }
