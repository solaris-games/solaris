...
import {
    ...
    type MapObject as MapObjectData,
    type Location,
    DistanceService,
} from "@solaris/common";
...
export class PathManager {
    ...
    addSharedPath(
        objectA: MapObjectData<string>,
        objectB: MapObjectData<string>,
        carrierMapObject: Carrier,
    ) {
        const mapObjects = [objectA, objectB];
        let objectAlpha =
            helpers.calculateDepthModifiers(this.userSettings, [
                objectA._id,
                objectB._id,
            ]) / 2;
        ...
        if (!path) {
            path = {
                id: pathID,
                carriers: Array(),
                graphics: this._createLoopedPathGraphics(
                    mapObjects[0],
                    mapObjects[1],
                    carrierMapObject.colour!,
                ),
            };
    ...
    removeSharedPath(pathID: string, carrier: CarrierData) {
        ...
        if (path) {
            ...
            path.graphics.alpha =
                helpers.calculateDepthModifier(this.userSettings, carrier._id) /
                    2 +
                path.carriers.length * 0.1;
    ...
    addUniquePath(
        mapObject: MapObjectData<string>,
        star: Star,
        looped: boolean,
        colour: string,
    ) {
        ...
        const objectAlpha =
            helpers.calculateDepthModifier(this.userSettings, mapObject._id) /
            2;
        ...
        if (looped) {
            ...
            path = this._createSolidPathGraphics(
                lineAlpha,
                lineWidth,
                mapObject,
                star,
    ...
    addPathToChunk(
        pathGraphics: GraphicsWithChunk,
        locA: Location,
        locB: Location,
    ) {
    ...
    setScale(zoomPercent: number, viewport, zoomChanging: boolean) {
        ...
        for (let ix = 0; ix < this.chunksXlen; ix += 1) {
            for (let iy = 0; iy < this.chunksYlen; iy += 1) {
                if (
                    ix >= firstX - this.firstChunkX &&
                    ix <= lastX - this.firstChunkX &&
                    iy >= firstY - this.firstChunkY &&
                    iy <= lastY - this.firstChunkY
                ) {
    ...
    _createLoopedPathGraphics(
        ...
        if (this.userSettings!.map.carrierLoopStyle == "solid") {
            return this._createSolidPathGraphics(
                lineAlpha,
                lineWidth / 3.0,
                objectA,
                objectB,
            ...
            return this._createDashedPathGraphics(
                lineAlpha,
                lineWidth,
                objectA,
                objectB,
    ...
    _createDashedPathGraphics(
        ...
        const DASH_LENGTH = Math.min(
            Math.max(1, this.userSettings!.map.carrierPathDashLength),
            16,
        );
        const VOID_LENGTH = DASH_LENGTH / 2.0;
        const COMBINED_LENGTH = DASH_LENGTH + VOID_LENGTH;

        const pathLength = this.distanceService.getDistanceBetweenLocations(
            pointA,
            pointB,
        );
        ...
        const endpointsLength =
            pathLength - dashCount * (DASH_LENGTH + VOID_LENGTH);
        ...
        for (let i = 0; i < dashCount; i++) {
            ...
            path.lineTo(
                initialX + i * COMBINED_LENGTH + DASH_LENGTH,
                -lineWidth,
            );
            path.lineTo(
                initialX + i * COMBINED_LENGTH + DASH_LENGTH,
        ...
        path.moveTo(
            Math.min(initialX + dashCount * COMBINED_LENGTH, pathLength),
            lineWidth,
        );
        path.lineTo(
            Math.min(initialX + dashCount * COMBINED_LENGTH, pathLength),
