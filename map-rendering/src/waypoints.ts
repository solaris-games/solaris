...
class Waypoints extends EventEmitter<keyof Events, Events> {
    ...
    _drawNextWaypoints() {
        ...
        for (let i = 0; i < this.game!.galaxy.stars.length; i++) {
            ...
            let distance = this.distanceService.getDistanceBetweenLocations(
                lastLocation,
                s.location,
            );
    ...
    _drawPaths() {
        ...
        for (let i = 0; i < this.carrier!.waypoints.length; i++) {
            ...
            const star = this.game!.galaxy.stars.find(
    ...
    _drawHyperspaceRange() {
        ...
        const radius =
            ((this.carrier!.effectiveTechs!.hyperspace || 1) + 1.5) *
            this.lightYearDistance!;

        const playerColour = this.context!.getPlayerColour(player!._id);

        graphics.star(
            lastLocationStar!.location.x,
            lastLocationStar!.location.y,
            radius,
            radius,
            radius - 3,
        );
    ...
    onStarClicked(e: StarData) {
        ...
        const lastLocationStar = this._getLastLocationStar();
        const lastLocation =
            lastLocationStar == null ? null : lastLocationStar.location;
        ...
        if (
            !canCreateWaypoint &&
            lastLocationStar &&
            lastLocationStar.wormHoleToStarId
        ) {
            const wormHolePairStar = helpers.getStarById(
                this.game!,
                lastLocationStar.wormHoleToStarId,
            );

            canCreateWaypoint = Boolean(
                wormHolePairStar && wormHolePairStar._id === e._id,
            );
        }
    ...
    _createWaypointRoute(sourceStarId: string, destinStarId: string) {
        const route = this.pathfindingService.calculateShortestRoute(
            this.game,
            helpers.getPlayerById(this.game, this.carrier!.ownedByPlayerId!)!,
            this.carrier!,
            sourceStarId,
            destinStarId,
        );
    ...
    _getLastLocationStar() {
        if (this.carrier!.waypoints.length) {
            const lastWaypointStarId =
                this.carrier!.waypoints[this.carrier!.waypoints.length - 1]
                    .destination;

            return this.game!.galaxy.stars.find(
                (s) => s._id === lastWaypointStarId,
            );
        } else {
            return this.game!.galaxy.stars.find(
                (s) => s._id === this.carrier!.orbiting,
            );
        }
    }
}
