...
export const createGameContainer = async (
    services: Services,
    drawingContext: DrawingContext,
    game: Game,
    userSettings: UserGameSettings | null,
...
export class GameContainer {
    ...
    subscribe() {
        ...
        }) => this.reloadGame(game, settings);
        const onStarReload = ({ star }: { star: Star }) =>
            this._reloadStar(star);
        const onCarrierReload = ({ carrier }: { carrier: Carrier }) =>
            this._reloadCarrier(carrier);
        const onCarrierRemove = ({ carrier }: { carrier: Carrier }) =>
            this.map.undrawCarrier(carrier);
        const onFitGalaxy = ({ location }: { location?: Location }) =>
            this._fitGalaxy(location?.x, location?.y);
        const zoomIn = () => this._zoomIn();
        const zoomOut = () => this._zoomOut();

        this.eventBus.on(
            GameCommandEventBusEventNames.GameCommandReloadGame,
            onGameReload,
        );
        this.eventBus.on(
            GameCommandEventBusEventNames.GameCommandReloadStar,
            onStarReload,
        );
        this.eventBus.on(
            GameCommandEventBusEventNames.GameCommandReloadCarrier,
            onCarrierReload,
        );
        this.eventBus.on(
            GameCommandEventBusEventNames.GameCommandRemoveCarrier,
            onCarrierRemove,
        );
        this.eventBus.on(
            MapCommandEventBusEventNames.MapCommandFitGalaxy,
            onFitGalaxy,
        );
        this.eventBus.on(MapCommandEventBusEventNames.MapCommandZoomIn, zoomIn);
        this.eventBus.on(
            MapCommandEventBusEventNames.MapCommandZoomOut,
            zoomOut,
        );
        ...
        this.unsubscribe = () => {
            ...
            this.eventBus.off(
                MapCommandEventBusEventNames.MapCommandZoomIn,
                zoomIn,
            );
            this.eventBus.off(
                MapCommandEventBusEventNames.MapCommandZoomOut,
    ...
    _setupViewport() {
        ...
        this.viewport.on(
            "pointerdown",
            this.map.onViewportPointerDown.bind(this.map),
        );
    }
    ...
    reloadGame(game: Game, userSettings: UserGameSettings) {
        ...
        if (
            userSettings?.technical?.performanceMonitor === "enabled" &&
            !this.debugTools
        ) {
            this.debugTools = new DebugTools(this.app!, this.map!);
            this.debugTools.draw();
    ...
    _reloadStar(star: Star) {
        const starObject = this.map.setupStar(
            this.game,
            this.userSettings,
            star,
        );
    ...
    _reloadCarrier(carrier: Carrier) {
        const carrierObject = this.map.setupCarrier(
            this.game,
            this.userSettings,
            carrier,
        );
