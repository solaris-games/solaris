...
export class Background {
    ...
    constructor(
        ...
        Background.zoomLevelDefinitions =
            userSettings.map.zoomLevels.background;
        ...
        this.timeScale =
            (1.0 / (2048.0 * 64.0)) *
            userSettings.map.background.nebulaMovementSpeed;
        this.blendMode =
            userSettings.map.background.blendMode == "ADD" ? "add" : "normal";
    }
    ...
    drawNebulas() {
        const NEBULA_FREQUENCY =
            this.userSettings.map.background.nebulaFrequency;
        ...
        try {
            NEBULA_COLOUR1 = this._getNumberFromHexString(
                this.userSettings!.map.background.nebulaColour1,
            );
            NEBULA_COLOUR2 = this._getNumberFromHexString(
                this.userSettings!.map.background.nebulaColour2,
            );
            NEBULA_COLOUR3 = this._getNumberFromHexString(
                this.userSettings!.map.background.nebulaColour3,
            );
        } catch (err) {
        ...
        for (let x = 0; x < chunksXlen; x += 1) {
            for (let y = 0; y < chunksYlen; y += 1) {
                if (chunks[x][y].length > MINIMUM_STARS) {
                    ...
                    const nebulaTextureCount =
                        TextureService.STARLESS_NEBULA_TEXTURES.length;
                    const textures = TextureService.STARLESS_NEBULA_TEXTURES;

                    if (
                        Math.round(this.rng.random() * 16) <= NEBULA_FREQUENCY
                    ) {
                        let nebulaCount = 0;
                        while (nebulaCount < NEBULA_DENSITY) {
                            ...
                            i = Math.round(
                                this.rng.random() * (nebulaTextureCount - 1),
                            );
                            const texture = textures[i];
                            const nebulaSprite = new Sprite(
                                texture,
                            ) as BackgroundSprite;
                            nebulaSprite.x =
                                x * CHUNK_SIZE +
                                firstChunkX * CHUNK_SIZE +
                                CHUNK_SIZE / 2.0;
                            nebulaSprite.x +=
                                NEBULA_MAX_OFFSET *
                                Math.round(this.rng.random() * 2.0 - 1.0);
                            nebulaSprite.y =
                                y * CHUNK_SIZE +
                                firstChunkY * CHUNK_SIZE +
                                CHUNK_SIZE / 2.0;
                            nebulaSprite.y +=
                                NEBULA_MAX_OFFSET *
                                Math.round(this.rng.random() * 2.0 - 1.0);
                            ...
                            nebulaSprite.parallax =
                                this.rng.random() * Background.MAX_PARALLAX;
                            nebulaSprite.blendMode = this.blendMode;

                            nebulaSprite.originX = nebulaSprite.x;
                            nebulaSprite.originY = nebulaSprite.y;
                            nebulaSprite.baseRotation =
                                this.rng.random() * Math.PI * 2.0;
                            nebulaSprite.baseRotationTime =
                                this.rng.random() * Math.PI * 2.0;
                            nebulaSprite.baseScaleTime =
                                this.rng.random() * Math.PI * 2.0;

                            nebulaSprite.tint = NEBULA_COLOUR1;
                            if (this.rng.random() > 1.0 / 3.0) {
                                nebulaSprite.tint = NEBULA_COLOUR2;
                            }
                            if (this.rng.random() > (1.0 / 3.0) * 2.0) {
                                nebulaSprite.tint = NEBULA_COLOUR3;
                            }
                            ...
                            while (starCount < Background.STAR_DENSITY) {
                                ...
                                const inverseParallaxNormalized =
                                    1.0 -
                                    starSprite.parallax /
                                        Background.MAX_PARALLAX;
                                starSprite.scale.x =
                                    (Background.STAR_SCALE +
                                        inverseParallaxNormalized *
                                            Background.STAR_SCALE) /
                                    2.0;
                                starSprite.scale.y =
                                    (Background.STAR_SCALE +
    ...
    refreshZoom(zoomPercent: number) {
        ...
        if (this.container) {
            this.container.visible =
                zoomPercent > Background.zoomLevelDefinitions.nebulas;
        }
        if (this.starContainer) {
            this.starContainer.visible =
                zoomPercent > Background.zoomLevelDefinitions.stars;
        }
    }
    ...
    onTick(deltaTime: number, viewportData) {
        ...
        for (let i = 0; i < this.container.children.length; i++) {
            ...
            if (this.moveNebulas) {
                // TODO compare performance of this conditional branch with looping the child array twice
                child.scale.x =
                    Background.NEBULA_SCALE +
                    Math.sin(child.baseScaleTime + compressedTime) *
                        Background.NEBULA_DELTA_SCALE;
                child.rotation =
                    child.baseRotation +
                    Math.sin(child.baseRotationTime + compressedTime) *
                        Background.NEBULA_DELTA_ROTATION;
            }
        }
