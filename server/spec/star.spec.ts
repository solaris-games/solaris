...
const fakeStarList = [{ _id: "aabbcc" }, { _id: "bbccdd" }, { _id: "ccddee" }];
...
describe("star", () => {
    ...
    it("should generate an unowned star", () => {
        ...
        const newStar = starService.generateUnownedStar(
            name,
            { x: 0, y: 0 },
            {
                economy: 10,
        ...
        expect(newStar.naturalResources.economy).toBeGreaterThanOrEqual(
            game.constants.star.resources.minNaturalResources,
        );
        expect(newStar.naturalResources.economy).toBeLessThanOrEqual(
            game.constants.star.resources.maxNaturalResources,
        );
        expect(newStar.naturalResources.industry).toBeGreaterThanOrEqual(
            game.constants.star.resources.minNaturalResources,
        );
        expect(newStar.naturalResources.industry).toBeLessThanOrEqual(
            game.constants.star.resources.maxNaturalResources,
        );
        expect(newStar.naturalResources.science).toBeGreaterThanOrEqual(
            game.constants.star.resources.minNaturalResources,
        );
        expect(newStar.naturalResources.science).toBeLessThanOrEqual(
            game.constants.star.resources.maxNaturalResources,
        );
    ...
    it("should setup a player's home star", () => {
        ...
        expect(homeStar.infrastructure.economy).toEqual(
            gameSettings.player.startingInfrastructure.economy,
        );
        expect(homeStar.infrastructure.industry).toEqual(
            gameSettings.player.startingInfrastructure.industry,
        );
        expect(homeStar.infrastructure.science).toEqual(
            gameSettings.player.startingInfrastructure.science,
        );
    });

    it("should find the index of a star in a sorted array", () => {
        expect(starService._binarySearchIndex(fakeStarList, "aabbcc")).toEqual(
            0,
        );
        expect(starService._binarySearchIndex(fakeStarList, "bbccdd")).toEqual(
            1,
        );
        expect(starService._binarySearchIndex(fakeStarList, "ccddee")).toEqual(
            2,
        );
        expect(starService._binarySearchIndex(fakeStarList, "dddeee")).toEqual(
            3,
        );
    });

    it("should find the index where a star should be in but not in the array", () => {
        expect(starService._binarySearchIndex(fakeStarList, "eeeeee")).toEqual(
            3,
        );
    ...
    it("should find the star in a array", () => {
        expect(starService.binarySearchStars(fakeStarList, "aabbcc")).toEqual(
            fakeStarList[0],
        );
        expect(starService.binarySearchStars(fakeStarList, "ccddee")).toEqual(
            fakeStarList[2],
        );
        expect(starService.binarySearchStars(fakeStarList, "eeeeee")).toEqual(
            undefined,
        );
    });
});
