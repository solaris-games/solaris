...
type ProcessingNotification =
    ...
    | ({
          kind: "playerGalacticCycleCompleted";
      } & InternalPlayerGalacticCycleCompleteEvent)
    | {
          kind: "playerResearchCompleted";
          gameId: DBObjectId;
          playerId: DBObjectId;
    ...
    | {
          kind: "playerCreditsSpecialistsReceived";
          gameId: DBObjectId;
          fromPlayer: Player;
          toPlayer: Player;
          amount: number;
    ...
    | ({
          kind: "conversationMessageSent";
      } & InternalConversationMessageSentEvent);
...
export class ProcessingNotificationService implements INotificationService {
    ...
    trySendLastPlayerTurnReminder(gameId: DBObjectId): Promise<void> {
        this._notifications.push({
            kind: "trySendLastPlayerTurnReminder",
            gameId,
        });
        return Promise.resolve();
    }
    ...
    onPlayerGalacticCycleCompleted(
        args: InternalPlayerGalacticCycleCompleteEvent,
    ): Promise<void> {
        this._notifications.push({
            kind: "playerGalacticCycleCompleted",
            ...args,
        ...
        for (const n of notifications) {
            switch (n.kind) {
                ...
                case "trySendLastPlayerTurnReminder":
                    await notificationService.trySendLastPlayerTurnReminder(
                        n.gameId,
                    );
                    break;
                case "playerGalacticCycleCompleted": {
                    const { kind, ...args } = n;
                    await notificationService.onPlayerGalacticCycleCompleted(
                        args,
                    );
                    break;
                }
