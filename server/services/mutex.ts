...
export default abstract class MutexService {
    ...
    protected async acquireMutexLocksInternal(
        ...futureMutexLocks: Partial<MutexLock>[]
    ): Promise<MutexLock[]>;
    protected async acquireMutexLocksInternal<TMutexLock extends MutexLock>(
        ...futureMutexLocks: Partial<MutexLock>[]
    ): Promise<TMutexLock[]>;
    protected async acquireMutexLocksInternal<TMutexLock extends MutexLock>(
        ...futureMutexLocks: Partial<MutexLock>[]
    ): Promise<TMutexLock[] | MutexLock[]> {
        ...
        await Promise.all(
            futureMutexLocks.map(async (futureMutexLock) => {
                // If duplicate keys are present, don't try and lock them twice!
                if (
                    mutexLocks.find((ml) => ml.key === futureMutexLock) == null
                ) {
                    futureMutexLock.mutexLockId = await this.getMutex(
                        futureMutexLock.key!,
                    ).wait();
    ...
    public async releaseMutexLocksInternal(
        ...mutexLocks: MutexLock[]
    ): Promise<void> {
        ...
        await Promise.all(
            mutexLocks.map((mutexLock) =>
                this.getMutex(mutexLock.key).release(mutexLock.mutexLockId!),
            ),
        );
