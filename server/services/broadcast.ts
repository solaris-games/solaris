...
export default class BroadcastService {
    constructor(
        private gameServerSocketEmitter: GameServerSocketEmitter,
    ...
    gamePlayerJoined(
        ...
        this.playerServerSocketEmitter.emitGamePlayerJoined(
            game._id.toString(),
            {
                playerId: playerId.toString(),
                alias,
                avatar: avatars.find(
                    (a) => a.id.toString() === avatar.toString(),
                )!.file,
            },
        );
    }
    ...
    gamePlayerConcededDefeat(game: Game, player: Player) {
        this.playerServerSocketEmitter.emitGamePlayerConcededDefeat(
            game._id.toString(),
            {
                playerId: player._id.toString(),
            },
    ...
    gamePlayerReady(game: Game, player: Player) {
        this.playerServerSocketEmitter.emitGamePlayerReady(
            game._id.toString(),
            {
                playerId: player._id.toString(),
            },
    ...
    gamePlayerNotReady(game: Game, player: Player) {
        this.playerServerSocketEmitter.emitGamePlayerNotReady(
            game._id.toString(),
            {
                playerId: player._id.toString(),
            },
    ...
    gamePlayerReadyToQuit(game: Game, player: Player | null) {
        this.playerServerSocketEmitter.emitGamePlayerReadyToQuit(
            game._id.toString(),
            {
                playerId: player?._id.toString(),
            },
    ...
    gamePlayerNotReadyToQuit(game: Game, player: Player | null) {
        this.playerServerSocketEmitter.emitGamePlayerNotReadyToQuit(
            game._id.toString(),
            {
                playerId: player?._id.toString(),
            },
    ...
    gameMessageSent(
        game: Game,
        message: ConversationMessageSentResult<DBObjectId>,
    ) {
        ...
        const toUserIds = game.galaxy.players
            .filter(
                (p) =>
                    message.toPlayerIds.find(
                        (m) => m.toString() === p._id.toString(),
                    ) != null,
            )
        ...
        toUserIds.forEach((u) =>
            this.userServerSocketEmitter.emitGameMessageSent(
                u.toString(),
                this.mapConversationMessageSentResultObjectIds(message),
            ),
        );
    }

    gameConversationRead(
        game: Game,
        conversation: Conversation<DBObjectId>,
        readByPlayerId: DBObjectId,
    ) {
        this.playerServerSocketEmitter.emitGameConversationRead(
            readByPlayerId.toString(),
            {
    ...
    gameConversationLeft(
        game: Game,
        conversation: Conversation<DBObjectId>,
        playerId: DBObjectId,
    ) {
        conversation.participants.forEach((p) =>
            this.playerServerSocketEmitter.emitGameConversationLeft(
                p.toString(),
                {
                    conversationId: conversation._id.toString(),
                    playerId: playerId.toString(),
    ...
    gameConversationMessagePinned(
        game: Game,
        conversation: Conversation<DBObjectId>,
        messageId: DBObjectId,
    ) {
        conversation.participants.forEach((p) =>
            this.playerServerSocketEmitter.emitGameConversationMessagePinned(
                p.toString(),
                {
                    conversationId: conversation._id.toString(),
    ...
    gameConversationMessageUnpinned(
        game: Game,
        conversation: Conversation<DBObjectId>,
        messageId: DBObjectId,
    ) {
        conversation.participants.forEach((p) =>
            this.playerServerSocketEmitter.emitGameConversationMessageUnpinned(
                p.toString(),
                {
                    conversationId: conversation._id.toString(),
                    messageId: messageId.toString(),
    ...
    playerEventRead(game: Game, playerId: DBObjectId, eventId: DBObjectId) {
        this.playerServerSocketEmitter.emitPlayerEventRead(
            playerId.toString(),
            {
    ...
    playerAllEventsRead(game: Game, playerId: DBObjectId) {
        this.playerServerSocketEmitter.emitPlayerAllEventsRead(
            playerId.toString(),
            {},
        );
    }

    gamePlayerCreditsReceived(
        game: Game,
        ...
        this.playerServerSocketEmitter.emitPlayerCreditsReceived(
            toPlayerId.toString(),
            {
    ...
    gamePlayerCreditsSpecialistsReceived(
        game: Game,
        fromPlayerId: DBObjectId,
        toPlayerId: DBObjectId,
        ...
        this.playerServerSocketEmitter.emitPlayerCreditsSpecialistsReceived(
            toPlayerId.toString(),
            {
                playerId: toPlayerId.toString(),
    ...
    gamePlayerRenownReceived(
        ...
        this.playerServerSocketEmitter.emitPlayerRenownReceived(
            toPlayerId.toString(),
            {
                playerId: toPlayerId.toString(),
    ...
    gamePlayerTechnologyReceived(
        ...
        this.playerServerSocketEmitter.emitPlayerTechnologyReceived(
            toPlayerId.toString(),
            {
                playerId: toPlayerId.toString(),
    ...
    gamePlayerDebtAdded(
        ...
        this.playerServerSocketEmitter.emitPlayerDebtAdded(
            debtorPlayerId.toString(),
            data,
        );
        this.playerServerSocketEmitter.emitPlayerDebtAdded(
            creditorPlayerId.toString(),
            data,
        );
    }

    gamePlayerDebtForgiven(
        debtorPlayerId: DBObjectId,
        creditorPlayerId: DBObjectId,
        amount: number,
        ledgerType: LedgerType,
        ...
        this.playerServerSocketEmitter.emitPlayerDebtForgiven(
            debtorPlayerId.toString(),
            data,
        );
        this.playerServerSocketEmitter.emitPlayerDebtForgiven(
    ...
    gamePlayerDebtSettled(
        ...
        this.playerServerSocketEmitter.emitPlayerDebtSettled(
            debtorPlayerId.toString(),
            data,
        );
        this.playerServerSocketEmitter.emitPlayerDebtSettled(
    ...
    gamePlayerDiplomaticStatusChanged(
        playerIdFrom: DBObjectId,
        playerIdTo: DBObjectId,
        diplomaticStatus: DiplomaticStatus<DBObjectId>,
    ) {
        let data = {
            diplomaticStatus:
                this.mapDiplomaticStatusObjectIds(diplomaticStatus),
        };

        this.diplomacyServerSocketEmitter.emitPlayerDiplomaticStatusChanged(
            playerIdFrom.toString(),
            data,
        );
        this.diplomacyServerSocketEmitter.emitPlayerDiplomaticStatusChanged(
            playerIdTo.toString(),
    ...
    private mapGameStateObjectIds(
        gameState: GameState<DBObjectId>,
    ): GameState<string> {
        const { winner, winningTeam, leaderboard, teamLeaderboard, ...gs } =
            gameState;
        ...
        newGameState.leaderboard =
            leaderboard?.map((id) => id.toString()) ?? null;
        newGameState.teamLeaderboard =
            teamLeaderboard?.map((id) => id.toString()) ?? null;

        return newGameState;
    }

    mapConversationMessageSentResultObjectIds(
        conversationMessageSentResult: ConversationMessageSentResult<DBObjectId>,
