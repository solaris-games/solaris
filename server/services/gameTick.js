...
module.exports = class GameTickService extends EventEmitter {
		...
		async _combatCarriers(game, gameUsers) {
				...
				// use a map to store the location of all stars where a carrier is moving to or from.
				let starMap = new Map();
				...
				let carrierPositions = game.galaxy.carriers
						...
						.map(c => {
								...
								if (starMap.has(waypoint.source)) {
										sourceStar = starMap.get(waypoint.source);
								} else {
										sourceStar = this.galaxyService.getStar(game, waypoint.source);
										starMap.set(waypoint.source, sourceStar);
								}
								if (starMap.has(waypoint.destination)) {
										destinationStar = starMap.get(waypoint.destination);
								} else {
										destinationStar = this.galaxyService.getStar(game, waypoint.destination);
										starMap.set(waypoint.destination, destinationStar);
								}
