...
module.exports = class GameTickService extends EventEmitter {
    ...
    async _combatCarriers(game, gameUsers) {
        ...
        // use a map to store the location of stars where carriers are moving to or from.
        ...
        let carrierPositions = game.galaxy.carriers
            ...
            .map(c => {
                ...
                if (starMap.has(waypoint.source.toString())) {
                    sourceStar = starMap.get(waypoint.source.toString());
                } else {
                    sourceStar = this.galaxyService.getStarById(game, waypoint.source.toString());
                    starMap.set(waypoint.source.toString(), sourceStar);
                }
                if (starMap.has(waypoint.destination.toString())) {
                    destinationStar = starMap.get(waypoint.destination.toString());
                } else {
                    destinationStar = this.galaxyService.getStarById(game, waypoint.destination.toString());
                    starMap.set(waypoint.destination.toString(), destinationStar);
                }
