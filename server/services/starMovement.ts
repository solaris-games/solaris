...
export default class starMovementService {
    ...
    orbitObject(game: Game, objectWithLocation: Star | Carrier) {
        objectWithLocation.location = this.getNextLocation(
            game,
            objectWithLocation,
        );
    }
    ...
    getNextLocation(game: Game, objectWithLocation: Star | Carrier) {
        ...
        let r = Math.hypot(
            galaxyCenter.x - objectWithLocation.location.x,
            galaxyCenter.y - objectWithLocation.location.y,
        );
    ...
    moveStellarEngines(game: Game) {
        const beaconStars = game.galaxy.stars.filter((s) =>
            this.specialistService.getStarAttract(s),
        );
        ...
        const engineStars = game.galaxy.stars.filter((s) =>
            this.specialistService.getStarMovement(s),
        );

        for (let star of engineStars) {
            let closestStar = this.starDistanceService.getClosestStar(
                star,
                beaconStars,
            );
            let distanceToClosestStar =
                this.starDistanceService.getDistanceBetweenStars(
                    star,
                    closestStar,
                );
            let starSpeed =
                this.specialistService.getStarMovementPerTick(star) *
                game.settings.specialGalaxy.carrierSpeed;

            // This line makes sure the Stellar Engine never moves too close to the target star
            starSpeed =
                distanceToClosestStar - starSpeed <=
                0.5 * game.constants.distances.minDistanceBetweenStars
                    ? distanceToClosestStar -
                      0.5 * game.constants.distances.minDistanceBetweenStars
                    : starSpeed;

            this.moveStarTowardsLocation(
                game,
                star,
        ...
        const nonEngineStars = game.galaxy.stars.filter(
            (s) => !this.specialistService.getStarMovement(s),
        );
    ...
    maintainDistance(game: Game, movedStars: Star[], stars: Star[]) {
        for (let star of movedStars) {
            let nearbyStars =
                this.starDistanceService.getStarsWithinRadiusOfStar(
                    star,
                    stars,
                    game.constants.distances.minDistanceBetweenStars,
                );
            let tooCloseStars =
                this.starDistanceService.getStarsWithinRadiusOfStar(
                    star,
                    stars,
                    0.49 * game.constants.distances.minDistanceBetweenStars,
                );
    ...
    shiftAway(
        ...
        this.moveStarTowardsLocation(
            game,
            movingStar,
            stationaryStar.location,
            shift,
        );
    }

    moveStarTowardsLocation(
        game: Game,
        star: Star,
        location: Location,
        speed: number,
        ...
        const carriersInOrbit = game.galaxy.carriers.filter(
            (c) => c.orbiting && c.orbiting.toString() === star._id.toString(),
        );
