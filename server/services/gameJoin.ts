...
export default class GameJoinService extends EventEmitter {
    ...
    async join(game: Game, userId: DBObjectId, playerId: DBObjectId | undefined, alias: string, avatar: number, password: string | undefined, eventService: IEventService, notificationService: INotificationService, emailService: EmailService): Promise<{ gameIsFull: boolean, playerId: DBObjectId }> {
    ...
    startGame(game: Game) {
        ...
        if (this.gameTypeService.isRealTimeGame(game)) {
            ...
            startDate = startDate.plus({ minutes: game.settings.gameTime.startDelay });
        }
