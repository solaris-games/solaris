...
export default class ShipTransferService {
    ...
    async garrisonAllShips(
        ...
        let carriersAtStar = this.carrierService
        ...
        if (!carriersAtStar.length) {
            throw new ValidationError(
                "The player does not own any carriers in orbit of this star.",
            );
        }
    ...
    async distributeAllShips(
        ...
        if (star.ships > 0) {
            ...
            let shipsPerCarrier = Math.floor(
                star.ships / carriersAtStar.length,
            );
    ...
    async transfer(
        ...
        if (!carrier.orbiting) {
            throw new ValidationError(
                "The carrier must be in orbit of a star to transfer ships.",
            );
        }

        if (carrier.orbiting.toString() !== star._id.toString()) {
            throw new ValidationError(
                "The carrier must be in orbit of a the desired star to transfer ships.",
            );
        }
        ...
        if (totalTransferShips != totalShips) {
            throw new ValidationError(
                "The total number of ships in the transfer does not equal to the total number of ships garrisoned",
            );
        }

        if (carrierShips <= 0) {
            throw new ValidationError(
                "The number of carrier ships in the transfer must be greater than 0. Carriers must have at least 1 ship.",
            );
        }

        if (starShips < 0) {
            throw new ValidationError(
                "The number of carrier ships in the transfer must be 0 or greater.",
            );
        }

        if (
            carrierShips !== parseInt(carrierShips.toString()) ||
            starShips !== parseInt(starShips.toString())
        ) {
            throw new ValidationError(
                "The number of ships in the transfer must be a whole number.",
            );
        }
