...
export default class GameListService {
    ...
    async listJoinableGames() {
        ...
        const official = games.filter(
            (g) => g.settings.general.type !== "custom",
        );
        const custom = games.filter(
            (g) => g.settings.general.type === "custom",
        );
    ...
    async listActiveGames(userId: DBObjectId) {
        ...
        return await Promise.all(
            games.map(async (game) => {
                game.userNotifications = await this.getUserPlayerNotifications(
                    game,
                    userId,
                    true,
                    true,
                ...
                $and: [
                    {
                        "galaxy.players.spectators": {
                            // User is spectating at least one player.
                            $elemMatch: {
                                $in: [userId],
                            },
                        },
    ...
    async getUserPlayerNotifications(
        game: Game,
        userId: DBObjectId,
        ...
        if (player) {
            if (includeUnreadConversastions)
                unreadConversations = this.conversationService.getUnreadCount(
                    game,
                    player._id,
                );
            if (includeUnreadEvents)
                unreadEvents = await this.eventService.getUnreadCount(
                    game,
                    player._id,
                );
            if (
                includeTurnWaiting &&
                this.gameTypeService.isTurnBasedGame(game)
            ) {
                turnWaiting = !player.ready;
                const undefeatedPlayers = game.galaxy.players.filter(
                    (p) => !p.defeated,
                );
                turnReadyCount = undefeatedPlayers.filter(
                    (p) => p.ready,
                ).length;
            ...
            if (includePosition) {
                if (this.gameTypeService.isTeamConquestGame(game)) {
                    position =
                        this.leaderboardService.getTeamLeaderboardPosition(
                            game,
                            player,
                        );
                } else {
                    position =
                        this.leaderboardService.getGameLeaderboardPosition(
                            game,
                            player,
                        );
                }
    ...
    async listInProgressGames() {
        ...
        for (let game of games) {
            game.state.openSlots = game.galaxy.players.filter(
                (p) => p.isOpenSlot,
            ).length;
