...
export default class CarrierService extends EventEmitter {
    ...
    getByIdBS(game: Game, id: DBObjectId) {
        ...
        // id wasn't found
        // Return the old way
        return game.galaxy.carriers.find(
    ...
    getCarriersAtStar(game: Game, starId: DBObjectId) {
        return game.galaxy.carriers.filter(
            (carrier) =>
                carrier.orbiting &&
                carrier.orbiting.toString() === starId.toString(),
        );
    }

    getCarrierLimit(game: Game, player: Player) {
        const starCount = game.galaxy.stars.filter(
            (star) =>
    ...
    createAtStar(star: Star, carriers: Carrier[], ships: number = 1): Carrier {
        if (!Math.floor(star.shipsActual!)) {
            throw new ValidationError(
                "Star must have at least 1 ship to build a carrier.",
            );
        }
        ...
        if (star.specialistId) {
            ...
            if (
                ...
                carrier.specialistId =
                    starSpecialist.modifiers.special!.autoCarrierSpecialistAssign!;
                carrier.specialist = this.specialistService.getByIdCarrier(
                    carrier.specialistId,
                );
            }
        }
    ...
    listCarriersOwnedByPlayerInOrbit(
        carriers: Carrier[],
        playerId: DBObjectId,
    ) {
        return this.listCarriersOwnedByPlayer(carriers, playerId).filter(
            (c) => c.orbiting,
        );
    }

    listCarriersOwnedByPlayersInOrbit(
        carriers: Carrier[],
        playerIds: DBObjectId[],
    ) {
        return this.listCarriersOwnedByPlayers(carriers, playerIds).filter(
            (c) => c.orbiting,
        );
    }
    ...
    sanitizeCarriersByPlayers(game: Game, playerIds: DBObjectId[]) {
        ...
        return game.galaxy.carriers.map((c) => {
            ...
            carrierData.waypoints = this.clearCarrierWaypointsNonTransit(
                c,
                true,
            );
    ...
    clearCarrierWaypointsNonTransit(
        carrier: Carrier,
        obfuscateFirstWaypoint: boolean = false,
    ) {
    ...
    clearPlayerCarrierWaypointsNonTransit(game: Game, player: Player) {
        let carriers = this.listCarriersOwnedByPlayer(
    ...
    clearPlayerCarriers(game: Game, player: Player) {
        game.galaxy.carriers = game.galaxy.carriers.filter(
            (c) =>
                !c.ownedByPlayerId ||
    ...
    async rename(
        ...
        if (carrier.ownedByPlayerId!.toString() !== player._id.toString()) {
            throw new ValidationError(
                `Cannot rename carrier, you are not its owner.`,
            );
        }
    ...
    async scuttle(game: Game, player: Player, carrierId: DBObjectId) {
        ...
        if (carrier.ownedByPlayerId!.toString() !== player._id.toString()) {
            throw new ValidationError(
                `Cannot scuttle carrier, you are not its owner.`,
            );
        }

        if (carrier.isScuttled) {
            throw new ValidationError(
                `Cannot scuttle carrier that is already scuttled.`,
            );
        }
    ...
    listGiftCarriersInOrbit(game: Game) {
        return game.galaxy.carriers.filter(
            (c) => c.isGift && c.orbiting && c.waypoints.length === 0,
        );
    }
}
