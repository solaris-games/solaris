...
export default class SaveWaypointsService {
    ...
    async saveWaypointsForCarrier(
        game: Game,
        player: Player,
        carrier: Carrier,
        waypoints: CarrierWaypointBase<DBObjectId>[],
        looped: boolean | null,
        ...
        if (carrier.isScuttled) {
            throw new ValidationError(
                `Cannot alter waypoints for a scuttled carrier`,
            );
        }

        if (carrier.isGift) {
            throw new ValidationError(
                "Cannot alter waypoints for a gifted carrier.",
            );
        }
        ...
        if (!carrier.orbiting) {
            ...
            if (
                !newFirstWaypoint ||
                currentWaypoint.source.toString() !==
                    newFirstWaypoint.source.toString() ||
                currentWaypoint.destination.toString() !==
                    newFirstWaypoint.destination.toString()
            ) {
                throw new ValidationError(
                    "The first waypoint course cannot be changed mid-flight.",
                );
            }

            if (+newFirstWaypoint.delayTicks) {
                throw new ValidationError(
                    "The first waypoint cannot have delay ticks if mid-flight.",
                );
            }
        } else if (waypoints?.length > 0) {
            if (
                carrier.orbiting.toString() !== waypoints[0].source.toString()
            ) {
                throw new ValidationError(
                    "The source star does not match the carrier location.",
                );
            }
        }
        ...
        if (this.gameTypeService.isDarkMode(game)) {
            ...
            if (
                waypoints.length >
                playerStars.length * ScanningService.SINGLE_TREE_COST_FACTOR
            ) {
                ...
                scannedStarSet = this.scanningService.getStarSetByScanningRange(
                    game,
                    [player],
                    kdTree,
                );
                ...
                treesWithRadius = this.scanningService.getScanningStarTrees(
                    game,
                    playerStars,
                );
        ...
        for (let i = 0; i < waypoints.length; i++) {
            ...
            const destinationStar = this.starService.getById(
                game,
            ...
            if (i === 0) {
                ...
                if (
                    previous.destination.toString() !==
                    waypoint.source.toString()
                ) {
                    throw new ValidationError(
                        `The waypoint source star does not match the previous waypoint destination star.`,
                    );
                }
            }

            const sourceStarName =
                sourceStar == null ? "Unknown" : sourceStar.name; // Could be travelling from a destroyed star.
            ...
            if (
                waypoint.actionShips == null ||
                (waypoint.actionShips as any) == "" ||
                +waypoint.actionShips < 0 ||
                !this.waypointService.supportsActionShips(waypoint.action)
            ) {
                waypoint.actionShips = 0;
            }
            ...
            if (+waypoint.actionShips < 0) {
                throw new ValidationError(
                    `The waypoint action ships cannot be less than 0.`,
                );
            }

            if (
                +waypoint.actionShips !==
                parseInt(waypoint.actionShips.toString())
            ) {
                throw new ValidationError(
                    `The waypoint action ships value must be a whole number.`,
                );
            }
            ...
            if (
                waypoint.delayTicks == null ||
                (waypoint.delayTicks as any) == "" ||
            ...
            if (+waypoint.delayTicks % 1 != 0) {
                throw new ValidationError(
                    `The waypoint ${sourceStarName} -> ${destinationStar.name} delay cannot be a decimal.`,
                );
            }
            ...
            if (i > 0 || (i === 0 && Boolean(carrier.orbiting))) {
                // Is one of the next waypoints OR is the first waypoint and isn't in transit
                if (
                    sourceStar &&
                    !this._waypointRouteIsBetweenWormHoles(game, waypoint) &&
                    ...
                    // Validation of whether the waypoint is within hyperspace range
                    throw new ValidationError(
                        `The waypoint ${sourceStarName} -> ${destinationStar.name} exceeds hyperspace range.`,
                    );
                }
                ...
                switch (scanningCheck) {
                    case "singleTree":
                        if (
                            ...
                            throw new ValidationError(
                                `The waypoint destination ${destinationStar.name} is not within scanning range.`,
                            );
                        }
                        break;
                    case "multiTree":
                        if (
                            !this.scanningService.isStarWithinScanningRangeOfStars(
                                game,
                                destinationStar,
                                treesWithRadius!,
                            )
        ...
        if (looped && !this.waypointService.canLoop(game, carrier)) {
            throw new ValidationError(
                `The carrier waypoints cannot be looped.`,
            );
        }
        ...
        const reportCarrier = Boolean(carrier.toObject)
            ? carrier.toObject()
            : carrier;
    ...
    async saveWaypoints(
        ...
        if (!carrier) {
            throw new ValidationError(
                `Could not find carrier with id ${carrierId}`,
            );
        }

        return await this.saveWaypointsForCarrier(
            game,
            player,
            carrier,
            waypoints,
    ...
    async loopWaypoints(
        ...
        if (carrier.isGift) {
            throw new ValidationError(
                "Cannot loop waypoints of a carrier that is a gift.",
            );
        }
        ...
        if (loop) {
            if (carrier.waypoints.length < 1) {
                throw new ValidationError(
                    "The carrier must have 2 or more waypoints to loop",
                );
            }

            if (!this.waypointService.canLoop(game, carrier)) {
                throw new ValidationError(
                    "The last waypoint star is out of hyperspace range of the first waypoint star.",
                );
            }
        }
    ...
    _waypointRouteIsBetweenWormHoles(
        game: Game,
        waypoint: CarrierWaypointBase<DBObjectId>,
    ) {
        const sourceStar = this.starService.getById(game, waypoint.source);
        const destinationStar = this.starService.getById(
            game,
        ...
        return this.starDataService.isStarPairWormHole(
            sourceStar,
            destinationStar,
        );
    }
