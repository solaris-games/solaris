module.exports = class ResourceService {
    ...
    distribute(game, locations, resourceDistribution) {
        ...
        if(game.settings.specialGalaxy.splitResources && game.settings.specialGalaxy.splitResources == 'enabled') {
            if (resourceDistribution !== 'random' && !forcedRandom) {
                return this._distributeSplitWeightedCenter(game, locations);
            }
    
            // In all other cases, random.
            return this._distributeSplitRandom(game, locations);
        }
    ...
    // Start of the split resource generation

    _distributeSplitRandom(game, locations) {
        // Allocate random resources.
        let minResources = game.constants.star.resources.minNaturalResources;
        let maxResources = game.constants.star.resources.maxNaturalResources;

        if (game.settings.galaxy.galaxyType === 'circular-balanced') {
            this._distributeSplitRandomMirrored(game, locations, minResources, maxResources);
        } else {
            this._distributeSplitRandomAny(game, locations, minResources, maxResources);
        }
    }

    _distributeSplitRandomMirrored(game, locations, minResources, maxResources) {
        let playerCount = game.settings.general.playerLimit;

        for (let i = 0; i < locations.length / playerCount; i++) {
            let economy = this.randomService.getRandomNumberBetween(minResources, maxResources);
            let industry = this.randomService.getRandomNumberBetween(minResources, maxResources);
            let science = this.randomService.getRandomNumberBetween(minResources, maxResources);
            let total = economy + industry + science;

            for (let j = 0; j < playerCount; j++) {
                locations[i*playerCount+j].resources = {
                    total,
                    economy,
                    industry,
                    science
    ...
    _distributeSplitRandomAny(game, locations, minResources, maxResources) {
        for (let location of locations) {
            let economy = this.randomService.getRandomNumberBetween(minResources, maxResources);
            let industry = this.randomService.getRandomNumberBetween(minResources, maxResources);
            let science = this.randomService.getRandomNumberBetween(minResources, maxResources);
    ...
    _distributeSplitWeightedCenter(game, locations) {
        // The closer to the center of the galaxy, the more likely (exponentially) to find stars with higher resources.
        let minResources = game.constants.star.resources.minNaturalResources;
        let maxResources = game.constants.star.resources.maxNaturalResources;
        let galaxyRadius = this.starDistanceService.getMaxGalaxyDiameter(locations) / 2;
        ...
        if (game.settings.galaxy.galaxyType == 'circular-balanced') {
            this._distributeSplitWeightedCenterMirrored(game, locations, minResources, maxResources, galaxyRadius, galacticCenter);
        } else {
            this._distributeSplitWeightedCenterAny(game, locations, minResources, maxResources, galaxyRadius, galacticCenter);
        }
    }

    _distributeSplitWeightedCenterMirrored(game, locations, minResources, maxResources, galaxyRadius, galacticCenter) {
        let playerCount = game.settings.general.playerLimit;
        ...
        for (let i = 0; i < locations.length / playerCount; i++) {
            ...
            // If you want the differences to be more extreme you can increase the 0.6 and decrease the 0.2 notice how: 1 - 0.6 = 2 * 0.2, keep that relation intact.
            // So for example a good tweak to make the center even stronger and the edges weaker would be to pick * 0.8 + 0.1, and notice again how 1 - 0.8 = 2 * 0.1
            let economy = this.randomService.getRandomNumberBetweenEXP(minResources, maxResources, radius/galaxyRadius * 0.6 + 0.2);;
            let industry = this.randomService.getRandomNumberBetweenEXP(minResources, maxResources, radius/galaxyRadius * 0.6 + 0.2);
            let science = this.randomService.getRandomNumberBetweenEXP(minResources, maxResources, radius/galaxyRadius * 0.6 + 0.2);
            let total = economy + industry + science;
    ...
    _distributeSplitWeightedCenterAny(game, locations, minResources, maxResources, galaxyRadius, galacticCenter) {
        for (let location of locations) {
            let radius = this.distanceService.getDistanceBetweenLocations(galacticCenter, location);
