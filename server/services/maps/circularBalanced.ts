...
export default class CircularBalancedMapService {
    ...
    _moveLocationTowards(location, towards, minDistance: number) {
        ...
        let dist = this.distanceService.getDistanceBetweenLocations(
            location,
            towards,
        );
        if (dist < minDistance) {
    ...
    generateLocations(
        ...
        if (this.gameTypeService.isKingOfTheHillMode(game)) {
            throw new ValidationError(
                `King of the hill is not supported in circular balanced maps.`,
            );
        }
        ...
        do {
            ...
            for (let i = 0; i < maxTries; i++) {
                ...
                let baseLocation = this._generateStarPositionInSector(
                    currentRadius,
                    rng,
                    playerCount,
                );
                ...
                for (
                    let sectorIndex = 0;
                    sectorIndex < playerCount;
                    sectorIndex++
                ) {
                    let location = this._getRotatedLocation(
                        baseLocation,
                    ...
                    if (
                        this.isLocationTooCloseToOthers(
                            game,
                            location,
                            locations,
                        ) ||
        ...
        let desiredLocation = this._getRotatedLocation(
            { x: 0.0, y: distanceFromCenter },
            playerAngle,
        );
        let firstHomeLocation = this.distanceService.getClosestLocation(
            desiredLocation,
            locations,
        );
        ...
        while (startingStarsCount--) {
            for (let homeLocation of homeLocations) {
                let closestUnlinkedLocation =
                    this.distanceService.getClosestLocation(
                        homeLocation,
                        unlinkedLocations,
                    ) as any;
        ...
        let minimumClaimDistance =
            this.distanceService.getHyperspaceDistance(
                game,
                initialHyperRange,
            ) - 2; //-2 to avoid floating point imprecisions
        ...
        for (let homeLocation of homeLocations) {
            ...
            while (unreachebleLocations.length > 0) {
                ...
                for (let unreachebleLocation of unreachebleLocations) {
                    ...
                    unreachebleLocation.distanceToClosestReachable =
                        distanceToClosestReachable;
                    unreachebleLocation.closestReachable =
                ...
                for (let unreachebleLocation of unreachebleLocations) {
                    if (
                        unreachebleLocation.distanceToClosestReachable! <
                        closestUnreachable.distanceToClosestReachable!
                    ) {
                        closestUnreachable = unreachebleLocation;
                    }
                }

                this._moveLocationTowards(
                    closestUnreachable,
                    closestUnreachable.closestReachable!,
                    minimumClaimDistance,
                );

                // after moving closer we can change the location from the unreachable to the reachable array
                unreachebleLocations.splice(
                    unreachebleLocations.indexOf(closestUnreachable),
                    1,
                );
