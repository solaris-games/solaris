...
export default class SpiralMapService {
    ...
    generateSpiral(
        ...
        do {
            ...
            let angle =
                (current_branch / BRANCHES) * 2 * Math.PI +
    ...
    applyQuadraticStretch(locations: Location[]): void {
        ...
        for (let i = 0; i < locations.length; i++) {
            ...
            let x =
                x_init *
                Math.pow(X_BASE, X_EXP * Math.pow(vectorScale, X_EXP2));
            let y =
                y_init *
                Math.pow(Y_BASE, Y_EXP * Math.pow(vectorScale, Y_EXP2));
        ...
        for (let i = 0; i < ITERATION_COUNT; i++) {
            for (var s_i = 0; s_i < locations.length; s_i++) {
                ...
                for (let d_i = 0; d_i < DISTANCE_CHECKING_DIRECTIONS; d_i++) {
                    ...
                    let val_at_offset = simplex(
                        s_x + av_x * DELTA_DISTANCE,
                        s_y + av_y * DELTA_DISTANCE,
                    );
            ...
            for (let i1 = 0; i1 < locations.length; i1++) {
                ...
                for (let i2 = i1 + 1; i2 < locations.length; i2++) {
                    ...
                    if (distance < MIN_D) {
                        ...
                        let x_repos =
                            cos * Math.min(MAX_REPOS, REPOS / distance);
                        let y_repos =
                            sin * Math.min(MAX_REPOS, REPOS / distance);
    ...
    scaleUp(game, locations: Location[]): Location[] {
        ...
        do {
            ...
            for (let i = 0; i < locs.length; i++) {
                let location = locs[i];
                let size =
                    (location.resources.economy +
                        location.resources.industry +
                        location.resources.science) /
                    3;
    ...
    isValidGalaxyCheck(game, locations: Location[]): boolean {
        ...
        let average =
            locations.reduce(
                (sum, l) =>
                    sum +
                    this.distanceService.getDistanceToClosestLocation(
                        l,
                        locations,
                    ),
                0,
