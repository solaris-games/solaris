...
export default class IrregularMapService {
    ...
    _generateHomeLocations(
        ...
        while (homeLocations.length < playerCount) {
            ...
            while (!positionIsValid) {
                let baseLocation =
                    homeLocations[
                        rand.getRandomNumber(homeLocations.length - 1)
                    ];
                ...
                position.noiseIntensity = simplexNoiseGenerator(
                    position.x / noiseSpread,
                    position.y / noiseSpread,
                );
    ...
    _generateSupplementaryHomeLocations(
        pivotDistance: number,
    ...
    _generateConcentricHexRingsLocations(
        ...
        for (let ringIndex = 0; ringIndex < ringCount; ringIndex++) {
            for (let sliceIndex = 0; sliceIndex < 6; sliceIndex++) {
                if (ringIndex == ringCount - 1 && sliceIndex < 3) {
                    continue;
                } //only create the first 3 edges of the outer ring
                ...
                for (let i = 0; i < ringIndex; i++) {
                    ...
                    edgePosition = this._rotatedLocation(
                        edgePosition,
                        edgeRotation,
                    );
                    edgePosition = this._displacedLocation(
                        position,
                        edgePosition,
                    );
    ...
    _randomlyDislocateLocations(
        ...
        for (let location of locations) {
            let amount =
                3.0 * (threshold / 4.0) + (rand.random() * threshold) / 4.0; // 0.75 to 1.0 times the threshold
    ...
    _pruneLocationsWithNoise(
        ...
        for (let location of locations) {
            (location as any).noiseIntensity = simplexNoiseGenerator(
                location.x / noiseSpread,
                location.y / noiseSpread,
            );
        }
    ...
    _pruneLocationsOutsideMetaball(
        locations: Location[],
        homeLocations: Location[],
    ...
    generateLocations(
        ...
        playerCount: number,
        customSeed: string | null | undefined,
    ): Location[] {
        if (this.gameTypeService.isKingOfTheHillMode(game)) {
            throw new ValidationError(
                `King of the hill is not supported in irregular maps.`,
            );
        }
        ...
        const MINIMUM_STAR_DISTANCE =
            game.constants.distances.minDistanceBetweenStars * 0.75; // TODO: This is a bit of a bodge to ensure that stars do not spawn too far away from players.

        const NOISE_SPREAD =
            NOISE_BASE_SPREAD * ((STARS_PER_PLAYER + 20) / 9.0);
        ...
        const RING_COUNT = this._getRingCount(
            STARS_PER_PLAYER,
            STARS_PER_PLAYER * STAR_COUNT_MULTIPLIER,
        );
        const STAR_DISTANCE = MINIMUM_STAR_DISTANCE * SPREAD;
        const STAR_DISLOCATION_THRESHOLD =
            MINIMUM_STAR_DISTANCE * ((SPREAD - 1.0) / 2.0);
        const PIVOT_DISTANCE = RING_COUNT * STAR_DISTANCE;

        let locations: Location[] = [];
        let homeLocations = this._generateHomeLocations(
            PIVOT_DISTANCE,
            playerCount,
            rand,
            SIMPLEX_NOISE,
        ...
        let supplementaryHomeLocations =
            this._generateSupplementaryHomeLocations(
                PIVOT_DISTANCE,
                homeLocations,
            );
        let baseLocations = [];
        ...
        for (let homeLocation of homeLocations) {
            this._generateConcentricHexRingsLocations(
                homeLocation,
                RING_COUNT,
                STAR_DISTANCE,
                baseLocations,
        ...
        this._pruneLocationsOutsideMetaball(
            locations,
            homeLocations,
            PIVOT_DISTANCE,
            rand,
        );
        this._randomlyDislocateLocations(
            locations,
            STAR_DISLOCATION_THRESHOLD,
            rand,
        );
        this._pruneLocationsWithNoise(
            locations,
            starCount - playerCount,
            SIMPLEX_NOISE,
            NOISE_SPREAD,
        );
        ...
        while (startingStarsCount--) {
            for (let homeLocation of homeLocations) {
                ...
                (homeLocation as any).linkedLocations.push(
                    closestUnlinkedLocation,
                );
        ...
        for (let homeLocation of homeLocations) {
            ...
            while (unreachebleLocations.length > 0) {
                ...
                for (let unreachebleLocation of unreachebleLocations) {
                    ...
                    (unreachebleLocation as any).distanceToClosestReachable =
                        distanceToClosestReachable;
                    (unreachebleLocation as any).closestReachable =
                ...
                for (let unreachebleLocation of unreachebleLocations) {
                    if (
                        (unreachebleLocation as any)
                            .distanceToClosestReachable <
                        (closestUnreachable as any).distanceToClosestReachable
                    ) {
                ...
                this._moveLocationTowards(
                    closestUnreachable,
                    (closestUnreachable as any).closestReachable,
