...
export default class ScanningService {
    ...
    starSetToSortedList(starSet: Set<Star>) {
        const stars = Array.from(starSet).sort((a, b) =>
    ...
    getStarSetByScanningRange(game: Game, players: Player[], kdTree: KDTree) {
        ...
        const starsOwnedOrInOrbit =
            this.starService.listStarsOwnedOrInOrbitByPlayers(
                game,
                players.map((p) => p._id),
            );
        ...
        for (const star of starsWithScanning) {
            this.getStarsWithinScanningRangeOfStar(game, star, kdTree).forEach(
                (s) => starsInRange.add(s),
            );
        }
        kdTree.resetMarked();
        ...
        starsOwnedOrInOrbit
            ...
            .map((s) => {
                return {
                    source: s,
                    destination: this.starService.getById(
                        game,
                        s.wormHoleToStarId!,
                    ),
                };
    ...
    getCarrierSetByScanningRange(
        ...
        for (const star of starsWithScanning) {
            this.getCarriersWithinScanningRangeOfStar(
                game,
                star,
                kdTree,
            ).forEach((c) => carriersInRange.add(c));
        }
        kdTree.resetMarked();
    ...
    getStarsWithinScanningRangeOfStar(
        game: Game,
        star: Star,
        kdTree: KDTree,
    ): Star[] {
        return this._getObjectsWithinScanningRangeOfStar(
            game,
            star,
            kdTree,
        ).map((i) => game.galaxy.stars[i]);
    }

    // Requires manually calling resetMarked() after
    getCarriersWithinScanningRangeOfStar(
        game: Game,
        star: Star,
        kdTree: KDTree,
    ): Carrier[] {
        return this._getObjectsWithinScanningRangeOfStar(
            ...
            star,
            kdTree,
        ).map((i) => game.galaxy.carriers[i]);
    }

    _getObjectsWithinScanningRangeOfStar(
        game: Game,
        star: Star,
        kdTree: KDTree,
    ): number[] {
        ...
        let scanningRangeDistance = this.distanceService.getScanningDistance(
            game,
            effectiveTechs.scanning,
        );

        return kdTree.getWithinRadius(star, scanningRangeDistance);
    }

    addWaypointDestinations(
        game: Game,
        players: Player[],
        starsInScanningRange: Star[],
    ) {
    ...
    getPlayersWithinScanningRangeOfStars(
        game: Game,
        otherPlayers: Player[],
        ...
        for (let otherPlayer of otherPlayers) {
            let otherPlayerStars = this.starService.listStarsOwnedByPlayer(
                game.galaxy.stars,
                otherPlayer._id,
            );
            ...
            for (let s of otherPlayerStars) {
                if (
                    this.isStarWithinScanningRangeOfStarsBySingleTree(
                        game,
                        s,
                        scannedStarSet,
                    )
    ...
    isStarWithinScanningRangeOfStarsBySingleTree(
        game: Game,
        star: Star,
        scannedStarSet: Set<Star>,
    ) {
    ...
    listStarsWithScanningRangeByPlayers(
        game: Game,
    ...
    isObjectWithinScanningRangeOfStars(
        object: MapObject,
        starTreesWithRadius: [number, KDTree][],
    ) {
        for (const starTreeWithRadius of starTreesWithRadius) {
            if (
                starTreeWithRadius[1].isWithinRadiusOfAny(
                    object.location,
                    starTreeWithRadius[0],
                )
            )
                return true;
        }
    ...
    isStarWithinScanningRangeOfStars(
        game: Game,
        star: Star,
        starTreesWithRadius: [number, KDTree][],
    ) {
    ...
    isInScanningRangeOfPlayer(
        ...
        const targetPlayerStars = this.starService.listStarsOwnedByPlayer(
            game.galaxy.stars,
            targetPlayer._id,
        );
        ...
        if (
            ...
            starsWithScanning.length * ScanningService.SINGLE_TREE_COST_FACTOR
        ) {
            return this._isInScanningRangeOfPlayerBySingleTree(
                game,
                sourcePlayer,
                targetPlayer,
            );
        ...
        const treesWithRadius = this.getScanningStarTrees(
            game,
            starsWithScanning,
        );
    ...
    _isInScanningRangeOfPlayerBySingleTree(
        game: Game,
        sourcePlayer: Player,
        targetPlayer: Player,
    ) {
        ...
        const scannedStarSet = this.getStarSetByScanningRange(
            game,
            [sourcePlayer],
            kdTree,
        );
        ...
        return (
            this.getPlayersWithinScanningRangeOfStars(
                game,
                [targetPlayer],
                scannedStarSet,
            ).find((p) => p._id.toString() === targetPlayer._id.toString()) !=
            null
        );
    }
    ...
    getScanningStarTrees(game: Game, starsWithScanning: Star[]) {
        ...
        for (const star of starsWithScanning) {
            ...
            const scanningRangeDistance =
                this.distanceService.getScanningDistance(
                    game,
                    effectiveTechs.scanning,
                );
        ...
        for (const group of starGroups.entries()) {
            treesWithRadius.push([
                group[0],
                new KDTree(this.distanceService, group[1]),
            ]);
        }
    ...
    calculateViewpointScanning(game: Game, players: Player[]) {
        ...
        if (
            ...
            for (const star of starsWithScanning) {
                const starsInRange = this.getStarsWithinScanningRangeOfStar(
                    game,
                    star,
                    kdTree,
                );
            ...
            if (!treesWithRadius)
                treesWithRadius = this.getScanningStarTrees(
                    game,
                    starsWithScanning,
                );
        ...
        if (
            ...
            for (const carrier of game.galaxy.carriers) {
                ...
                for (const tree of treesWithRadius) {
                    if (
                        tree[1].isWithinRadiusOfAny(carrier.location, tree[0])
                    ) {
