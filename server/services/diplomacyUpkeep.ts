...
export default class DiplomacyUpkeepService {
    ...
    async deductUpkeep(
        ...
        const cycleRewards =
            this.playerCycleRewardsService.calculatePlayerCreditsEndOfCycleRewards(
                game,
                player,
            );

        let upkeep = this.getUpkeepCost(
            game,
            cycleRewards.creditsTotal,
            allianceCount,
        );
        ...
        if (saveToDB) {
            ...
            if (player.credits < upkeep.totalCost) {
                throw new ValidationError(
                    `You cannot afford to declare an alliance with this player. The upfront alliance fee is ${upkeep.totalCost} credits.`,
                );
            }
