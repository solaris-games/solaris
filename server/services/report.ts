...
export default class ReportService {
    ...
    async reportPlayer(
        game: Game,
        req: ReportCreateReportRequest<DBObjectId>,
        reportedByUserId: DBObjectId,
    ) {
        const reportedPlayer = this.playerService.getById(game, req.playerId);
        const reportedByPlayer = this.playerService.getByUserId(
            game,
            reportedByUserId,
        )!;
        ...
        if (!reportedPlayer || !reportedPlayer.userId) {
            throw new ValidationError(
                `The reported player is not a valid user.`,
            );
        }
    ...
    async conversationForReport(
        ...
        if (!report.reportedMessageId || !report.reportedConversationId) {
            throw new ValidationError(
                "Report does not have a conversation attached",
            );
        }
        ...
        return this.conversationService.detail(
            game!,
            report.reportedByPlayerId,
            report.reportedConversationId,
        );
    }
    ...
    async _reportFilter(
        ...
        if (isAdmin) {
            ...
            const userGameIds = (
                await this.gameListService.listActiveGames(userId)
            )
                .concat(
                    await this.gameListService.listUserCompletedGames(userId),
                )
                .map((game) => game._id.toString());

            return (report) =>
                !this.isUserInvolved(report, userId, userGameIds);
        }
    }
