...
export default class ScheduleBuyService extends EventEmitter {
    ...
    async buyScheduledInfrastructure(
        game: Game,
        ...
        for (let player of game.galaxy.players) {
            ...
            for (let action of currentActions) {
                try {
                    ...
                    const report =
                        this.starUpgradeService.generateUpgradeBulkReport(
                            game,
                            player,
                            action.buyType,
                            action.infrastructureType,
                            amount,
                        );
                    ...
                    await this.starUpgradeService.executeBulkUpgradeReport(
                        game,
                        player,
                        report,
                        eventService,
            ...
            try {
                ...
                const percentageActions = currentActions.filter(
                    (a) => a.buyType == "percentageOfCredits",
                );
                const totalPercentage = percentageActions.reduce(
                    (total, cur) => total + cur.amount,
                    0,
                );
                await this._executePercentageAction(
                    game,
                    player,
                    percentageActions,
                    totalPercentage,
    ...
    async _executePercentageAction(
        game: Game,
        player: Player,
        percentageActions: PlayerScheduledActions[],
        totalPercentage: number,
        eventService: IEventService,
        statisticsService: IStatisticsService,
    ) {
        for (let action of percentageActions) {
            const percentageToCredits = Math.floor(
                (action.amount / Math.max(totalPercentage, 100)) *
                    player.credits,
            );

            // pass as total credits since percentage was already calculated
            const report = this.starUpgradeService.generateUpgradeBulkReport(
                game,
                player,
                "totalCredits",
                action.infrastructureType,
    ...
    async toggleBulkRepeat(game: Game, player: Player, actionId: DBObjectId) {
        const action = player.scheduledActions.find(
        ...
        await this.gameRepo.updateOne(
            ...
            {
                $set: {
                    "galaxy.players.$[p].scheduledActions.$[a].repeat":
                        action.repeat,
                    "galaxy.players.$[p].scheduledActions.$[a].active":
                        action.active,
                },
            },
            {
