...
export default class ScheduleBuyService extends EventEmitter {
    ...
    async buyScheduledInfrastructure(game: Game, eventService: IEventService, statisticsService: IStatisticsService) {
        for (let player of game.galaxy.players) {
            ...
            for (let action of currentActions) {
                try {
                    ...
                    await this.starUpgradeService.executeBulkUpgradeReport(game, player, report, eventService, statisticsService);
                } catch (e) {
                    log.error(e)
                }
            }
            ...
            try {
                ...
                await this._executePercentageAction(game, player, percentageActions, totalPercentage, eventService, statisticsService);
            } catch (e) {
                log.error(e)
            }
    ...
    async _executePercentageAction(game: Game, player: Player, percentageActions: PlayerScheduledActions[], totalPercentage: number, eventService: IEventService, statisticsService: IStatisticsService) {
