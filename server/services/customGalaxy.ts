...
export default class CustomGalaxyService {
    ...
    validateAndCompleteCustomGalaxy(
        ...
        if (starIdSet.size !== customGalaxy.stars.length) {
            throw new ValidationError(
                `Multiple stars cannot have the same ID.`,
            );
        }

        const validStarSpecialistIdSet = new Set<number>(
            this.specialistService
                .listStar(null)
                .filter((s) => s.active.custom)
                .map((s) => s.id),
        );
        ...
        for (const star of customGalaxy.stars) {
            if (star.wormHoleToStarId != null) {
                if (star.wormHoleToStarId === star.id) {
                    throw new ValidationError(
                        `A star cannot have a worm hole to itself.`,
                    );
                }

                if (!starIdSet.has(star.wormHoleToStarId)) {
                    throw new ValidationError(
                        `A star with ID '${star.wormHoleToStarId}' does not exist.`,
                    );
                }
            }
            ...
            if (
                ...
                !validStarSpecialistIdSet.has(star.specialistId)
            ) {
                throw new ValidationError(
                    `A star specialist with ID '${star.specialistId}' does not exist.`,
                );
            }
            ...
            if (star.playerId == null) {
                if (star.homeStar) {
                    throw new ValidationError(
                        `Home stars must have a player ID.`,
                    );
                }
            ...
            if (
                ...
                if (
                    ...
                    throw new ValidationError(
                        `Dead stars must have 0 infrastructure, no specialists and no warp gates.`,
                    );
                }
            }
        }
        ...
        if (isAdvancedCustomGalaxy) {
            this._validateAdvancedCustomGalaxy(
                customGalaxy,
                starIdSet,
                settings,
            );
        ...
        settings.general.playerLimit =
            settings.galaxy.customGalaxy!.stars.filter(
                (s) => s.homeStar,
            ).length;

        if (
            settings.galaxy.customGalaxy!.stars.find(
                (s) =>
                    s.naturalResources.economy != s.naturalResources.industry ||
                    s.naturalResources.economy != s.naturalResources.science,
            ) != null
        ) {
        ...
        if (isAdvancedCustomGalaxy) {
            if (settings.general.mode === "teamConquest") {
                settings.conquest.teamsCount =
                    settings.galaxy.customGalaxy!.teams!.length;
                settings.diplomacy.maxAlliances =
                    settings.galaxy.customGalaxy!.players!.length - 1;
            }
        }
    }

    private _validateAdvancedCustomGalaxy(
        customGalaxy: CustomGalaxy,
        starIdSet: Set<string>,
        settings: GameSettingsReq,
    ) {
        if (!customGalaxy.players) {
            throw new ValidationError(
                `Missing required field 'players' in custom galaxy JSON.`,
            );
        }

        const playerIdSet = new Set<string>(
            customGalaxy.players.map((p) => p.id),
        );
        if (playerIdSet.size !== customGalaxy.players.length) {
            throw new ValidationError(
                `Multiple players cannot have the same ID.`,
            );
        }
        ...
        for (const player of customGalaxy.players) {
            if (!starIdSet.has(player.homeStarId)) {
                throw new ValidationError(
                    `A star with ID '${player.homeStarId}' does not exist.`,
                );
            }
        ...
        if (playerHomeStarIds.length !== homeStarPlayerIdMap.size) {
            throw new ValidationError(
                `Multiple players cannot have the same capital star.`,
            );
        }
        ...
        if (settings.general.mode === "teamConquest") {
            if (!customGalaxy.teams) {
                throw new ValidationError(
                    `Missing required field 'teams' in custom galaxy JSON.`,
                );
            }

            const allTeamsPlayerIds = customGalaxy.teams.flatMap(
                (t) => t.players,
            );

            if (allTeamsPlayerIds.length < customGalaxy.players.length) {
                throw new ValidationError(
                    `In a team game, all players must be in a team.`,
                );
            }

            if (
                new Set<string>(allTeamsPlayerIds).size !==
                allTeamsPlayerIds.length
            ) {
            ...
            for (const teamPlayerId of allTeamsPlayerIds) {
                if (!playerIdSet.has(teamPlayerId)) {
                    throw new ValidationError(
                        `A player with ID '${teamPlayerId}' does not exist.`,
                    );
                }
            }
        }
        ...
        for (const star of customGalaxy.stars) {
            if (star.playerId != null && !playerIdSet.has(star.playerId)) {
                throw new ValidationError(
                    `A player with ID '${star.playerId}' does not exist.`,
                );
            }

            if (
                star.homeStar &&
                homeStarPlayerIdMap.get(star.id) !== star.playerId
            ) {
                throw new ValidationError(
                    `All capital stars must belong to a player and be controlled by that player.`,
                );
            }

            if (!star.homeStar && homeStarPlayerIdMap.get(star.id)) {
                throw new ValidationError(
                    `A player's capital star must have the 'homeStar' field set to true.`,
                );
            }
        }
        ...
        if (customGalaxy.carriers) {
            if (customGalaxy.carriers.length > 500) {
                throw new ValidationError(
                    `A maximum of 500 carriers are allowed in an advanced custom galaxy.`,
                );
            }

            const carrierIdSet = new Set<string>(
                customGalaxy.carriers.map((c) => c.id),
            );
            if (carrierIdSet.size !== customGalaxy.carriers.length) {
                throw new ValidationError(
                    `Multiple carriers cannot have the same ID.`,
                );
            }

            const validCarrierSpecialistIdSet = new Set<number>(
                this.specialistService
                    .listCarrier(null)
                    .filter((s) => s.active.custom)
                    .map((s) => s.id),
            ...
            for (const carrier of customGalaxy.carriers) {
                if (!playerIdSet.has(carrier.playerId)) {
                    throw new ValidationError(
                        `A player with ID '${carrier.playerId}' does not exist.`,
                    );
                }

                if (
                    carrier.orbiting != null &&
                    !starIdSet.has(carrier.orbiting)
                ) {
                    throw new ValidationError(
                        `A star with ID '${carrier.orbiting}' does not exist.`,
                    );
                }
                ...
                if (carrier.orbiting == null) {
                    if (carrier.waypoints.length === 0) {
                        throw new ValidationError(
                            `In-flight carriers must have at least 1 waypoint.`,
                        );
                    }

                    if (carrier.waypoints[0].delayTicks !== 0) {
                        throw new ValidationError(
                            `The delay of the first waypoint of an in-flight carrier must be 0.`,
                        );
                    }

                    if (carrier.progress === undefined) {
                        throw new ValidationError(
                            `In-flight carriers must have a progress value.`,
                        );
                    }
                }
                ...
                for (let wi = 0; wi < carrier.waypoints.length; wi++) {
                    if (!starIdSet.has(carrier.waypoints[wi].source)) {
                        throw new ValidationError(
                            `A star with ID '${carrier.waypoints[wi].source}' does not exist.`,
                        );
                    }

                    if (!starIdSet.has(carrier.waypoints[wi].destination)) {
                        throw new ValidationError(
                            `A star with ID '${carrier.waypoints[wi].destination}' does not exist.`,
                        );
                    }

                    if (wi !== 0) {
                        if (
                            carrier.waypoints[wi - 1].destination !==
                            carrier.waypoints[wi].source
                        ) {
                            throw new ValidationError(
                                `The destination of a waypoint must be the source of the subsequent waypoint.`,
                            );
                        }
                    }
                }
                ...
                if (
                    ...
                    !validCarrierSpecialistIdSet.has(carrier.specialistId)
                ) {
                    throw new ValidationError(
                        `A carrier specialist with ID '${carrier.specialistId}' does not exist.`,
                    );
                }
            }
        } else {
            // Ensure that all players have a home star and vice versa.
            const starPlayerIds = customGalaxy.stars
                .map((s) => s.playerId)
                .filter((pid) => pid != null);
            const homeStarPlayerIds = customGalaxy.stars
                .filter((s) => s.homeStar)
                .map((s) => s.playerId!);
            ...
            if (homeStarPlayerIds.length !== homeStarPlayerIdSet.size) {
                throw new ValidationError(
                    `A player can have only one home star.`,
                );
            }
            ...
            for (const starPlayerId of starPlayerIds) {
                if (!homeStarPlayerIdSet.has(starPlayerId)) {
                    throw new ValidationError(
                        `All players must have a home star.`,
                    );
                }
            }
        }
        ...
        if (game.settings.general.mode === "teamConquest") {
            ...
            for (let ti = 0; ti < numTeams; ti++) {
                ...
                for (let pi = 0; pi < customTeamPlayerIds.length; pi++) {
                    const customPlayer = customGalaxy.players!.find(
                        (p) => p.id === customTeamPlayerIds[pi],
                    )!;
    ...
    private _createPlayer(
        game: Game,
        shapeColour: PlayerColourShapeCombination,
        customPlayer: CustomGalaxyPlayer,
        generatedPlayers: Map<string, Player>,
    ) {
        const player = this.playerService.createEmptyPlayer(
            game,
            shapeColour.colour,
            shapeColour.shape,
        );
        ...
        player.research = {
            ...
            experimentation: {
                level: customPlayer.technologies.experimentation,
            },
    ...
    generateStarsAdvanced(
        ...
        const starNames = this.nameService.getRandomStarNames(
            customGalaxy.stars.length,
        );
        ...
        for (let i = 0; i < customGalaxy.stars.length; i++) {
            ...
            const star = this._createUnownedCustomGalaxyStar(
                starName,
                customStar,
            );

            if (customStar.playerId != null) {
                star.ownedByPlayerId = generatedPlayers.get(
                    customStar.playerId,
                )!._id;

                if (customStar.homeStar) {
                    generatedPlayers.get(customStar.playerId)!.homeStarId =
                        star._id;
                }
            }
        ...
        for (const customStar of customGalaxy.stars) {
            if (customStar.wormHoleToStarId != null) {
                ...
                const targetStar = generatedStars.get(
                    customStar.wormHoleToStarId,
                )!;
    ...
    generateCarriers(
        game: Game,
        generatedPlayers: Map<string, Player>,
        generatedStars: Map<string, Star>,
        customGalaxy: CustomGalaxy,
    ) {
        ...
        for (const customCarrier of customGalaxy.carriers) {
            ...
            for (const customWaypoint of customCarrier.waypoints) {
                ...
                const destination = generatedStars.get(
                    customWaypoint.destination,
                )!._id;
            ...
            if (customCarrier.orbiting != null) {
                ...
                const name = customCarrier.name
                    ? customCarrier.name
                    : this.carrierService.generateCarrierName(
                          carrierStar,
                          carriers,
                      );

                carriers.push(
                    this._createCustomGalaxyCarrier(
                        name,
                        ownerId,
                        carrierStar.location,
                        carrierStar._id,
                        waypoints,
                        customCarrier,
                    ),
                ...
                const carrierOriginStar = generatedStars.get(
                    customCarrier.waypoints[0].source,
                )!;
                const carrierDestinationStar = generatedStars.get(
                    customCarrier.waypoints[0].destination,
                )!;
                const name = customCarrier.name
                    ? customCarrier.name
                    : this.carrierService.generateCarrierName(
                ...
                const location = this._progressAlongPathToLocation(
                    customCarrier.progress!,
                    carrierOriginStar.location,
                    carrierDestinationStar.location,
                );
    ...
    _createUnownedCustomGalaxyStar(name: string, customStar: CustomGalaxyStar) {
        return {
            ...
            ships: customStar.shipsActual
                ? Math.floor(customStar.shipsActual)
                : undefined,
    ...
    _getLinkedStarIds(
        ...
        generatedStars: Map<string, Star>,
        customStars: CustomGalaxyStar[],
    ) {
        const linkedStarIds: DBObjectId[] = [];
        const linkedCustomStarIds = customStars
            .filter((cs) => cs.playerId === customPlayerId)
            .map((cs) => cs.id);
    ...
    _progressAlongPathToLocation(
        progress: number,
        sourceLoc: Location,
        destinationLoc: Location,
    ): Location {
        return {
            x: sourceLoc.x + (destinationLoc.x - sourceLoc.x) * progress,
            y: sourceLoc.y + (destinationLoc.y - sourceLoc.y) * progress,
        };
    }
}
