...
export default class ResourceService {
    ...
    _distributeWeightedCenterMirrored(
        ...
        for (let i = 0; i < locations.length / playerCount; i++) {
            let radius = this.distanceService.getDistanceBetweenLocations(
                galacticCenter,
    ...
    _setResources(
        ...
        if (isSplitResources) {
            return {
                economy: this.randomService.getRandomNumberBetweenEXP(
                    minResources,
                    maxResources,
                    EXP,
                ),
                industry: this.randomService.getRandomNumberBetweenEXP(
                    minResources,
                    maxResources,
                    EXP,
                ),
                science: this.randomService.getRandomNumberBetweenEXP(
                    minResources,
                    maxResources,
                    EXP,
                ),
        ...
        let resources = this.randomService.getRandomNumberBetweenEXP(
            minResources,
            maxResources,
            EXP,
        );
