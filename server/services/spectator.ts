...
export default class SpectatorService {
    ...
    async invite(game: Game, player: Player, usernames: string[]) {
        if (!this.isSpectatingEnabled(game)) {
            throw new ValidationError(
                `Spectating is not enabled in this game.`,
            );
        }

        const requestUser = async (
            username: string,
        ): Promise<{ username: string; user: User | null }> => {
        ...
        const attemptedUsers: { username: string; user: User | null }[] =
            await Promise.all(
                usernames.map((username) => requestUser(username)),
            );
        ...
        if (!users.length) {
            throw new ValidationError(
                `No users found for the provided usernames: ${missingUsernames.join(", ")}`,
            );
        }
        ...
        if (missingUsernames.length !== 0) {
            throw new ValidationError(
                `The following usernames were not found: ${missingUsernames.join(", ")}`,
            );
        }
    }
    ...
    async listSpectators(game: Game): Promise<GameSpectator[] | null> {
        ...
        for (const player of game.galaxy.players.filter((p) => p.spectators)) {
            userIds = userIds.concat(
                player.spectators.map((s) => s.toString()),
            );
        }
    ...
    listSpectatingPlayers(game: Game, userId: DBObjectId) {
        return game.galaxy.players.filter((p) =>
            p.spectators.find((s) => s.toString() === userId.toString()),
        );
    }
}
