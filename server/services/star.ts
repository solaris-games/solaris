...
export default class StarService extends EventEmitter {
    ...
    //@Return int: how much the specialist on the star costs, 0 if no specialist.
    resetStarsExcludingSpecs (game: Game, star: Star, player: Player) {
        star.shipsActual = Math.max(game.settings.player.startingShips, +star.homeStar); // Must be at least 1 star at the home star so that a carrier can be built there.
        star.ships = star.shipsActual;
        star.warpGate = false; // TODO: BUG - This resets warp gates generated by map terrain.
        ...
        if (starSpecialist) {
            return this.specialistService.getSpecialistActualCost(game, starSpecialist) // Return how much the specialist costs
        } else {
            return {credits: 0, creditsSpecialists: 0}; //Return 0 if no specialist
        }
