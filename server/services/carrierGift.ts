...
export default class CarrierGiftService extends EventEmitter {
    ...
    async convertToGift(game: Game, player: Player, carrierId: DBObjectId) {
        let carrier = game.galaxy.carriers.find(
            (c) => c._id.toString() === carrierId.toString(),
        )!;

        if (game.settings.specialGalaxy.giftCarriers === "disabled") {
            throw new ValidationError(
                `Gifting carriers has been disabled in this game.`,
            );
        }

        if (carrier.ownedByPlayerId!.toString() !== player._id.toString()) {
            throw new ValidationError(
                `Cannot convert carrier into a gift, you do not own this carrier.`,
            );
        }
        ...
        if (carrier.isGift) {
            throw new ValidationError(
                `The carrier has already been converted into a gift.`,
            );
        }
    ...
    transferGift(
        ...
        if (!star.ownedByPlayerId) {
            throw new ValidationError(
                `Cannot transfer ownership of a gifted carrier to this star, no player owns the star.`,
            );
        }

        const starPlayer = game.galaxy.players.find(
            (p) =>
                star.ownedByPlayerId &&
                p._id.toString() === star.ownedByPlayerId.toString(),
        )!;
        const starUser = gameUsers.find(
            (u) =>
                starPlayer.userId &&
                u._id.toString() === starPlayer.userId.toString(),
        );

        const carrierPlayer = game.galaxy.players.find(
            (p) => p._id.toString() === carrier.ownedByPlayerId!.toString(),
        )!;
        const carrierUser = gameUsers.find(
            (u) =>
        ...
        const isSamePlayer =
            starPlayer._id.toString() === carrierPlayer._id.toString();
        ...
        if (!isSamePlayer) {
            ...
            if (
                ...
                !this.diplomacyService.isDiplomaticStatusBetweenPlayersAllied(
                    game,
                    [carrierPlayer._id, starPlayer._id],
                )
            ) {
            ...
            eventService.createGiftSentEvent(
                game._id,
                game.state.tick,
                carrierPlayer,
                starPlayer,
