...
export class NotificationService implements INotificationService {
    ...
    async _getNotificationContext(
        ...
        const players = game.galaxy.players.filter(
            (p) => !playerIds || playerIds.includes(p._id.toString()),
        );
    ...
    async _trySendNotifications(
        gameId: DBObjectId,
        playerIds: string[] | null,
        type: SubscriptionType,
        event: SubscriptionEvent,
        sendNotificationCallback: (game: Game, user: User) => any,
    ) {
        ...
        for (let user of context.users) {
            ...
            if (
                user.subscriptions.settings.notifyActiveGamesOnly &&
    ...
    _generateBaseDiscordMessageTemplate(
        game: Game,
        title: string,
        description: string,
    ) {
    ...
    async onGameStarted(args: InternalGameEvent) {
        ...
        await this._trySendNotifications(
            ...
            async (game: Game, user: User) => {
                const template = this._generateBaseDiscordMessageTemplate(
                    game,
                    "Game Started",
        ...
        await this._trySendNotifications(
            ...
            async (game: Game, user: User) => {
                const template = this._generateBaseDiscordMessageTemplate(
                    game,
                    "Game Turn Ended",
                    "A turn has finished, it's your turn to play!",
                );
    ...
    async trySendLastPlayerTurnReminder(gameId: DBObjectId) {
        ...
        if (!this.gameTypeService.isTurnBasedGame(game)) {
            throw new Error(
                "Cannot send a last turn reminder for non turn based games.",
            );
        }
        ...
        const undefeatedPlayers = game.galaxy.players.filter(
            (p: Player) => !p.defeated && p.userId,
        );

        if (undefeatedPlayers.length <= 2) {
            // No need to send a turn reminder in a 2-player game
            return;
        }

        const undefeatedUnreadyPlayers = undefeatedPlayers.filter(
            (p) => !p.ready,
        );
            ...
            await this._trySendNotifications(
                ...
                async (game: Game, user: User) => {
                    const template = this._generateBaseDiscordMessageTemplate(
                        game,
                        "It's your turn",
                        `The players in ${gameName} are waiting for you to make your move!`,
                    );
    ...
    async onPlayerGalacticCycleCompleted(
        ...
        await this._trySendNotifications(
            ...
            async (game: Game, user: User) => {
                const template = this._generateBaseDiscordMessageTemplate(
                    game,
                    "Galactic Cycle Complete",
                    "A galactic cycle has finished! You have received new funds to spend upgrading your empire.",
                );
                ...
                if (
                    args.carrierUpkeep &&
                    (args.carrierUpkeep.carrierCount ||
                        args.carrierUpkeep.totalCost)
                ) {
                    template.fields.push({
                        name: "Carrier Upkeep",
                ...
                if (
                    args.allianceUpkeep &&
                    (args.allianceUpkeep.allianceCount ||
                        args.allianceUpkeep.totalCost)
                ) {
                    template.fields.push({
    ...
    async onPlayerResearchCompleted(
        gameId: DBObjectId,
        playerId: DBObjectId,
        ...
        await this._trySendNotifications(
            ...
            async (game: Game, user: User) => {
                const template = this._generateBaseDiscordMessageTemplate(
                    game,
                    "Research Complete",
                    "You have finished researching a technology.",
                );
        ...
        await this._trySendNotifications(
            ...
            async (game: Game, user: User) => {
                const template = this._generateBaseDiscordMessageTemplate(
                    game,
                    "Credits Received",
                    `You have received **$${amount}** credits from **${fromPlayer.alias}**.`,
                );
    ...
    async onPlayerCreditsSpecialistsReceived(
        gameId: DBObjectId,
        fromPlayer: Player,
        toPlayer: Player,
        amount: number,
        ...
        await this._trySendNotifications(
            ...
            async (game: Game, user: User) => {
                const template = this._generateBaseDiscordMessageTemplate(
                    game,
                    "Specialist Tokens Received",
                    `You have received **${amount}** specialist token(s) from **${fromPlayer.alias}**.`,
                );
    ...
    async onPlayerTechnologyReceived(
        gameId: DBObjectId,
        fromPlayer: Player,
        toPlayer: Player,
        ...
        await this._trySendNotifications(
            ...
            async (game: Game, user: User) => {
                const template = this._generateBaseDiscordMessageTemplate(
                    game,
                    "Technology Received",
                    `You have received **Level ${technology.level} ${technology.name}** from **${fromPlayer.alias}**.`,
                );
        ...
        await this._trySendNotifications(
            ...
            async (game: Game, user: User) => {
                const template = this._generateBaseDiscordMessageTemplate(
                    game,
                    "Renown Received",
                    `You have received **${amount}** renown from **${fromPlayer.alias}**.`,
                );
    ...
    async onConversationMessageSent(
        args: InternalConversationMessageSentEvent,
    ) {
        if (!this.discordService.isConnected()) return;
        const toPlayerIds = args.sentMessageResult.toPlayerIds.map((id) =>
            id.toString(),
        );
        const readBy = args.sentMessageResult.readBy.map((id) => id.toString());

        // Filter the players who haven't auto-read the message.
        const playerIdsToCheck = toPlayerIds.filter(
            (pid) => !readBy.includes(pid),
        );
        ...
        await this._trySendNotifications(
            ...
            async (game: Game, user: User) => {
                ...
                const formattedMessage = args.sentMessageResult.message.replace(
                    /{{(\w)\/(\w+?)\/(.+?)}}/g,
                    (match, type, id, name) => `{${name}}`,
                );

                const template = this._generateBaseDiscordMessageTemplate(
                    game,
                    `Message from ${args.sentMessageResult.fromPlayerAlias}`,
                    formattedMessage,
                );
