    ...
    maskStars(
        ...
        for (let i = 0; i < game.galaxy.stars.length; i++) {
            ...
            if (
                ...
                gameStar.ownedByPlayerId.toString() ===
                    userPlayer._id.toString()
            ) {
                continue;
            }

            const historyStar = history.stars.find(
                (x) => x.starId.toString() === gameStar._id.toString(),
            );
            ...
            if (historyStar) {
                ...
                if (
                    !isHistorical &&
                    userPlayer &&
                    historyStar.ownedByPlayerId &&
                    gameStar.ownedByPlayerId == null &&
                    historyStar.ownedByPlayerId.toString() ===
                        userPlayer._id.toString()
                ) {
                    continue;
                }
                ...
                gameStar.location =
                    historyStar.location == null ||
                    historyStar.location.x == null ||
                    historyStar.location.y == null
                        ? gameStar.location
                        : historyStar.location; // TODO: May not have history for the star (BR Mode). Can delete this in a few months after the history is cleaned.
