...
export default class PlayerCycleRewardsService {
    ...
    calculatePlayerCreditsEndOfCycleRewards(game: Game, player: Player) {
        let playerStars = this.starService.listStarsOwnedByPlayer(
            game.galaxy.stars,
            player._id,
        );
        ...
        let creditsFromBanking = this._getBankingReward(
            game,
            player,
            playerStars,
            totalEco,
        ...
        let creditsFromSpecialistsTechnology =
            this._getCreditsSpecialistsReward(game, player, playerStars);
    ...
    givePlayerCreditsEndOfCycleRewards(game: Game, player: Player) {
        let rewards = this.calculatePlayerCreditsEndOfCycleRewards(
            game,
            player,
        );
    ...
    _getBankingReward(
        ...
        let isBankingEnabled = this.technologyService.isTechnologyEnabled(
            game,
            "banking",
        );

        if (!isBankingEnabled || !playerStars.length) {
            // Players must have stars in order to get credits from banking.
            return 0;
        }
        ...
        switch (game.settings.technology.bankingReward) {
            case "standard":
                return Math.round(
                    banking * multiplier + 0.15 * banking * totalEco,
                );
            case "legacy":
        ...
        throw new Error(
            `Unsupported banking reward type: ${game.settings.technology.bankingReward}.`,
        );
    }

    _getCreditsSpecialistsReward(
        game: Game,
        player: Player,
        playerStars: Star[],
    ) {
        ...
        let isSpecialistsCreditsEnabled =
            this.technologyService.isTechnologyEnabled(game, "specialists");
        let isCreditsSpecialistsCurrency =
        ...
        throw new Error(
            `Unsupported specialist reward type: ${game.settings.technology.specialistTokenReward}.`,
        );
    }

    giveFinancialAnalystCredits(game: Game) {
        for (let player of game.galaxy.players) {
            let playerStars = this.starService
                .listStarsOwnedByPlayer(game.galaxy.stars, player._id)
                .filter((s) => !this.starDataService.isDeadStar(s));

            for (let star of playerStars) {
                let creditsByScience =
                    this.specialistService.getCreditsPerTickByScience(star);

                player.credits +=
                    creditsByScience * star.infrastructure.science!;
            }
        }
    }
}
