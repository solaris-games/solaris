...
function getNewConversation(
    ...
    name: string,
    participantIds: DBObjectId[],
): Conversation<DBObjectId> {
    ...
    if (
        playerId &&
        !participantIds.find((x) => x.toString() === playerId.toString())
    ) {
    ...
    if (participantIds.length < 2) {
        throw new ValidationError(
            `There must be at least 2 participants including yourself.`,
        );
    }
    ...
    const existingConvo = game.conversations
        ...
        .find((c) =>
            arrayIsEqual(
                ...
                participantIds.map((pid) => pid.toString()),
            ),
        );

    if (existingConvo && existingConvo.name === name) {
        throw new ValidationError(
            `A conversation already exists with the same participants and name.`,
        );
    }
...
export default class ConversationService extends EventEmitter {
    ...
    async create(
        ...
        const newConvo = getNewConversation(
            game,
            playerId,
            name,
            participantIds,
    ...
    createConversationAllPlayers(game: Game): void {
        ...
        const participantIds: DBObjectId[] = game.galaxy.players.map(
            (p) => p._id,
        );
    ...
    async list(game: Game, playerId: DBObjectId) {
        ...
        let convos = game.conversations.filter((c) =>
            c.participants.find((p) => p.toString() === playerId.toString()),
        );

        return convos.map((c) => {
            const msgs: ConversationMessage<DBObjectId>[] =
                c.messages as ConversationMessage<DBObjectId>[];
            ...
            const unreadCount = msgs.filter(
                (m) =>
                    m.readBy.find(
                        (r) => r.toString() === playerId.toString(),
                    ) == null,
            ).length;
            const isMuted =
                c.mutedBy!.find((m) => m.toString() === playerId.toString()) !=
                null;
    ...
    async privateChatSummary(
        ...
        let convo = convos
            ...
            .find((c) => {
                return (
                    c.participants.find(
                        (a) => a.toString() === playerIdA.toString(),
                    ) &&
                    c.participants.find(
                        (b) => b.toString() === playerIdB.toString(),
                    )
                );
            });
    ...
    _getConversationForPlayer(
        ...
        convo.messages.forEach((cm) => {
            ...
            cm.readBy = cm.readBy.filter(
                (pid) => pid.toString() === playerId.toString(),
            );
        });

        if (convo.lastMessage != null) {
            convo.lastMessage.readBy = convo.lastMessage.readBy.filter(
                (pid) => pid.toString() === playerId.toString(),
            );
        }
    ...
    async detail(game: Game, playerId: DBObjectId, conversationId: DBObjectId) {
        let convo = this._getConversationForPlayer(
            game,
            conversationId,
            playerId,
        );
        ...
        convo.isMuted =
            convo.mutedBy!.find((m) => m.toString() === playerId.toString()) !=
            null;
        ...
        if (convo.participants.length === 2) {
            ...
            const playerIdB = convo.participants.filter(
                (p) => p.toString() !== playerIdA.toString(),
            )[0];
            ...
            let tradeEvents =
                await this.tradeService.listTradeEventsBetweenPlayers(
                    game,
                    playerId,
                    convo.participants,
                );
            ...
            let diploEvents =
                await this.diplomacyService.listDiplomacyEventsBetweenPlayers(
                    game,
                    playerIdA,
                    playerIdB,
                );
        ...
        convo.messages = convo.messages.sort(
            (a, b) =>
    ...
    async sendSystemMessage(
        ...
        return await this._pushMessage(
            game,
            conversation,
            toPlayerIds,
            newMessage,
    ...
    async send(
        ...
        if (
            !newMessage.readBy.find(
                (r) => r.toString() === player._id.toString(),
            )
        ) {
            newMessage.readBy.push(player._id);
        }

        const toPlayerIds = convo.participants.filter(
            (p) => p.toString() !== player._id.toString(),
        );
    ...
    async _pushMessage(
        ...
        newMessage: ConversationMessage<DBObjectId>,
        notificationService: INotificationService,
    ) {
    ...
    async markConversationAsRead(
        ...
        let unreadMessages = (
            convo.messages as ConversationMessage<DBObjectId>[]
        )
            .filter(
                (m) =>
                    m.readBy.find(
    ...
    leaveAll(game: Game, playerId: DBObjectId) {
        let convos = game.conversations.filter(
            (c) => c.createdBy && c.participants.indexOf(playerId) > -1,
        );

        for (let convo of convos) {
            convo.participants.splice(convo.participants.indexOf(playerId), 1);
        }
    }
    ...
    getUnreadCount(game: Game, playerId: DBObjectId) {
        return (game.conversations || [])
            ...
            .reduce((sum, c) => {
                return (
                    sum +
                    (c.messages as ConversationMessage<DBObjectId>[]).filter(
                        (m) =>
                            m.readBy.find(
                                (r) => r.toString() === playerId.toString(),
                            ) == null,
    ...
    async pinMessage(
        ...
        return await this.setPinnedMessage(
            game,
            conversationId,
            messageId,
            true,
