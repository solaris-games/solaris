...
export default class RandomService {
    ...
    getRandomRadiusInRange(minRadius: number, maxRadius: number): number {
        return (
            (Math.random() * (maxRadius ** 2 - minRadius ** 2) +
                minRadius ** 2) **
            0.5
        );
    }
    ...
    getRandomPositionInCircleFromOrigin(
        originX: number,
        originY: number,
        radius: number,
    ): Location {
    ...
    generateStarNaturalResources(
        ...
        if (fuzzy) {
            ...
            let floorFuzzyNR = Math.max(
                minResources,
                naturalResources - FUZZY_LIMIT,
            );
            let ceilFuzzyNR = Math.min(
                maxResources,
                naturalResources + FUZZY_LIMIT,
            );

            naturalResources = this.getRandomNumberBetween(
                floorFuzzyNR,
                ceilFuzzyNR,
            );
        }
