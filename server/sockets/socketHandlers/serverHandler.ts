...
import ServerSocketEventNames, {
    ServerSocketEventType,
} from "../socketEventNames/server";
...
export class ServerHandler extends EventEmitter {
    ...
    constructor(server: Server, logger: Logger) {
        ...
        this.socketOn(
            server,
            ServerSocketEventNames.Connection,
            async (socket: Socket) => {
        ...
        this.socketOn(
            server.engine,
            ServerSocketEventNames.ConnectionError,
            (err: Error) => {
                if (err) {
                    logger.error(
                        `Socket error: ${err.name}: ${err.message}, ${err.stack}`,
                    );
                }
            },
        );
    ...
    protected socketOn<
        TSocketEventName extends EventName<ServerSocketEventType, TData>,
