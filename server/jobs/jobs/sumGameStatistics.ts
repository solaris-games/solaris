...
export const sumGameStatisticsJob =
    (container: DependencyContainer) => async () => {
        try {
            ...
            const slices =
                await statisticsService.getClosedUnprocessedSlicesActive();

            const gamesToSlices = groupBy(
                slices,
                (slice: StatsSlice<DBObjectId>) => slice.gameId.toString(),
            );

            for (let [gameId, slicesForGame] of gamesToSlices) {
                const game = await gameService.getByIdLean(
                    objectIdFromString(gameId),
                    {
                        "galaxy.players": 1,
                        settings: 1,
                        state: 1,
                ...
                if (!game.state.endDate) {
                    log.warn(
                        `Game with ID ${gameId} is not finished, skipping...`,
                    );
                    continue;
                }
                ...
                log.info(
                    `Processed ${slicesForGame.length} slices for game ${gameId}`,
                );
            }
