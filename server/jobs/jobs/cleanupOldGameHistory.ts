...
export const cleanupOldGameHistoryJob =
    (container: DependencyContainer) => async () => {
        try {
            const games =
                await container.gameListService.listOldCompletedGamesNotCleaned(
                    months,
                );
            ...
            for (let i = 0; i < games.length; i++) {
                ...
                try {
                    ...
                    await container.initialGameStateService.deleteByGameId(
                        game._id,
                    );
