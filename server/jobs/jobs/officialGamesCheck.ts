...
import {
    OfficialGameCategory,
    OfficialGameKind,
} from "../../config/officialGames";
import { logger } from "../../utils/logging";
import { GameSettingsReq } from "../../services/gameCreate";
import {
    getOfficialGameCategoryName,
    getOfficialGameSettings,
} from "../../services/officialGames";
...
const chooseSetting = (
    ...
    if (category.kind === OfficialGameKind.Standard) {
        ...
        return category.rotation[
            container.randomService.getRandomNumber(
                category.rotation.length - 1,
            )
        ];
    } else {
...
const findExistingGame = (setting: OfficialGameCategory, games: Game[]) => {
    if (setting.kind === OfficialGameKind.Standard) {
        return games.find(
            (x) => x.settings.general.type === setting.settings.general.type,
        );
...
export const officialGamesCheckJob =
    (container: DependencyContainer) => async () => {
        try {
            ...
            const openGames =
                await container.gameListService.listOfficialGames();
            const runningGames =
            ...
            for (const category of settings) {
                ...
                if (!existingOpen) {
                    log.info(
                        `Could not find game [${getOfficialGameCategoryName(category)}], creating it now...`,
                    );

                    const existingRunning = findExistingGame(
                        category,
                        runningGames,
                    );
                    const existingTemplate =
                        existingRunning?.settings.general.createdFromTemplate;

                    let newSetting: GameSettingsReq;
                    if (
                        existingRunning &&
                        existingTemplate &&
                        category.kind === OfficialGameKind.Carousel &&
                        category.distribution === "sequential"
                    ) {
                        const index = category.rotation.findIndex(
                            (x) =>
                                x.general.createdFromTemplate &&
                                x.general.createdFromTemplate ===
                                    existingTemplate,
                        );
                        const nextIndex =
                            (index + 1) % category.rotation.length;
                    ...
                    try {
                        const newGame =
                            await container.gameCreateService.create(
                        ...
                        log.info(
                            `${newGame.settings.general.type} game created: ${newGame.settings.general.name}`,
                        );
                    } catch (e) {
                        log.error(e);
                    }
