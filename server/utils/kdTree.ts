...
export class KDTree {
    ...
    _sortSlice(
        ...
        const sorted = array
            ...
            .sort(
                (a, b) =>
                    this.mapObjects[a.objectIndex].location[key] -
                    this.mapObjects[b.objectIndex].location[key],
            );
    ...
    radiusSearch(
        ...
        if (!current.inRange) {
            const sqDistance =
                this.distanceService.getDistanceSquaredBetweenLocations(
                    query,
                    this.mapObjects[current.objectIndex].location,
                );
        ...
        if (current.inRange) {
            ...
            if (
                (n <= begin || this.nodes[left].marked) &&
                (end <= n + 1 || this.nodes[right].marked)
            ) {
                current.marked = true;
                return;
            }
        ...
        const goLeft =
            query[key] < this.mapObjects[current.objectIndex].location[key];
    ...
    firstValidNeighbourSearch(
        ...
        if (goLeft) {
            this.firstValidNeighbourSearch(
                query,
                begin,
                n,
                radius,
