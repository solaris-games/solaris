import { ResearchProgressService, RESEARCH_TYPES_NOT_RANDOM } from '@solaris/common';
import type { ResearchTypeNotRandom } from '@solaris/common';

const service = new ResearchProgressService();

const EXPENSE_TIERS = ['cheap', 'standard', 'expensive', 'veryExpensive', 'crazyExpensive'] as const;
...
const makeGame = (tech: ResearchTypeNotRandom, expenseTier: ExpenseTier, progression: ProgressionConfig) => ({
    constants: {
        research: {
    ...
    settings: {
        technology: {
            researchCosts: Object.fromEntries(
                RESEARCH_TYPES_NOT_RANDOM.map(t => [t, t === tech ? expenseTier : 'standard'])
            ),
            researchCostProgressions: Object.fromEntries(
                RESEARCH_TYPES_NOT_RANDOM.map(t => [t, t === tech ? progression : { progression: 'standard' }])
            ),
        },
    },
});
...
// CSV header
console.log('technologyKey,expenseCost,progression,scalingFactor,growthFactor,level,requiredProgress');
    ...
    for (const expenseTier of EXPENSE_TIERS) {
        for (const progression of PROGRESSIONS) {
            ...
            for (let level = MIN_LEVEL; level <= MAX_LEVEL; level++) {
                const required = service.getRequiredResearchProgress(game, tech, level);
                console.log([
                    tech,
                    expenseTier,
