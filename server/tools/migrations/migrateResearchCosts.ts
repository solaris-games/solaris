...
const migrateGame = (log: Logger, game: Game) => {
    if (game.settings.technology.researchCostProgressions) {
        log.info(
            `Game ${game._id} already has research cost progressions, skipping migration.`,
        );
        return null;
    }

    const oldTechSettings = game.settings
        .technology as unknown as OldGameSettingsTechnology;
    ...
    return {
        updateOne: {
            ...
            update: {
                $set: {
                    "settings.technology.researchCostProgressions":
                        newProgressions,
                },
            },
        },
