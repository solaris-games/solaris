...
import {
    ...
    InitialPlayer,
    InitialStar,
} from "../../services/types/InitialGameState";
...
export const migrateInitialGameState = async (ctx: JobParameters) => {
    ...
    for (let slimGame of games) {
        ...
        const prevInitState =
            await ctx.container.initialGameStateService.getByGameId(
                slimGame._id,
            );
        ...
        const fullGame = (await ctx.container.gameService.getByIdAll(
            slimGame._id,
        ))!;
        ...
        await ctx.container.initialGameStateService.initialGameStateRepo.insertOne(
            initialState,
        );

        log.info(
            `Added initial game state for game ${fullGame.settings.general.name} (${fullGame._id.toString()})`,
        );
    }
