...
export const migrateWormholesHistory = async (ctx: JobParameters) => {
    ...
    for (let thinGame of thinGames) {
        ...
        const thinHistories =
            await ctx.container.historyService.historyRepo.find(
                {
                    gameId: thinGame._id,
                },
                {
        ...
        log.info(
            `Found ${thinHistories.length} history entries for game ${game.settings.general.name} (${game._id.toString()})`,
        );

        for (let thinHistory of thinHistories) {
            const fullHistory: GameHistory =
                await ctx.container.historyService.historyRepo.findByIdAsModel(
                    thinHistory._id!,
                    {
                        stars: 1,
                    },
            ...
            for (const historyStar of fullHistory.stars) {
                const currentStar = game.galaxy.stars.find(
                    (s) => s._id.toString() === historyStar.starId.toString(),
                );
