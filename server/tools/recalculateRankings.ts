...
const job = makeJob(
    ...
    async ({ log, container, mongo }: JobParameters) => {
        ...
        for (let user of users) {
            user.achievements.badges = user.achievements.badges.filter(
                (b) => !AUTOMATIC_BADGE_TYPES.includes(b.badge),
            );
        }
        ...
        do {
            ...
            for (let game of games) {
                // All players
                let playerUserIds: string[] = game.galaxy.players
                    .filter((p) => p.userId)
                    .map((p) => p.userId!.toString());
                // Quitters
                let quitterUserIds: string[] = game.quitters
                    .filter((q) => q != null)
                    .map((q) => q.toString());
                // Afkers and in game afk players
                let afkerUserIds: string[] = [
                    ...new Set(
                        game.afkers
                            .filter((a) => a != null)
                            .map((a) => a.toString())
                            .concat(
                                game.galaxy.players
                                    .filter((p) => p.userId && p.afk)
                                    .map((p) => p.userId!.toString()),
                            ),
                    ),
                ] as string[];
                // In game defeated players (not afk)
                let defeatedUserIds: string[] = game.galaxy.players
                    .filter((p) => p.userId && p.defeated && !p.afk)
                    .map((p) => p.userId!.toString());
                // In game completed players (not defeated or afk)
                let completedUserIds: string[] = game.galaxy.players
                    .filter((p) => p.userId && !p.defeated && !p.afk)
                    .map((p) => p.userId!.toString());
                // All players + quitters + afkers
                let joinerUserIds: string[] = [
                    ...new Set(
                        playerUserIds
                            .concat(quitterUserIds)
                            .concat(afkerUserIds),
                    ),
                ];
                ...
                let leaderboard =
                    container.leaderboardService.getGameLeaderboard(
                        game,
                    ).leaderboard;
                ...
                if (container.gameTypeService.isTeamConquestGame(game)) {
                    let teamLeaderboard =
                        container.leaderboardService.getTeamLeaderboard(game)!;

                    game.state.teamLeaderboard =
                        teamLeaderboard.leaderboard.map((l) => l.team._id);

                    const winningTeam =
                        container.leaderboardService.getGameWinnerTeam(
                            game,
                    ...
                    const winner = container.leaderboardService.getGameWinner(
                        game,
                        leaderboard,
                    );
                ...
                const rankingResult =
                    container.gameTickService._awardEndGameRank(
                        game,
                        users,
                        false,
                    );

                const gameEndEvent = await eventRepo.findOne({
                    gameId: game._id,
                    type: EVENT_TYPES.GAME_ENDED,
                });

                if (gameEndEvent) {
                    await eventRepo.updateOne(
                        { _id: gameEndEvent!._id },
                        {
                            $set: {
                                data: rankingResult,
                            },
