...
import {
    ...
    GAME_DARK_GALAXY_MODES,
    GAME_EXPERIMENTATION_DISTRIBUTIONS,
    GAME_EXPERIMENTATION_REWARDS,
    GAME_GALAXY_TYPE,
    GAME_INFRUSTRUCTURE_COSTS,
    GAME_MODES,
    GAME_PLAYER_DISTRIBUTIONS,
    GAME_RESEARCH_COSTS,
    GAME_RESOURCE_DISTRIBUTIONS,
    GAME_SPECIALIST_COST,
    GAME_SPECIALIST_CURRENCY,
    GAME_SPECIALIST_TOKEN_REWARDS,
    GAME_TIME_MAX_TURN_WAITS,
    GAME_TIME_SPEEDS,
    GAME_TIME_START_DELAYS,
    GAME_TIME_TYPES,
    GAME_TRADE_COSTS,
    GAME_TRADE_SCANNING,
    GAME_TYPES,
    ...
    GAME_WARPGATE_COST,
    GameAllianceUpkeepCost,
    GameAwardRankTo,
    GameBankingReward,
    GameCarrierCost,
    GameCarrierUpkeepCost,
    GameDarkGalaxyMode,
    GameExperimentationDistribution,
    GameExperimentationReward,
    GameGalaxyType,
    ...
    GamePlayerType,
    GameResearchCost,
    GameResearchProgression,
    GameResourceDistribution,
    GameSettingEnabledDisabled,
    GameSettingsGalaxyBase,
    GameSettingsGameTime,
    GameSettingsGeneralBase,
    GameSettingsPlayer,
    GameSettingsSpecialGalaxyBase,
    GameSettingsTechnology,
    GameSpecialistCost,
    GameSpecialistCurrency,
    GameSpecialistTokenReward,
    GameTimeMaxTurnWait,
    GameTimeSpeed,
    GameTimeStartDelay,
    GameTimeType,
...
import {
    GameSettingsGalaxyReq,
    GameSettingsReq,
} from "../../services/gameCreate";
...
const parseGameSettingsGeneral: Validator<GameSettingsGeneralBase> = object({
    ...
    playerType: stringEnumeration<GamePlayerType, GamePlayerType[]>([
        "all",
        "establishedPlayers",
    ]),
    anonymity: stringEnumeration<GamePlayerAnonymity, GamePlayerAnonymity[]>([
        "normal",
        "extra",
        "revealAtEnd",
    ]),
    playerOnlineStatus: stringEnumeration<
        GamePlayerOnlineStatus,
        GamePlayerOnlineStatus[]
    >(["hidden", "visible"]),
    playerIPWarning: enabledDisabled,
    awardRankTo: withDefault(
        "all",
        stringEnumeration<GameAwardRankTo, GameAwardRankTo[]>(
            GAME_AWARD_RANK_TO,
        ),
    ),
    awardRankToTopN: maybeUndefined(
        numberAdv({
            integer: true,
            range: {
                from: 1,
    ...
    readyToQuitFraction: maybeUndefined(
        numberEnumeration<ReadyToQuitFraction, ReadyToQuitFraction[]>(
            READY_TO_QUIT_FRACTIONS,
        ),
    ),
    readyToQuitTimerCycles: maybeUndefined(
        numberEnumeration<ReadyToQuitTimerCycles, ReadyToQuitTimerCycles[]>(
            READY_TO_QUIT_TIMER_CYCLES,
        ),
    ),
    readyToQuitVisibility: withDefault(
        "visible",
        stringEnumeration<ReadyToQuitVisibility, ReadyToQuitVisibility[]>(
            READY_TO_QUIT_VISIBILITY,
        ),
    ),
...
const parseGameSettingsGalaxy: Validator<GameSettingsGalaxyReq> = object({
    galaxyType: stringEnumeration<GameGalaxyType, GameGalaxyType[]>(
        GAME_GALAXY_TYPE,
    ),
    ...
    customGalaxy: maybeUndefined((str) =>
        customGalaxyValidator(JSON.parse(str)),
    ),
...
const parseGameSettingsSpecialGalaxy: Validator<GameSettingsSpecialGalaxyBase> =
    object({
        carrierCost: stringEnumeration<GameCarrierCost, GameCarrierCost[]>(
            GAME_CARRIER_COST,
        ),
        carrierUpkeepCost: stringEnumeration<
            GameCarrierUpkeepCost,
            GameCarrierUpkeepCost[]
        >(GAME_CARRIER_UPKEEP_COST),
        warpgateCost: stringEnumeration<GameWarpgateCost, GameWarpgateCost[]>(
            GAME_WARPGATE_COST,
        ),
        specialistCost: stringEnumeration<
            GameSpecialistCost,
            GameSpecialistCost[]
        >(GAME_SPECIALIST_COST),
        specialistsCurrency: stringEnumeration<
            GameSpecialistCurrency,
            GameSpecialistCurrency[]
        >(GAME_SPECIALIST_CURRENCY),
        ...
        darkGalaxy: stringEnumeration<GameDarkGalaxyMode, GameDarkGalaxyMode[]>(
            GAME_DARK_GALAXY_MODES,
        ),
        ...
        resourceDistribution: stringEnumeration<
            GameResourceDistribution,
            GameResourceDistribution[]
        >(GAME_RESOURCE_DISTRIBUTIONS),
        playerDistribution: stringEnumeration<
            GamePlayerDistribution,
            GamePlayerDistribution[]
        >(GAME_PLAYER_DISTRIBUTIONS),
...
const parseGameSettingsConquest = object({
    victoryCondition: stringEnumeration<
        GameVictoryCondition,
        GameVictoryCondition[]
    >(GAME_VICTORY_CONDITIONS),
    victoryPercentage: numberEnumeration<
        GameVictoryPercentage,
        GameVictoryPercentage[]
    >(GAME_VICTORY_PERCENTAGES),
    capitalStarElimination: enabledDisabled,
    teamsCount: maybeUndefined(
        numberAdv({
            integer: true,
            range: {
                from: 2,
...
const parseDevelopmentCost: Validator<GameInfrastructureCost> =
    stringEnumeration<GameInfrastructureCost, GameInfrastructureCost[]>(
        GAME_INFRUSTRUCTURE_COSTS,
    );
...
const parseGameSettingsPlayer: Validator<GameSettingsPlayer> = object({
    ...
    tradeCost: numberEnumeration<GameTradeCost, GameTradeCost[]>(
        GAME_TRADE_COSTS,
    ),
    tradeScanning: stringEnumeration<GameTradeScanning, GameTradeScanning[]>(
        GAME_TRADE_SCANNING,
    ),
...
const researchCosts: Validator<GameResearchCost> = stringEnumeration<
    GameResearchCost,
    GameResearchCost[]
>(GAME_RESEARCH_COSTS);

const researchCostProgression: Validator<GameResearchProgression> = withDefault(
    ...
    or(
        or(
            ...
            object({
                ...
                growthFactor: stringEnumeration<
                    "soft" | "medium" | "hard",
                    ("soft" | "medium" | "hard")[]
                >(["soft", "medium", "hard"]),
            }),
...
const parseGameSettingsTechnology: Validator<GameSettingsTechnology> = object({
    ...
    bankingReward: stringEnumeration<GameBankingReward, GameBankingReward[]>(
        GAME_BANKING_REWARDS,
    ),
    experimentationDistribution: stringEnumeration<
        GameExperimentationDistribution,
        GameExperimentationDistribution[]
    >(GAME_EXPERIMENTATION_DISTRIBUTIONS),
    experimentationReward: stringEnumeration<
        GameExperimentationReward,
        GameExperimentationReward[]
    >(GAME_EXPERIMENTATION_REWARDS),
    specialistTokenReward: stringEnumeration<
        GameSpecialistTokenReward,
        GameSpecialistTokenReward[]
    >(GAME_SPECIALIST_TOKEN_REWARDS),
});
...
const parseGameSettingsGameTime: Validator<GameSettingsGameTime> = object({
    ...
    speed: withDefault(
        1800,
        numberEnumeration<GameTimeSpeed, GameTimeSpeed[]>(GAME_TIME_SPEEDS),
    ),
    ...
    startDelay: withDefault(
        240,
        numberEnumeration<GameTimeStartDelay, GameTimeStartDelay[]>(
            GAME_TIME_START_DELAYS,
        ),
    ),
    turnJumps: withDefault(
        8,
        numberAdv({
            integer: true,
            range: {
    ...
    maxTurnWait: withDefault(
        1440,
        numberEnumeration<GameTimeMaxTurnWait, GameTimeMaxTurnWait[]>(
            GAME_TIME_MAX_TURN_WAITS,
        ),
    ),
    afk: object({
        lastSeenTimeout: withDefault(
            2,
            numberAdv({
                integer: true,
                range: {
...
export const parseGameSettingsReq: Validator<GameSettingsReq> = object({
    ...
    diplomacy: object({
        ...
        upkeepCost: withDefault(
            "none",
            stringEnumeration<GameAllianceUpkeepCost, GameAllianceUpkeepCost[]>(
                GAME_ALLIANCE_UPKEEP_COST,
            ),
        ),
...
export const parseGameJoinGameRequest: Validator<
    GameJoinGameRequest<DBObjectId>
> = object({
...
export const mapToGameSaveNotesRequest = (body: any): GameSaveNotesRequest => {
    ...
    if (!keyHasStringValue(body, "notes", 0, 5000)) {
        errors.push(
            "Notes is required and must not be greater than 5000 characters.",
        );
    }
