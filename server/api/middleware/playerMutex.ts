...
export interface PlayerMutexMiddleware {
    ...
    release: (
        ignoreCheckUserInGame?: boolean,
    ) => (
        req: Request<{ gameId?: DBObjectId }>,
        res: Response,
        next: NextFunction,
    ) => Promise<any>;
}
        ...
        wait: () => {
            return async (
                ...
                try {
                    ...
                    let gamePlayers:
                        | { _id: DBObjectId; userId: DBObjectId | null }[]
                        | undefined =
                        await container.gameService.getPlayersLean(
                            objectIdFromString(req.params.gameId),
                        );
                    ...
                    let userPlayerId: DBObjectId | undefined = gamePlayers.find(
                        (p) =>
                            p.userId?.toString() ===
                    ...
                    if (
                        req.playerMutexLocks != null &&
                        req.playerMutexLocks.length > 0
                    ) {
                    ...
                    let playerIdFields: (keyof playerIdsType)[] = [
                        "playerId",
                        "withPlayerId",
                        "toPlayerId",
                    ];
                    ...
                    let requestParamsPossiblePlayerIds: playerIdsType =
                    ...
                    let requestBodyPossiblePlayerIds: playerIdsType =
                        req.body as {
                            playerId;
                            withPlayerId;
                            toPlayerId;
                    ...
                    for (let playerIdField of playerIdFields) {
                        for (let playerIdsObj of playerIdObjs) {
                            if (playerIdsObj != null) {
                                let possiblePlayerId =
                                    playerIdsObj[playerIdField]?.toString();

                                if (
                                    possiblePlayerId != null &&
                                    !playerIds.includes(possiblePlayerId)
                                ) {
                    ...
                    req.playerMutexLocks =
                        await container.gamePlayerMutexService.acquireMutexLocks(
                            req.params.gameId.toString(),
                            playerIds,
                        );
            ...
            return async (
                ...
                try {
                    ...
                    if (!ignoreCheckUserInGame) {
                        const userPlayerId: DBObjectId | undefined =
                            gamePlayers.find(
                                (p) =>
                                    p.userId?.toString() ===
                                    req.session.userId?.toString(),
                    ...
                    if (req.playerMutexLocks != null) {
                        await container.gamePlayerMutexService.releaseMutexLocks(
                            req.params.gameId.toString(),
                            req.playerMutexLocks,
                        );
                    }
