            ...
            return async (req, res, next) => {
                try {
                    ...
                    for (const [key, value] of Object.entries(options)) {
                        if (value || value == null) {
                            // If undefined/null, assume it is wanted.
                            select[key] = 1;
                        }
                    }

                    if (options.lean) {
                        req.game = await container.gameService.getByIdLean(
                            req.params.gameId,
                            select,
                        );
                    } else {
                        req.game = await container.gameService.getById(
                            req.params.gameId,
                            select,
                        );
                    }
        ...
        validateGameState: (options: GameStateValidationOptions) => {
            return (req, res, next) => {
                try {
                    if (
                        options.isUnlocked &&
                        container.gameStateService.isLocked(req.game)
                    ) {
                        throw new ValidationError(
                            "You cannot perform this action, the game is locked by the system. Please try again.",
                        );
                    }

                    if (
                        options.isInProgress &&
                        !container.gameStateService.isInProgress(req.game)
                    ) {
                        throw new ValidationError(
                            "You cannot perform this action, the game is not in progress.",
                        );
                    }

                    if (
                        options.isStarted &&
                        !container.gameStateService.isStarted(req.game)
                    ) {
                        throw new ValidationError(
                            "You cannot perform this action, the game is not in progress.",
                        );
                    ...
                    if (
                        options.isNotFinished &&
                        container.gameStateService.isFinished(req.game)
                    ) {
                        throw new ValidationError(
                            "You cannot perform this action, the game is not in progress.",
                        );
